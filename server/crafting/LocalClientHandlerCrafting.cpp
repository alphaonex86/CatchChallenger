...
void Client::useSeed(const CATCHCHALLENGER_TYPE_PLANT &plant_id,const CATCHCHALLENGER_TYPE_MAPID &mapIndex,const COORD_TYPE &x,const COORD_TYPE &y)
{
    if(!haveReputationRequirements(CommonDatapack::commonDatapack.get_plant(plant_id).requirements.reputation))
    {
        errorOutput("Don't have the requirement: "+std::to_string(CommonDatapack::commonDatapack.get_plant(plant_id).itemUsed)+" to plant as seed");
        return;
    }
    else if(objectQuantity(CommonDatapack::commonDatapack.get_plant(plant_id).itemUsed)==0)
    {
        errorOutput("Don't have the item id: "+std::to_string(CommonDatapack::commonDatapack.get_plant(plant_id).itemUsed)+" to plant as seed");
        return;
    }
    else
    {
        appendReputationRewards(CommonDatapack::commonDatapack.get_plant(plant_id).rewards.reputation);
        removeObject(CommonDatapack::commonDatapack.get_plant(plant_id).itemUsed);
...
void Client::useRecipe(const uint8_t &query_id,const uint16_t &recipe_id)
{
    ...
    const CraftingRecipe &recipe=CommonDatapack::commonDatapack.get_craftingRecipe(recipe_id);
