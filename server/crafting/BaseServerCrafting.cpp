...
void BaseServer::preload_plant_on_map_return()
{
    while(GlobalServerData::serverPrivateVariables.db_server->next())
    {
        ...
        if(!ok)
        {
            DebugClass::debugConsole(QStringLiteral("Plant id ignored because is not a number: %1").arg(GlobalServerData::serverPrivateVariables.db_server->value(0)));
            continue;
        }
        ...
        const quint32 &map_database_id=QString(GlobalServerData::serverPrivateVariables.db_server->value(1)).toUInt(&ok);
        if(!ok)
        {
        ...
        const quint8 &x=QString(GlobalServerData::serverPrivateVariables.db_server->value(2)).toUInt(&ok);
        if(!ok)
        {
        ...
        const quint8 &y=QString(GlobalServerData::serverPrivateVariables.db_server->value(3)).toUInt(&ok);
        if(!ok)
        {
        ...
        const quint8 &plant=QString(GlobalServerData::serverPrivateVariables.db_server->value(4)).toUInt(&ok);
        if(!ok)
            continue;
        ...
        const quint32 &character=QString(GlobalServerData::serverPrivateVariables.db_server->value(5)).toUInt(&ok);
        if(!ok)
            continue;
        ...
        const quint64 &plant_timestamps=QString(GlobalServerData::serverPrivateVariables.db_server->value(6)).toULongLong(&ok);
        if(!ok)
        {
