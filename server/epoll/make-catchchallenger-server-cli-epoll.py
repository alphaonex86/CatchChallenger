...
cxx_flags = "-std=c++0x -fstack-protector-all -std=c++0x -g -fno-rtti -O2 -std=gnu++11 -D_REENTRANT -fPIC -DEPOLLCATCHCHALLENGERSERVER -DQT_NO_EMIT -DEXTERNALLIBZSTD -D__linux__ -DTILED_ZLIB -DCATCHCHALLENGER_CACHE_HPS -DCATCHCHALLENGER_CLASS_ALLINONESERVER -DCATCHCHALLENGER_DB_POSTGRESQL -DCATCHCHALLENGER_DB_PREPAREDSTATEMENT -DCATCHCHALLENGER_XLMPARSER_TINYXML2 -DCATCHCHALLENGER_DYNAMIC_MAP_LIST=1 -DXXH_INLINE_ALL -DQT_NO_DEBUG -I../epoll -I. -I/usr/lib/x86_64-linux-gnu/qt5/mkspecs/linux-gcc"
...
exclude_list = [
    ...
    # server/epoll/ - client-to-server connectors (used by game-server-alone/gateway/login, not standalone)
...
_link_rc = os.system(CXX + MOLD_FLAGS + " -o " + os.path.join(BUILD_DIR, "catchchallenger-server-cli-epoll") + " " + " ".join(object_files) + " -lzstd -lz -lpq")
print('Finished')
if not has_success:
	sys.exit(123)
if _link_rc != 0:
