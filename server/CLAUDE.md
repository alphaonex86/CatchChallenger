...
- `<mapVisibility><minimize>`: `cpu` / `balanced` / `network`. `balanced` was named `network` before the view-range algorithm existed (its method keeps the name `min_network()`); `network` is `min_range()` (view rectangle `CATCHCHALLENGER_SERVER_MAP_VIEW_X/_Y` in `MapVisibilityAlgorithm.hpp`, border maps included) and there `Max` caps what ONE player sees, not what a map holds.
