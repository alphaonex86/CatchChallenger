...
- `<mapVisibility><minimize>`: `cpu` (`min_CPU()`) / `balanced` (`min_balanced()`, was named `network` before the view-range algorithm existed) / `network` (`min_network()`: view rectangle resolved at load from the datapack zoom, border maps included; there `Max` caps what ONE player sees, not what a map holds, and `Client::otherPlayerIsInRange()` follows the same rectangle). `cpu`/`balanced` stay on the LOCAL map on purpose: no other map's client list to walk (cache miss) and no distance arithmetic. The `benchmark_min_network*` binaries and their result keys keep their name and measure `min_balanced()`.
- `<mapVisibility><ViewMargin>`: `network` only, hysteresis in tiles (default 5). Inserted at the view limit, removed only past view+ViewMargin, else a player walking on the edge flip-flops and costs an insert+remove every tick instead of a 4 bytes move.
...
## Datapack push (server is headless — strip media)
When pushing a datapack to a server node (benchmark/test harness rsync), the server is headless: it needs the DATA files only, never audio or images. `CATCHCHALLENGER_EXTENSION_ALLOWED` (`general/base/GeneralVariable.hpp`) is the only set of extensions a datapack contains; the harness loads it AT RUNTIME, drops the media subset (images png/jpg/gif, audio ogg/opus) and any extension outside the list, and keeps the rest (tmx/xml/tsx/js...). Use an rsync whitelist (`--include=*/ --include=*.<ext> ... --exclude=*`) plus `--delete-excluded` so already-pushed media is pruned. Keep `skin/fighter/` as EMPTY folders (don't exclude the dir): a no-cache XML-parse boot enumerates it to build the fighter-skin list, but the `.png` inside are excluded by the media rule. Implemented in `benchmark/benchmark_remote.py:server_datapack_excludes()` / `datapack_keep_extensions()`; updating the `#define` updates the push automatically.
