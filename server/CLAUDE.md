...
- `<mapVisibility><minimize>`: `cpu` (`min_CPU()`) / `balanced` (`min_balanced()`, was named `network` before the view-range algorithm existed) / `network` (`min_range()`: view rectangle `CATCHCHALLENGER_SERVER_MAP_VIEW_X/_Y` in `MapVisibilityAlgorithm.hpp`, border maps included; there `Max` caps what ONE player sees, not what a map holds, and `Client::otherPlayerIsInRange()` follows the same rectangle). The `benchmark_min_network*` binaries and their result keys keep their name and measure `min_balanced()`.
