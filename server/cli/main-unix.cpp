...
// Generated by stage1 (server-cli --save with CATCHCHALLENGER_DATAPACK_CPP_EMIT).
// Holds the whole datapack + server settings as human-readable C++ const arrays
// placed in .rodata = flash on ESP32 (XIP): read in place, no filesystem, no
// byte deserialization. See CATCHCHALLENGER_DATAPACK_CPP_DIR in
// server/cli/CMakeLists.txt and test/ESP32.md.
...
int main(int argc, char *argv[])
{
    ...
    #ifdef CATCHCHALLENGER_CACHE_HPS
    #ifdef CATCHCHALLENGER_DATAPACK_CPP
    // datapack-cpp build (ESP32 / no-FS): the binary cache lives inside the
    // executable as a const blob. There is no "save" mode and no file to stat —
    // load the datapack straight from flash/.rodata, no syscall, no RAM copy.
    const bool save=false;
    server->setLoadFromCppData(CatchChallenger::datapack_cpp_data());
    #else
    ...
    #endif
    //LAN announce timer (off unless server-properties.xml <broadcastName> is set;
    //the value rides along in datapack-cache.bin so the NOXML/ESP32 build keeps
    //it). Periodically sends the client-discovery UDP broadcast so LAN clients
    //auto-find this server. Heap-allocated and intentionally kept for the
    //process lifetime, like the other server-wide singletons.
    TimerBroadcastAnnounce *timerBroadcastAnnounce=NULL;
    if(!GlobalServerData::serverSettings.broadcastName.empty())
    {
        timerBroadcastAnnounce=new TimerBroadcastAnnounce(
                    GlobalServerData::serverSettings.broadcastName,
                    server->getNormalSettings().server_port);
        if(timerBroadcastAnnounce->socketReady())
            if(!timerBroadcastAnnounce->start(1000))
                std::cerr << "timerBroadcastAnnounce fail to set" << std::endl;
    }
    #ifndef CATCHCHALLENGER_NOXML
    delete settings;
    #endif
