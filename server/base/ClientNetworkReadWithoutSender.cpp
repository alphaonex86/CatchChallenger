...
void ClientNetworkReadWithoutSender::doDDOSAction()
{
    if(CATCHCHALLENGER_SERVER_DDOS_MAX_VALUE>0)
    {
        ...
        while(index<size)
        {
            //ClientList::list->isNull() filters out everything that is not
            ...
            //skip removed (null) slots — maxIndex only grows, never shrinks
            if(!ClientList::list->isNull(index) &&
               ClientList::list->rw(index).getClientStat()!=Client::ClientStat::Free)
