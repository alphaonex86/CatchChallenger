...
public:
    ...
    std::vector<uint8_t> dictionary_reputation_database_to_internal;//255 == not found
    std::vector<uint8_t> dictionary_skin_database_to_internal;//255 == not found
    std::vector<uint8_t> dictionary_starter_database_to_internal;//255 == not found
    std::vector<uint8_t> dictionary_skin_internal_to_database;
    std::vector<uint8_t> dictionary_starter_internal_to_database;
    #if defined(CATCHCHALLENGER_DB_MYSQL) || defined(CATCHCHALLENGER_DB_POSTGRESQL) || defined(CATCHCHALLENGER_DB_SQLITE)
private:
    #elif CATCHCHALLENGER_DB_BLACKHOLE
    #else
