...
void PreparedDBQueryCommon::initDatabaseQueryCommonWithoutSP(const DatabaseBase::DatabaseType &type)
{
    switch(type)
    {
        ...
        PreparedDBQueryCommon::db_query_update_character_item_warehouse="UPDATE `character` SET `item_warehouse`=UNHEX('%1') WHERE `id`=%2";
        ...
        PreparedDBQueryCommon::db_query_monster_update_skill_and_endurance="UPDATE `monster` SET `skills`=UNHEX('%1'),`skills_endurance`=UNHEX('%2') WHERE `id`=%3";
        PreparedDBQueryCommon::db_query_monster_update_endurance="UPDATE `monster` SET `skills_endurance`=UNHEX('%1') WHERE `id`=%2";
        PreparedDBQueryCommon::db_query_update_monster_buff="UPDATE `monster` SET `buffs`=UNHEX('%1') WHERE `id`=%2";

        PreparedDBQueryCommon::db_query_character_by_id="SELECT `account`,`pseudo`,`skin`,`type`,`clan`,`cash`,`warehouse_cash`,`clan_leader`,`time_to_delete`,`starter`,LOWER(HEX(`allow`)),LOWER(HEX(`item`)),LOWER(HEX(`item_warehouse`)),LOWER(HEX(`recipes`)),LOWER(HEX(`reputations`)),LOWER(HEX(`encyclopedia_monster`)),LOWER(HEX(`encyclopedia_item`)),LOWER(HEX(`achievements`)),`blob_version`,`date` FROM `character` WHERE `id`=%1";
...
void PreparedDBQueryCommon::initDatabaseQueryCommonWithSP(const DatabaseBase::DatabaseType &type,const bool &useSP)
{
    ...
    switch(type)
    {
        ...
        if(useSP)
        {
            PreparedDBQueryCommon::db_query_select_monsters_by_player_id="SELECT `id`,`character`,`place`,`hp`,`monster`,`level`,`xp`,`captured_with`,`gender`,`egg_step`,`character_origin`,LOWER(HEX(`buffs`)),LOWER(HEX(`skills`)),LOWER(HEX(`skills_endurance`)),`sp` FROM `monster` WHERE `character`=%1 ORDER BY `position` ASC";
        ...
        {
            PreparedDBQueryCommon::db_query_select_monsters_by_player_id="SELECT `id`,`character`,`place`,`hp`,`monster`,`level`,`xp`,`captured_with`,`gender`,`egg_step`,`character_origin`,LOWER(HEX(`buffs`)),LOWER(HEX(`skills`)),LOWER(HEX(`skills_endurance`)) FROM `monster` WHERE `character`=%1 ORDER BY `position` ASC";
        ...
        {
            PreparedDBQueryCommon::db_query_update_monster_xp_hp_level="UPDATE monster SET hp=%1,xp=%2,level=%3,sp=%4 WHERE id=%5";
            PreparedDBQueryCommon::db_query_select_monsters_by_player_id="SELECT id,character,place,hp,monster,level,xp,captured_with,gender,egg_step,character_origin,encode(buffs,'hex'),encode(skills,'hex'),encode(skills_endurance,'hex'),sp FROM monster WHERE character=%1 ORDER BY position ASC";
            PreparedDBQueryCommon::db_query_update_monster_xp="UPDATE monster SET xp=%1,sp=%2 WHERE id=%3";
        }
        else
        {
            PreparedDBQueryCommon::db_query_update_monster_xp_hp_level="UPDATE monster SET hp=%1,xp=%2,level=%3 WHERE id=%4";
            PreparedDBQueryCommon::db_query_select_monsters_by_player_id="SELECT id,character,place,hp,monster,level,xp,captured_with,gender,egg_step,character_origin,encode(buffs,'hex'),encode(skills,'hex'),encode(skills_endurance,'hex') FROM monster WHERE character=%1 ORDER BY position ASC";
            PreparedDBQueryCommon::db_query_update_monster_xp="UPDATE monster SET xp=%1 WHERE id=%2";
        }
        #endif
        break;
