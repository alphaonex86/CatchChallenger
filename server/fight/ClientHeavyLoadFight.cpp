...
void Client::loadMonsters_return()
{
    ...
    while(GlobalServerData::serverPrivateVariables.db.next())
    {
        ...
        if(!warehouse && public_and_private_informations.playerMonster.size()>=CATCHCHALLENGER_MONSTER_MAX_WEAR_ON_PLAYER)
            warehouse=true;
        //stats
        if(ok)
        {
...
void Client::generateRandomNumber()
{
    if((randomIndex+randomSize)<CATCHCHALLENGER_SERVER_RANDOM_INTERNAL_SIZE)
    {
        ...
        sendFullPacket(0xD0,0x0009,GlobalServerData::serverPrivateVariables.randomData.mid(randomIndex+randomSize,CATCHCHALLENGER_SERVER_RANDOM_LIST_SIZE));
    }
    else
    {
        //need return to the first block
        sendFullPacket(0xD0,0x0009,GlobalServerData::serverPrivateVariables.randomData.mid(0,CATCHCHALLENGER_SERVER_RANDOM_LIST_SIZE));
    }
