...
* **epoll pays 6.96 syscalls PER DISPATCH** (3x `ioctl(FIONREAD)` + `fcntl(F_GETFL)` + 2x `read` + `epoll_wait`) against io_uring's 0.61. So epoll dispatches 10-25x less often yet issues ~28% MORE syscalls per move (0.1031 vs 0.0804). Do not describe epoll's read batching as making it syscall-efficient; the per-dispatch cost is where its syscalls go, and it is an unexplored optimisation target for the epoll backend.
