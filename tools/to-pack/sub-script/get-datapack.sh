...
#Done via: /root/script-custom/force-update-datapack-git.sh

# /root/script-custom/force-update-datapack-git.sh /usr/bin/ionice -c 3 /usr/bin/nice -n 19 /usr/bin/rsync --rsync-path="/usr/bin/nice -n 19 /usr/bin/ionice -c 3/usr/bin/rsync" -art --delete ${CATCHCHALLENGERSOURCESPATH}/../datapackorg/datapack/ ${CATCHCHALLENGERSOURCESPATH}/../datapack/ --exclude=*.xcf --exclude='* *' --exclude=*/map.png > /dev/null 2>&1
