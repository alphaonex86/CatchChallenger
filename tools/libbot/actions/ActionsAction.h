...
public:
    ...
    void insert_player(CatchChallenger::Api_protocol *api,const CatchChallenger::Player_public_informations &player,const CATCHCHALLENGER_TYPE_MAPID &mapId,const COORD_TYPE &x,const COORD_TYPE &y,const CatchChallenger::Direction &direction);
    void insert_player_all(CatchChallenger::Api_protocol *api,const CatchChallenger::Player_public_informations &player,const CATCHCHALLENGER_TYPE_MAPID &mapId,const COORD_TYPE &x,const COORD_TYPE &y,const CatchChallenger::Direction &direction);
    void remove_player(CatchChallenger::Api_protocol *api,const SIMPLIFIED_PLAYER_ID_FOR_MAP &id);
    ...
    static bool haveBeatBot(const CatchChallenger::Api_protocol_Qt  *api, const uint16_t &botFightId);
    static bool botHaveQuest(const CatchChallenger::Api_protocol_Qt  *api, const uint16_t &botId);
    static bool tryValidateQuestStep(CatchChallenger::Api_protocol_Qt  *api, const uint16_t &questId, const uint16_t &botId, bool silent);
    static void addBeatenBotFight(CatchChallenger::Api_protocol_Qt  *api,const uint16_t &botFightId);
    static bool haveNextStepQuestRequirements(const CatchChallenger::Api_protocol_Qt  *api,const CatchChallenger::Quest &quest);
    static bool haveStartQuestRequirement(const CatchChallenger::Api_protocol_Qt  *api,const CatchChallenger::Quest &quest);
    static bool startQuest(CatchChallenger::Api_protocol_Qt  *api,const CatchChallenger::Quest &quest);
    static std::vector<std::pair<uint16_t,std::string> > getQuestList(CatchChallenger::Api_protocol_Qt  *api, const uint16_t &botId);
    ...
    static void remove_to_inventory(CatchChallenger::Api_protocol_Qt  *api,const std::unordered_map<uint16_t,uint32_t> &items);
    void remove_to_inventory_slot(const std::unordered_map<uint16_t, uint32_t> &items);
    static void remove_to_inventory(CatchChallenger::Api_protocol_Qt  *api,const uint32_t &itemId,const uint32_t &quantity=1);
    void setEvents_slot(const std::vector<std::pair<uint8_t, uint8_t> > &events);
    void newEvent_slot(const uint8_t &event,const uint8_t &event_value);
    void setEvents(CatchChallenger::Api_protocol_Qt  *api,const std::vector<std::pair<uint8_t,uint8_t> > &events);
    void newEvent(CatchChallenger::Api_protocol_Qt  *api,const uint8_t &event,const uint8_t &event_value);
    void forcedEvent(CatchChallenger::Api_protocol_Qt  *api,const uint8_t &event,const uint8_t &event_value);
    void newRandomNumber_slot(const std::string &data);
    ...
    static void seed_planted(CatchChallenger::Api_protocol_Qt  *api,const bool &ok);
    void seed_planted_slot(const bool &ok);
    static void plant_collected(CatchChallenger::Api_protocol_Qt  *api,const CatchChallenger::Plant_collect &stat);
