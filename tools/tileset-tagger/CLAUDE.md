...
## Auto-detect + generator CLI (measured)
* AUTO-DETECT = KNN `suggestFromTags(minPercent>=55)` (k=3 weighted vote over VERIFIED tags only; refines untagged OR auto=guess tiles, leaves verified alone). Measure: `--evalknn <tsx|dir>` (leave-one-out, sweeps k×minPercent). On 9 tagged gen2 tilesets: 92% correct when it ANSWERS, 64% coverage (36% abstain → heuristic `MapUsageIndex::suggestCategory` fills those). k=1/3/5 all ~92%. Vision LLMs fail (5-22%) — don't re-try (categories are logical/structural, not pixel).
* GENERATOR = OVERLAPPING-pattern WFC `--wfco <model-dir> <out.png> [N W H seed]` (default N=2). Learns every NxN category PATTERN; neighbours must agree on their (N-1) overlap → reproduces local SHAPE. Strict AC-4 contradicts on sparse/heterogeneous data, so a greedy scanline fallback (honour left+up overlap, relax to a RANDOM edge if empty) always emits. Coherence (% of output 2×2 windows seen in the model): N=2 **98.9%** vs single-tile `--wfc` **27%**. The OLD `--wfc` (single-category 4-neighbour) is mushy — kept only for comparison. Both print the coherence metric.
* THE BRIDGE: `--genwfc <model-map-dir> <target-tileset-dir> <out-dir> [count W H seed]` writes real-tile `.tmx` into `map/main/generated/` (the WFC grid → `writeTmxFromGrid`, shared with struct-based `--genmap`). `--catlayers <model-dir> [out.json]` learns category→engine-layer from the model. Robust to a heuristic-only target (skip <50%-opaque tiles; alias terrain→ground, building→building-wall, bush/flower). QUALITY now gated on the HUMAN target fix-pass (auto `--suggest` over-tags cliff onto building-small/inside-*, lacks building/sign/interior content). NEXT: multi-map regions (borders/warps), per-layer under+over (not just topmost), sibling `<map>.xml`.
...
* VISUAL info (`category` = what the tile LOOKS like) is AUTO-DETECTED and applied best-effort by `--suggest`/`MapUsageIndex::suggestCategory` from: dominant drawn-on layer + effective walkability + green-vs-not colour (`TagModel::tileGreenish`) + STRUCTURE — object-blob size (connected Collisions∪over component: tiny→rock, medium→building, huge→cliff) and vertical pairing (roof directly above ⇒ building-wall). Terrain (water/grass/ledge/lava) = high-confidence; the rest (tree-trunk/building-wall/cliff/rock, tree-canopy/building-roof, grass-short/ground/path, bush/flower) = low-confidence, marked `auto=guess` and shown YELLOW in the GUI for the human to fix. Diagnose without writing via `--classify <tsx>`. Keep improving (near-dup clustering, regular-shape detection) so the fix-list shrinks. Categories like "table"; attributes `horizontalRepeat`, `horizontalMiddleRepeat` (centre repeats, borders fixed), vertical*. (Tiles never used on any map have no signal → stay untagged/red.)
