...
* VISUAL info (`category` = what the tile LOOKS like) is AUTO-DETECTED and applied best-effort (`--suggest`: layer + map usage + tile appearance/clustering), and the human only FIXES the few wrong ones — push detection to cover as much as possible so few remain. Categories like "table"; attributes like `horizontalRepeat`, `horizontalMiddleRepeat` (centre repeats, borders fixed), vertical*. (`--suggest` today auto-applies the unambiguous terrain water/grass/ledge/lava; extend it toward the ambiguous classes so the human fix-list shrinks.)
