...
void CCBackground::paint(QPainter *painter, const QStyleOptionGraphicsItem *option, QWidget *widget)
{
    ...
    if(zoom!=targetZoom)
    {
        ...
        if(treefront.isNull())
            abort();
        ddd=treefront;
        QImage transparent(ddd.toImage());
        transparent.fill(Qt::transparent);
        miniBase=QPixmap::fromImage(transparent);
        if(targetZoom>1)
        {
    ...
    QPixmap mini=miniBase;
    {
        QPainter painter2(&mini);
        painter2.drawPixmap(0,0,ddd.width(),ddd.height(),ddd);
        painter2.drawPixmap(0,0,ddd.width()-1,ddd.height(),ddd);
        painter2.drawPixmap(0,0,ddd.width()-2,ddd.height(),ddd);
        painter2.drawPixmap(0,0,ddd.width()-3,ddd.height(),ddd);
        painter2.drawPixmap(0,0,ddd.width(),ddd.height()-1,ddd);
        painter2.drawPixmap(0,0,ddd.width(),ddd.height()-2,ddd);
        painter2.drawPixmap(0,0,ddd.width(),ddd.height()-3,ddd);
    }

    auto start = std::chrono::steady_clock::now();
    painter->drawPixmap(0,0,widget->width(),widget->height(),mini);
