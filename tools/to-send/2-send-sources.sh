...
/usr/bin/rsync -avrtz --compress-level=9 --rsh='ssh -p54973' --delete --partial --progress /home/user/Desktop/CatchChallenger/tools/ root@ssh.first-world.info:/root/catchchallenger/sources/tools/ --exclude='*build*' --exclude='*Qt_5*' --exclude='*qt5*' --exclude='*.pro.user' --exclude='*.qm' --exclude='*.dll' --exclude=/to-pack/
