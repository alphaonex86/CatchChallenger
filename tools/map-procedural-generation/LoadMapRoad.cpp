...
//shared with LoadMapSea.cpp (the islets use the same cliff ring)
...
void LoadMapAll::generateRoadContent(Tiled::Map &worldMap, const SettingsAll::SettingsExtra &setting){
    ...
    scanSeaTemplates(worldMap,mapWidth,mapHeight);
    ...
    //make sure the tilesets named by the cave tiles are part of the world
    {
        ...
        //the water tilesets are loaded here too: addSeaContent runs after this
        //pass and fetchTile only finds a tileset the world already has
    ...
    while(y<h)
    {
        ...
        while(x<w)
        {
            ...
            if((zoneOrientation&(Orientation_bottom|Orientation_left|Orientation_right|Orientation_top)) != 0){
                ...
                //a SEA chunk keeps the terrain it was drawn with and skips every
                //road pass: its lane, its rock and its content are drawn on the
                //WHOLE WORLD at once by addSeaContent, because a coast does not
                //stop at a map border
                {
                    bool chunkIsWater=false;
                    if(roadCoordToIndex.find(x)!=roadCoordToIndex.cend()
                            && roadCoordToIndex.at(x).find(y)!=roadCoordToIndex.at(x).cend())
