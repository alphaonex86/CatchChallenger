...
char *DatapackDownloaderBase::proxyStringForCurl=NULL;
...
/*
do datapack checksum via sendDatapackContentBase() //then will call datapackChecksumDoneBase()
if(CommonSettingsCommon::commonSettingsCommon.datapackHashBase server checksum!=client checksum)//sha224
{
...
DatapackDownloaderBase::DatapackDownloaderBase()
{
    ...
    const unsigned int destination_proxy_ip_len=0;
    if(destination_proxy_ip_len>0)
    {
        std::string string_port("80");
        ...
        s = inet_pton(domain,"::1", buf);
        if (s <= 0)
        {
            //not ipv6, then host:port
            final_proxy_string=std::string("::1")+":"+string_port;
        }
        else
        {
            //ipv6, then [host]:port
            final_proxy_string=std::string("[")+std::string("::1")+"]:"+string_port;
        }
...
{
    newError("Download datapack error","DatapackDownloaderBase::datapackDownloadError()");
    wait_datapack_content_base=false;
}
...
void DatapackDownloaderBase::datapackFileList(const char * const data,const unsigned int &size)
{
    ...
    while(index<datapackFilesListBase.size())
    {
        ...
        {
            std::cerr << "remove the file: " << datapackPathBase() << '/' << datapackFilesListBase.at(index) << std::endl;
            if(remove((datapackPathBase()+'/'+datapackFilesListBase.at(index)).c_str())!=0)
...
void DatapackDownloaderBase::writeNewFileBase(const std::string &fileName, const std::string &data)
{
    ...
    std::string fullPath=datapackPathBase()+'/'+fileName;
...
void DatapackDownloaderBase::datapackChecksumDoneBase(const std::vector<std::string> &datapackFilesList,const std::vector<char> &hash,const std::vector<uint32_t> &partialHashList)
{
    ...
    {
        {
            if(remove((datapackPathBase()+"/pack/datapack.tar.zst").c_str())!=0 && errno!=ENOENT)
            {
                std::cerr << "Unable to remove " << datapackPathBase() << "/pack/datapack.tar.zst" << std::endl;
                abort();
            }
        }
        {
            if(remove((datapackPathBase()+"/datapack-list/base.txt").c_str())!=0 && errno!=ENOENT)
            {
                std::cerr << "Unable to remove " << datapackPathBase() << "/datapack-list/base.txt" << std::endl;
                abort();
            }
        }
    ...
    else
    {
        if(datapackFilesListBase.empty())
        {
            index_mirror_base=0;
            test_mirror_base();
            std::cerr << "Datapack is empty, get from mirror into" << datapackPathBase() << std::endl;
        }
        else
        {
            std::cerr << "Datapack don't match with server hash, get from mirror for base: local: " << binarytoHexa(hash) << " and remote: " << binarytoHexa(CommonSettingsCommon::commonSettingsCommon.datapackHashBase) << std::endl;
...
void DatapackDownloaderBase::decodedIsFinishBase(const std::vector<char> &rawData)
{
    ...
    else
    {
        ...
        if(tarDecode.decodeData(decodedData))
        {
            ...
            {
                if(!mkpath(FacilityLibGeneral::getFolderFromFile(datapackPathBase()+fileList.at(index))))
                {
                    std::cerr << "unable to mkpath file of datapack " << datapackPathBase()+fileList.at(index) << ": " << errno << std::endl;
                    return;
                }

                if(extensionAllowed.find(FacilityLibGeneral::getSuffix(fileList.at(index)))!=extensionAllowed.cend())
                {
                    FILE *file=::fopen((datapackPathBase()+fileList.at(index)).c_str(),"wb");
                    if(file)
                    {
                        if(fwrite(dataList.at(index).data(),1,dataList.at(index).size(),file)!=dataList.at(index).size())
                        {
                            fclose(file);
                            std::cerr << "unable to write file content of datapack " << datapackPathBase()+fileList.at(index) << ": " << errno << std::endl;
                            return;
                        }
                        fclose(file);
                        uint32_t h=0;
                        ...
                        utimbuf butime;butime.actime=h;butime.modtime=h;
                        #ifndef CATCHCHALLENGER_EXTRA_CHECK
                        utime((datapackPathBase()+fileList.at(index)).c_str(),&butime);
                        #else
                        if(utime((datapackPathBase()+fileList.at(index)).c_str(),&butime)!=0)
                        {
                            std::cerr << "hash cache into modification time set failed (abort) " << __FILE__ << ":" << __LINE__ << std::endl << std::endl;
                            abort();
                        }
                        ...
                        #ifdef CATCHCHALLENGER_EXTRA_CHECK
                        struct stat sb;
                        if (stat((datapackPathBase()+fileList.at(index)).c_str(), &sb) == 0)
                        {
                            if(sb.st_mtime!=h)
                            {
                                std::cerr << "hash cache into modification time wrong (abort) " << __FILE__ << ":" << __LINE__ << std::endl << std::endl;
                    ...
                    {
                        std::cerr << "unable to write file of datapack " << datapackPathBase()+fileList.at(index) << ": " << errno << std::endl;
                        return;
                    }
                }
                else
                {
                    std::cerr << "file not allowed: " << datapackPathBase()+fileList.at(index) << std::endl;
                    return;
                }
                index++;
            }
    ...
    else
    {
        ...
        else
        {
                ...
                if(found!=std::string::npos)
                {
                    ...
                    if(regex_search(fileString,DatapackDownloaderBase::regex_DATAPACK_FILE_REGEX))
                    {
                        ...
                        if(indexInDatapackList!=-1)
                        {
                            ...
                            if(!FacilityLibGeneral::isFile(datapackPathBase()+fileString))
                            {
            ...
            while(index<datapackFilesListBase.size())
            {
                ...
                std::cout << "remove: " << datapackPathBase()+datapackFilesListBase.at(index) << std::endl;
                index++;
            }
            datapackFilesListBase.clear();
            if(correctContent==0)
...
const std::vector<std::string> DatapackDownloaderBase::listDatapackBase(std::string suffix)
{
    ...
    const std::vector<FacilityLibGeneral::InodeDescriptor> &entryList=FacilityLibGeneral::listFolderNotRecursive(datapackPathBase()+suffix);//possible wait time here
    ...
    for(int index=0;index<sizeEntryList;++index)
    {
        ...
        else
        {
            ...
            else
            {
                ...
                if(::remove((datapackPathBase()+suffix+fileInfo.name).c_str())!=0)
                    std::cerr << "listDatapack(): unable remove invalid file: " << suffix << fileInfo.absoluteFilePath << ": " << errno << std::endl;
                std::cout << "remove: " << datapackPathBase()+suffix+fileInfo.name << std::endl;
            }
        }
    }
...
{
    std::vector<FacilityLibGeneral::InodeDescriptor> entryList=FacilityLibGeneral::listFolderNotRecursive(datapackPathBase()+suffix);//possible wait time here
    int sizeEntryList=entryList.size();
    for (int index=0;index<sizeEntryList;++index)
    {
        const FacilityLibGeneral::InodeDescriptor &fileInfo=entryList.at(index);
    ...
    entryList=FacilityLibGeneral::listFolderNotRecursive(datapackPathBase()+suffix);//possible wait time here
    if(entryList.size()==0)
        FacilityLibGeneral::rmpath(datapackPathBase()+suffix);
}
...
void DatapackDownloaderBase::sendDatapackContentBase()
{
    ...
    mkpath(datapackPathBase());
    datapackFilesListBase=listDatapackBase(std::string());
    std::sort(datapackFilesListBase.begin(),datapackFilesListBase.end());
    const DatapackChecksum::FullDatapackChecksumReturn &fullDatapackChecksumReturn=DatapackChecksum::doFullSyncChecksumBase(datapackPathBase());
    #ifdef CATCHCHALLENGER_EXTRA_CHECK
...
size_t DatapackDownloaderBase::WriteMemoryCallback(void *contents, size_t size, size_t nmemb, void *userp)
{
