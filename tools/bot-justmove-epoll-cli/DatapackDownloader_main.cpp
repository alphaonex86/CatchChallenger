...
void DatapackDownloaderMainSub::writeNewFileMain(const std::string &fileName, const std::string &data)
{
    ...
    std::string fullPath=datapackPathMain()+'/'+fileName;
...
void DatapackDownloaderMainSub::datapackChecksumDoneMain(const std::vector<std::string> &datapackFilesList,const std::vector<char> &hash,const std::vector<uint32_t> &partialHashList)
{
    ...
    {
        {
            if(remove((datapackPathMain()+"/pack/datapack-main-"+mainDatapackCode()+".tar.zst").c_str())!=0 && errno!=ENOENT)
            {
                std::cerr << "Unable to remove " << datapackPathMain() << "/pack/datapack-main-"+mainDatapackCode()+".tar.zst" << std::endl;
                abort();
            }
            std::cout << "remove: " << datapackPathMain()+"/pack/datapack-main-"+mainDatapackCode()+".tar.zst" << std::endl;
        }
        {
            if(remove((datapackPathMain()+"/datapack-list/main-"+mainDatapackCode()+".txt").c_str())!=0 && errno!=ENOENT)
            {
                std::cerr << "Unable to remove " << datapackPathMain() << "/datapack-list/main-"+mainDatapackCode()+".txt" << std::endl;
                abort();
            }
            std::cout << "remove: " << datapackPathMain()+"/datapack-list/main-"+mainDatapackCode()+".txt" << std::endl;
        }
    ...
    else
    {
        if(datapackFilesListMain.empty())
        {
            index_mirror_main=0;
            test_mirror_main();
            std::cerr << "Datapack is empty, get from mirror into" << datapackPathMain() << std::endl;
        }
        else
        {
            std::cerr << "Datapack don't match with server hash, get from mirror for main: local: " << binarytoHexa(hash) << " and remote: " << binarytoHexa(CommonSettingsServer::commonSettingsServer.datapackHashServerMain) << std::endl;

            if(index_mirror_main>=DatapackDownloaderMainSub::httpDatapackMirrorServerList.size())
                index_mirror_main=0;
            const std::string url=DatapackDownloaderMainSub::httpDatapackMirrorServerList.at(index_mirror_main)+"pack/diff/datapack-main-"+binarytoHexa(hash)+".tar.zst";
...
void DatapackDownloaderMainSub::test_mirror_main()
{
    if(!datapackTarMain)
    {
        const std::string url=DatapackDownloaderMainSub::httpDatapackMirrorServerList.at(index_mirror_main)+"pack/datapack-main-"+mainDatapackCode()+".tar.zst";
    ...
    {
        if(index_mirror_main>=DatapackDownloaderMainSub::httpDatapackMirrorServerList.size())
            /* here and not above because at last mirror you need try the tar.zst and after the datapack-list/main.txt, and only after that's quit */
            return;

        const std::string url=DatapackDownloaderMainSub::httpDatapackMirrorServerList.at(index_mirror_main)+"datapack-list/main-"+mainDatapackCode()+".txt";
...
void DatapackDownloaderMainSub::decodedIsFinishMain(const std::vector<char> &rawData)
{
    ...
    else
    {
        ...
        if(tarDecode.decodeData(decodedData))
        {
            ...
            {
                if(!mkpath(FacilityLibGeneral::getFolderFromFile(datapackPathMain()+fileList.at(index))))
                {
                    std::cerr << "unable to mkpath file of datapack " << datapackPathMain()+fileList.at(index) << ": " << errno << std::endl;
                    return;
                }

                if(extensionAllowed.find(FacilityLibGeneral::getSuffix(fileList.at(index)))!=extensionAllowed.cend())
                {
                    FILE *file=::fopen((datapackPathMain()+fileList.at(index)).c_str(),"wb");
                    if(file)
                    {
                        if(fwrite(dataList.at(index).data(),1,dataList.at(index).size(),file)!=dataList.at(index).size())
                        {
                            fclose(file);
                            std::cerr << "unable to write file content of datapack " << datapackPathMain()+fileList.at(index) << ": " << errno << std::endl;
                            return;
                        }
                        fclose(file);
                        uint32_t h=0;
                        ...
                        utimbuf butime;butime.actime=h;butime.modtime=h;
                        #ifndef CATCHCHALLENGER_EXTRA_CHECK
                        utime((datapackPathMain()+fileList.at(index)).c_str(),&butime);
                        #else
                        if(utime((datapackPathMain()+fileList.at(index)).c_str(),&butime)!=0)
                        {
                            std::cerr << "hash cache into modification time set failed (abort) " << __FILE__ << ":" << __LINE__ << std::endl << std::endl;
                            abort();
                        }
                        ...
                        #ifdef CATCHCHALLENGER_EXTRA_CHECK
                        struct stat sb;
                        if (stat((datapackPathMain()+fileList.at(index)).c_str(), &sb) == 0)
                        {
                            if(sb.st_mtime!=h)
                            {
                                std::cerr << "hash cache into modification time wrong (abort) " << __FILE__ << ":" << __LINE__ << std::endl << std::endl;
                    ...
                    {
                        std::cerr << "unable to write file of datapack " << datapackPathMain()+fileList.at(index) << ": " << errno << std::endl;
                        return;
                    }
                }
                else
                {
                    std::cerr << "file not allowed: " << datapackPathMain()+fileList.at(index) << std::endl;
                    return;
                }
                index++;
            }
...
void DatapackDownloaderMainSub::httpFinishedForDatapackListMain(const std::vector<char> data)
{
    ...
    else
    {
        if(!datapackTarMain)
        {
            std::cerr << "datapack-main-"+mainDatapackCode()+".tar.zst size:" << data.size()/1000 << "KB" << std::endl;
            datapackTarMain=true;
            decodedIsFinishMain(data);
            return;
        }
        else
        {
            /*ref crash here*/const std::string selectedMirror=DatapackDownloaderMainSub::httpDatapackMirrorServerList.at(index_mirror_main)+"map/main/"+mainDatapackCode()+"/";
                ...
                if(found!=std::string::npos)
                {
                    ...
                    if(regex_search(fileString,DatapackDownloaderMainSub::regex_DATAPACK_FILE_REGEX))
                    {
                        ...
                        if(indexInDatapackList!=-1)
                        {
                            ...
                            if(!FacilityLibGeneral::isFile(datapackPathMain()+fileString))
                            {
...
const std::vector<std::string> DatapackDownloaderMainSub::listDatapackMain(std::string suffix)
{
    ...
    const std::vector<FacilityLibGeneral::InodeDescriptor> &entryList=FacilityLibGeneral::listFolderNotRecursive(datapackPathMain()+suffix);//possible wait time here
    ...
    for(int index=0;index<sizeEntryList;++index)
    {
        ...
        else
        {
            ...
            else
            {
                ...
                if(::remove((datapackPathMain()+suffix+fileInfo.name).c_str())!=0)
                    std::cerr << "listDatapack(): unable remove invalid file: " << suffix << fileInfo.absoluteFilePath << ": " << errno << std::endl;
                std::cout << "remove: " << datapackPathMain()+suffix+fileInfo.name << std::endl;
            }
        }
    }
...
{
    std::vector<FacilityLibGeneral::InodeDescriptor> entryList=FacilityLibGeneral::listFolderNotRecursive(datapackPathMain()+suffix);//possible wait time here
    int sizeEntryList=entryList.size();
    for (int index=0;index<sizeEntryList;++index)
    {
        const FacilityLibGeneral::InodeDescriptor &fileInfo=entryList.at(index);
    ...
    entryList=FacilityLibGeneral::listFolderNotRecursive(datapackPathMain()+suffix);//possible wait time here
    if(entryList.size()==0)
        FacilityLibGeneral::rmpath(datapackPathMain()+suffix);
}
...
void DatapackDownloaderMainSub::sendDatapackContentMain()
{
    ...
    mkpath(datapackPathMain());
    datapackFilesListMain=listDatapackMain(std::string());
    std::sort(datapackFilesListMain.begin(),datapackFilesListMain.end());
    const DatapackChecksum::FullDatapackChecksumReturn &fullDatapackChecksumReturn=DatapackChecksum::doFullSyncChecksumMain(datapackPathMain());
