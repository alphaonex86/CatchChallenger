...
// under <out>/map/main/tuxemon/<region>/<location>/ (the reference dataset's
...
// share one folder).  File names are shortened the reference way (location/gym.tmx,
...
// Tiled: firstgid+0 = bot, +2 = door/teleport — the reference scheme.
...
private:
    ...
    std::unordered_map<std::string,std::string> fileBase_;  // slug -> output file basename (short, reference style)
    int warpsTotal_;
    int collisionCells_;
    int botsTotal_;
    int encounterZones_;
