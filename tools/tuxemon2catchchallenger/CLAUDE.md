...
* **yaml-cpp iterator proxy is a TEMPORARY.** `for(it: node) { const Node &n = it->second; ... ++it; }` DANGLES — `it->second` is a member of the iterator's temporary proxy pair; a reference to it is UB and newer yaml-cpp aborts with `YAML::InvalidNode`. COPY the node (`YAML::Node n = it->second;` — cheap shared handle). Bit us in `loadYamlEvents`.
* **Item refs resolve by LOWERCASED ENGLISH DISPLAY NAME** (engine `tempNameToItemId[str_tolower(<name>)]`), NOT the Tuxemon slug. Map floor-item objects emit `item="<nameEn(slug) lowercased>"`; DB-internal refs (evolution `item=`, attack `byitem=`) emit the NUMERIC item id via `DatapackWriter::idForItem(slug)` (collision-proof). Unresolvable slug → SKIP, never guess an id.
* **Now converted (all engine-consumed, verified by loading):** item evolutions (`<evolution type="item">`), TM/MM teach-items (`<attack byitem=>`, monster's own moveset = eligibility since Tuxemon `learn_tm` ignores eligibility), restore/revive (`<hp add="all"/>`+`<buff remove="all"/>`), repellent (`<repel>`), positive/stat statuses (`<inFight attack=/defense=/>` + positive hp + `step_interval`), area techniques (`applyOn="allEnemy"`), all 13 l18n locales (`<name lang=XX>`/`<description>`); maps: non-NPC dialogs → skinless text bots, `wander` → `lookAt="move"`, floor items → object scheme, heal events → `<step type="heal">`, `translated_dialog_choice` → `<a href>` steps, per-map `backgroundsound`, cave walk-anywhere lists (`<cave>`/`<caveNight>` + layers.xml night event), `${{...}}` template scrub; world: all 34/93 tracks transcoded (shared `WorldWriter::sanitizeTrackSlug` = tolower + non-`[a-z0-9]`→`-`, `music/<san>.opus`), faction → reputation, per-biome environment battle backgrounds.
* **Self-buff/heal target bug class:** a technique's `give`/`healing` effect targets `params[1]=="own_monster"` → emit `applyOn="themself"`; the loader defaults MISSING `applyOn` to `aloneEnemy`, so a self-buff with no applyOn silently buffs the ENEMY (and the `!emitted` fallback deals enemy damage to a pure-heal move). Always set applyOn explicitly.
* **Buff duration goes on the `<buff>` element, NOT `<effect>`** — `FightLoaderBuff` reads `duration`/`durationNumberOfTurn`/`capture_bonus` only from `<buff>`; anywhere else = silently ThisFight.
* **Out-of-bounds placements:** a Tuxemon scripted event can carry a NOMINAL tile past the map edge (`player_house_bedroom` "Intro No" x=9 on a 9-wide map) — bots/items/warps MUST be `tile>=0 && tile<w/h` or the engine warns+drops. Floor items and warps already clamp; bots now do too.
* **Pre-existing benign warnings (match the committed baseline, do NOT chase to 0):** ~15 `botsFight point already on the map` — crossing trainer sightlines both claim one corridor trigger cell; the engine keeps the first. Inherent to the source maps.
* **`convertAll()` returns the FAILURE count (0 = success)**, not the converted-map count — `main` exits non-zero on `!=0`.
