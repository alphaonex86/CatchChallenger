...
//CREATE TABLE maps(file text primary key, region text, zone text, subzone text, name text, city text, finished int);
...
{
    ui->setupUi(this);
    const QString dir=QFileDialog::getExistingDirectory(NULL,"Folder containing region directory");
    if(dir.isEmpty())
        abort();
    ...
    /*{
        ...
        while (it.hasNext()) {
            ...
            if(element.contains(regex))
            {
                ...
                std::cout << element.toStdString() << " -> " << pngDest.toStdString() << std::endl;
            }
        }
    }*/
}
...
bool MainWindow::preload_the_map(const std::string &path)
{
    ...
    while(index<size)
    {
        ...
        if(regex_search(fileName,mapFilter) && !regex_search(fileName,mapExclude))
        {
            ...
            {
                CatchChallenger::MapServer *mapServer=static_cast<CatchChallenger::MapServer *>(flat_map_list_temp.back());
