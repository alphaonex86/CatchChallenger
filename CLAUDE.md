...
* **Never put the same tile at the same position on two different layers** (a 100%-transparent tile is exempt). The real tile lives once on `Walkable`; semantic layers (Collisions/Grass/Water/Ledges) use a TRANSPARENT tile (engine only needs a non-empty gid) so nothing is drawn twice and the editor render matches the game — not the Walkable tile (duplicate) nor a visible marker/tint (obscures the map).
