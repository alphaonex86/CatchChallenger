...
* **no float/double in the server** — integer math only, it must run on FPU-less CPUs. (An armhf sysroot still needs `-mcpu=<cpu-with-fp>`: its libc uses the hard-float ABI. Never pair `-march=` with `-mcpu=`.)
...
* **The owner's external reference datapack is LOCAL-only, NOT part of this project.** Its name and path stay OUT of git (test harness reads them from `~/.config/catchchallenger-testing/config.json` `paths.datapacks`; tools take them from an argument or env). Never commit, redistribute, or ship it or its contents. It is a structural learning MODEL only — derive abstract layout/size/distribution rules from it; never copy its assets. All generated content and the target `CatchChallenger-datapack/` must be ORIGINAL and independently created.
* **HARD RULE — NO franchise wording anywhere in git:** code, comments, docs, commit messages, file/asset names. No Pokemon/Pokeball/Pokedex/Nintendo, no game/place/character name from an existing work, not even as a style reference ("gen2-style", "the Pokemon style") — say what it is instead (creature-collecting, classic GBA-era tile RPG). Only exceptions: a ROM converter's INPUT identifiers (filename globs, ROM symbol names, per-ROM output labels) and the denylists that exist to filter those words.
...
* **System lib first, embedded only as fallback** (blake3 / xxhash / tinyxml2). `EXTERNALLIB*` default ON on EVERY build incl. cross/fleet nodes — each node's (cross) sysroot is a copy of the real target hardware, so its `.so` is what the device runs; prefer it. The vendored sources compile in ONLY when `find_library()` can't locate the lib in the sysroot at configure time. Exception, applied automatically by `test/soname_compat.py` (both harnesses): when an exec node's runtime lacks the SONAME its compile node links (newer board on an older container), that lib alone is forced vendored for that node. Keep the vendored copy at the latest upstream release (e.g. general/tinyXML2 = v11, single tinyxml2.cpp + tinyxml2.hpp wrapped in `#ifndef CATCHCHALLENGER_NOXML`; see version.txt). tinyXML2 is NOT in the do-not-modify list above — updating it from upstream is allowed.
...
* **Put the work in a Python script, not in ad-hoc shell.** Anything beyond a one-off look (measuring, staging, deploying, comparing, cleaning up) goes into a script under the relevant dir, so it can be AUDITED and RERUN by someone else. A result produced by hand-typed shell is unreviewable and unreproducible, and the mistakes hide: absolute-path `pkill`/`pgrep` patterns that silently match nothing (or match the checker itself), `sed` quoting that writes invalid XML, a pipeline ending in `| tail` so a failed build reports success, a build against a stale source copy. Each of those passed silently in a shell one-liner and would have been caught the first time in a reviewed script.
...
# Project Overview
CatchChallenger is a creature-collecting MMORPG engine in C++/Qt. Designed for extremely constrained hardware (i486 66MHz, Geode LX800, MIPS2, RISC-V, RPI1). EventLoop server handles hundreds of players with 10-20MB RAM base + 1-4KB per player.
