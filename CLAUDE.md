...
* **Semantic tile layers (Collisions/Grass/Water/Ledges) each use their OWN distinct, SEMI-TRANSPARENT marker** from `map/invisible.tsx` (Collisions=red, Water=blue, Grass=green, Ledges=yellow/orange/cyan/magenta). Toggling/hiding one layer in Tiled MUST produce a visible change, so: a transparent marker is wrong (invisible); the real `Walkable` tile is wrong (identical to the tile directly below → no change on toggle); a 100%-opaque marker is wrong (it hides the layer below → toggling THAT layer shows nothing). A distinct semi-transparent colour is visible, tells layers apart, AND never hides a layer below it (in-game it is a faint tint — the engine/map2png draw every tile layer). `CCWriter::layerVisibilityGuard` asserts no tile layer is fully hidden by a 100%-opaque tile above (the only opaque upper layer is `WalkBehind` over-tiles, checked via `TilesetBuilder::overOpaque`; transparent/partial tiles above don't count). `WalkBehind` keeps the real lifted over-tiles; Moving/Object markers stay tiles 0-3.
