...
* **HARD RULE — a ROM converter writes ONLY inside its per-ROM output folder `map/main/<label>/`** (e.g. `datapack-pkmn-convert/map/main/firered/`). NEVER create or modify any file outside it. In particular NEVER write the shared `map/invisible.png` / `map/invisible.tsx`: that is the engine's marker tileset for things that are invisible in the Tiled editor because CatchChallenger adds them at runtime (teleporters, bots, …); it is referenced **read-only**. Every map TILE asset a converter produces — the ROM-extracted pool sheets AND the semantic-layer marker tileset — lives LOCAL under `map/main/<label>/tileset/`. (Bot SKINS are a separate, deliberate mechanism: extracted sprites matched/reused against the shared `skin/bot/` library — that is by design, not a tile asset.)
* **Semantic tile layers (Collisions/Grass/Water/Ledges) each use their OWN distinct, SEMI-TRANSPARENT marker** from the LOCAL `map/main/<label>/tileset/marker.tsx` (NOT the shared `map/invisible.tsx`): Collisions=red, Water=blue, Grass=green, Ledges=yellow/orange/cyan/magenta. Toggling/hiding one layer in Tiled MUST produce a visible change, so: a transparent marker is wrong (invisible); the real `Walkable` tile is wrong (identical to the tile directly below → no change on toggle); a 100%-opaque marker is wrong (it hides the layer below → toggling THAT layer shows nothing). A distinct semi-transparent colour is visible, tells layers apart, AND never hides a layer below it (in-game it is a faint tint — the engine/map2png draw every tile layer). Three guards check it: `CCWriter::layerVisibilityGuard` (no layer fully hidden by a 100%-opaque tile above — only `WalkBehind` over-tiles can, via `TilesetBuilder::overOpaque`; transparent/partial above don't count), the top-layer-cover guard, and `renderVisibilityGuard` (re-renders every `.tmx` with libtiled on all CPU cores: hiding each layer must change the pixels). `WalkBehind` keeps the real lifted over-tiles; Moving/Object markers use the shared `invisible.tsx` tiles 0-3.
