...
* **Do NOT trust the tile-LAYER ORDER in a `.tmx`** — CatchChallenger RE-ORDERS the tile layers at load (it finds them by NAME, e.g. inserts the player just before `WalkBehind`). A map must be correct regardless of layer order; never rely on "layer X is drawn above/below layer Y" from the file order.
