...
* **Do NOT trust the tile-LAYER ORDER or grouping in a `.tmx`** — CatchChallenger finds layers by NAME and may RE-ORDER and/or GROUP them at load, INCONSISTENTLY (sometimes it re-orders, sometimes not; sometimes it groups layers, sometimes not; it inserts the player just before `WalkBehind`). So the on-disk layer order/grouping is non-authoritative and unpredictable. A map MUST be correct regardless of layer order or grouping — never rely on "layer X is drawn above/below Y" from the file, and never depend on layers staying separate or staying together. (This is why feature layers are real-tile DISJOINT: order-independent by construction.)
* **Semantic/feature tile layers use REAL ROM tiles, DISJOINT (no generated markers).** Each cell's real below-player tile goes to EXACTLY ONE layer chosen by its behaviour — `Water`→Water, `Ledge*`→that ledge layer, `Grass`→Grass, else collidable→`Collisions`, else `Walkable` — and its above-player over-tile goes to `WalkBehind`. Because the ground layers never overlap, hiding any single layer in Tiled removes its cells → a visible change (the requirement; see `map/main/test/city.tmx`). `Walkable` is EMPTY at water/grass/ledge/collision cells: the engine still makes them passable — water/grass/lava via the layer's `walkOn` `monstersCollision` (`map/layers.xml`), ledges are one-way (250-253), collisions block (254). Three guards verify it: `layerVisibilityGuard`, the top-layer-cover guard, and `renderVisibilityGuard` (re-renders every `.tmx` with libtiled on all CPU cores; hiding each layer must change the pixels). Moving/Object markers use the shared `invisible.tsx` tiles 0-3.
