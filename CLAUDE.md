...
* **Semantic tile layers (Collisions/Grass/Water/Ledges) re-use the cell's REAL `Walkable` tile** (the official CatchChallenger convention, e.g. `official/altate-islands`). The engine only needs a non-empty gid, but a transparent marker is invisible when the layer is isolated in Tiled — you can't see where the collision/water/grass/ledge is. Re-using the real tile makes each layer visible when shown alone and is a no-op in the game render (the client/map2png draw every tile layer; the same opaque tile drawn on `Walkable` + the semantic layer = identical pixels). Do NOT use a transparent marker or a visible tint/marker. (Objects — bot/teleport/border in the Moving/Object groups — still use `map/invisible.tsx`; `WalkBehind` still holds the lifted over-tiles, transparent where it matches the under tile.)
