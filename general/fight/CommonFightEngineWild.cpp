...
PlayerMonster CommonFightEngine::getRandomMonster(const std::vector<MapMonster> &monsterList,bool *ok)
{
    ...
    Monster monsterDef=CommonDatapack::commonDatapack.get_monster(playerMonster.monster);
                                                     ...
                                                     , const std::vector<uint8_t> &events)
{
    ...
    if(isInFight())
    {
        errorFightEngine("error: map generateWildFightIfCollision is in fight");
        return false;
    }
    ...
    while(index<monstersCollisionValue.walkOn.size())
    {
        ...
        if(monstersCollision.item==0 || items.find(monstersCollision.item)!=items.cend())
        {
            ...
            else
            {
                if(!ableToFight)
                {
                    errorFightEngine("LocalClientHandlerFight::singleMove(), can't walk into the grass into map ("+std::to_string(x)+","+std::to_string(y)+")");
                    return false;
                }
                if(stepFight==0)
                {
                    ...
                    {
                        errorFightEngine("error: no more random seed here, map ("+std::to_string(x)+","+std::to_string(y)+"), is in fight");
                        return false;
                    }
                    else
                ...
                if(stepFight==0)
                {
                    ...
                    else
                    {
                        ...
                        {
                            errorFightEngine("error: no more random seed here to have the get on map ("+std::to_string(x)+","+std::to_string(y)+")");
                            return false;
                        }
                    }
                }
...
uint32_t CommonFightEngine::tryCapture(const uint16_t &item)
{
    ...
    if(internalTryCapture(CommonDatapack::commonDatapack.get_trap(item)))
    {
        #ifdef CATCHCHALLENGER_DEBUG_FIGHT
...
bool CommonFightEngine::internalTryCapture(const Trap &trap)
{
    ...
    const Monster::Stat &wildMonsterStat=getStat(CatchChallenger::CommonDatapack::commonDatapack.get_monster(wildMonsters.front().monster),wildMonsters.front().level);
    ...
    if(wildMonsters.front().buffs.size())
    {
        ...
        while(index<wildMonsters.front().buffs.size())
        {
            ...
            {
                const Buff &buff=CatchChallenger::CommonDatapack::commonDatapack.get_monsterBuff(playerBuff.buff);
    ...
    const uint32_t catch_rate=(uint32_t)CatchChallenger::CommonDatapack::commonDatapack.get_monster(wildMonsters.front().monster).catch_rate;
