...
size_t CommonMap::map_object_size=0;//store in full length to easy multiply by index (16Bits) and have full size pointer
...
const void * CommonMap::indexToMap(const CATCHCHALLENGER_TYPE_MAPID &index)
{
    ...
    if(map_object_size<=sizeof(CommonMap))
    {
...
#endif
    return ((char *)flat_map_list)+index*map_object_size;
}
