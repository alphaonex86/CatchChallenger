...
void Map_loader::loadAllMapsAndLink(std::vector<CommonMap> &flat_map_list,const std::string &datapack_mapPath,std::vector<Map_semi> &semi_loaded_map,std::unordered_map<std::string, CATCHCHALLENGER_TYPE_MAPID> &mapPathToId,std::vector<tinyxml2::XMLDocument*> *xmlDocsToKeep)
{
    ...
    while(index<returnList.size())
    {
        ...
        if(regex_search(fileName,mapFilter) && !regex_search(fileName,mapExclude))
        {
            ...
            if(map_temp.tryLoadMap(datapack_mapPath+fileName,mapFinal,true))
            {
                ...
                // copy BaseMap data from map_to_send to mapFinal (zones, industries, botFights, shops, etc.)
                mapFinal.zones=std::move(map_temp.map_to_send.zones);
                mapFinal.industries=std::move(map_temp.map_to_send.industries);
                mapFinal.botFights=std::move(map_temp.map_to_send.botFights);
                mapFinal.industries_pos=std::move(map_temp.map_to_send.industries_pos);
                mapFinal.shops=std::move(map_temp.map_to_send.shops);
                mapFinal.botsFightTrigger=std::move(map_temp.map_to_send.botsFightTrigger);
                mapFinal.monsterDrops=std::move(map_temp.map_to_send.monsterDrops);
                // convert Map_to_send::items (ItemOnMap_Semi) to BaseMap::items (ItemOnMap)
                {
                    unsigned int item_index=0;
                    while(item_index<map_temp.map_to_send.items.size())
                    {
                        const Map_to_send::ItemOnMap_Semi &item_semi=map_temp.map_to_send.items.at(item_index);
                        ItemOnMap itemOnMap;
                        itemOnMap.item=item_semi.item;
                        itemOnMap.infinite=item_semi.infinite;
                        mapFinal.items[item_semi.point]=itemOnMap;
                        item_index++;
                    }
                }
    ...
    while(index<semi_loaded_map.size())
    {
        ...
        CommonMap &currentTempMap=flat_map_list[indexMap];
        if(currentTempMap.border.bottom.mapIndex!=65535)
        {
            CommonMap &bottomTempMap=flat_map_list[currentTempMap.border.bottom.mapIndex];
        ...
        {
            CommonMap &topTempMap=flat_map_list[currentTempMap.border.top.mapIndex];
        ...
        {
            CommonMap &leftTempMap=flat_map_list[currentTempMap.border.left.mapIndex];
        ...
        {
            CommonMap &rightTempMap=flat_map_list[currentTempMap.border.right.mapIndex];
