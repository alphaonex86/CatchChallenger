...
// ───────────────────────── Flat ROM-image source ─────────────────────────
Mp2kRomSource::Mp2kRomSource(const uint8_t *image, uint32_t imageSize)
    : image_(image), imageSize_(imageSize) {}
uint8_t Mp2kRomSource::u8(uint32_t o) const { return o < imageSize_ ? image_[o] : 0; }
uint16_t Mp2kRomSource::u16(uint32_t o) const { return (uint16_t)u8(o) | ((uint16_t)u8(o + 1) << 8); }
uint32_t Mp2kRomSource::u32(uint32_t o) const
{ return (uint32_t)u8(o) | ((uint32_t)u8(o + 1) << 8) | ((uint32_t)u8(o + 2) << 16) | ((uint32_t)u8(o + 3) << 24); }
bool Mp2kRomSource::isPointer(uint32_t v) const
{ return v >= 0x08000000u && v < 0x0A000000u && (v - 0x08000000u) < imageSize_; }
uint32_t Mp2kRomSource::pointer(uint32_t o, bool *ok) const
{
    uint32_t v = u32(o);
    if(!isPointer(v)) { if(ok) *ok = false; return 0; }
    if(ok) *ok = true;
    return v - 0x08000000u;
}
