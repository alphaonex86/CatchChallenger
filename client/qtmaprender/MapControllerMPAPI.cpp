             ...
             const uint32_t &mapId,const uint16_t &x,const uint16_t &y,const CatchChallenger::Direction &direction,bool inReplayMode)
{
    ...
    if(mapId>=(uint32_t)QtDatapackClientLoader::GetInstance()->get_maps().size())
    {
        ...
        emit error("mapId greater than QtDatapackClientLoader::GetInstance->maps.size(): "+
                   std::to_string(QtDatapackClientLoader::GetInstance()->get_maps().size()));
        return true;
    }
    #ifdef DEBUG_CLIENT_PLAYER_ON_MAP
    qDebug() << QStringLiteral("insert_player(%1->%2,%3,%4,%5,%6)").arg(player.pseudo).arg(player.simplifiedId).arg(QtDatapackClientLoader::GetInstance()->maps.value(mapId)).arg(x).arg(y).arg(CatchChallenger::MoveOnTheMap::directionToString(direction));
    #endif
    //current player
    ...
    else
    {
        ...
        const std::string &mapPath=QFileInfo(QString::fromStdString(datapackMapPathSpec+QtDatapackClientLoader::GetInstance()->get_maps().at(mapId)))
                                            ...
                                            const CatchChallenger::Direction &direction,bool inReplayMode)
{
    ...
    /// \warning search by loop because otherPlayerList.value(id).current_map is the full path, QtDatapackClientLoader::GetInstance->maps relative path
    ...
    if(all_map.find(tempCurrentMap)==all_map.cend())
    {
        qDebug() << "internal problem, revert map (" << QString::fromStdString(otherPlayerList.at(id).current_map)
                 << ") index is wrong (" << QString::fromStdString(stringimplode(QtDatapackClientLoader::GetInstance()->get_maps(),";")) << ")";
        if(!inReplayMode)
        {
            DelayedReinsertSingle tempItem;
            tempItem.id=id;
