...
QtDatapackClientLoader::~QtDatapackClientLoader() {
    if (mDefaultInventoryImage == NULL) delete mDefaultInventoryImage;
#ifndef NOTHREADS
    quit();
    wait();
#endif
...
QtDatapackClientLoader *QtDatapackClientLoader::GetInstance() {
    if (datapackLoader == nullptr) {
        datapackLoader = new QtDatapackClientLoader();
    }
...
void QtDatapackClientLoader::parseDatapack(const std::string &datapackPath)
{
    ...
    std::string hash;
    if (Settings::settings->contains("DatapackHashBase-" +
    ...
    if (!l.empty()) {
        ...
        if (stringEndsWith(b, "/")) b = b.substr(0, b.size() - 1);
        b += "-cache/";
        cachepathdir = b;
    ...
    DatapackClientLoader::text_DATAPACK_BASE_PATH_MAPMAIN =
        DATAPACK_BASE_PATH_MAPMAIN "na/";
    DatapackClientLoader::text_DATAPACK_BASE_PATH_MAPSUB =
        std::string(DATAPACK_BASE_PATH_MAPSUB1) + "na/" +
        std::string(DATAPACK_BASE_PATH_MAPSUB2) + "nabis/";
    ...
    if (!loaded) {
        ...
        DatapackClientLoader::parseDatapack(
            datapackPath, hash, Language::language.getLanguage().toStdString());
        parseMonstersExtra();
        parseBuffExtra();
        ...
        if (!cachepath.empty()) {
            ...
            if (out_file.good() && out_file.is_open()) {
                ...
                out_file.flush();
                out_file.close();
                if (::rename((cachepath + ".tmp").c_str(), cachepath.c_str()) !=
                    0)
                    std::cerr
                        << "unable to rename from: " << (cachepath + ".tmp")
                        << " to: " << cachepath << std::endl;
            }
        }
        auto end_time = std::chrono::high_resolution_clock::now();
        auto time = end_time - start_time;
    ...
    for (const auto &n :
        ...
        const std::string basePath =
            datapackPath + DATAPACK_BASE_PATH_BUFFICON + std::to_string(id);
        const std::string pngFile = basePath + ".png";
        const std::string gifFile = basePath + ".gif";
        if (QFile(QString::fromStdString(pngFile)).exists())
            QtmonsterBuffExtraEntry.icon =
                QPixmap(QString::fromStdString(pngFile));
        else if (QFile(QString::fromStdString(gifFile)).exists())
            QtmonsterBuffExtraEntry.icon =
                QPixmap(QString::fromStdString(gifFile));
        if (QtmonsterBuffExtraEntry.icon.isNull())
            QtmonsterBuffExtraEntry.icon =
                QPixmap(QStringLiteral(":/CC/images/interface/buff.png"));
        QtDatapackClientLoader::datapackLoader->QtmonsterBuffsExtra[id] =
            QtmonsterBuffExtraEntry;
    }

    for (const auto &n : itemsExtra) ImageitemsToLoad.push_back(n.first);
    for (const auto &n : monsterExtra) ImagemonsterToLoad.push_back(n.first);
    auto end_time = std::chrono::high_resolution_clock::now();
    auto time = end_time - start_time;
    std::cout << "CatchChallenger::CommonDatapack::commonDatapack."
                 "parseDatapack end took "
              << time / std::chrono::milliseconds(1) << "ms to parse "
              << datapackPath << std::endl;
    startThread();
    inProgress = false;
...
void QtDatapackClientLoader::startThread() {
    ...
    while (index < tc || index == 0)  // always create 1 thread
    {
        ...
        connect(t, &QThread::finished, this,
                &QtDatapackClientLoader::threadFinished, Qt::QueuedConnection);
        connect(t, &QtDatapackClientLoaderThread::loadItemImage, this,
                &QtDatapackClientLoader::loadItemImage, Qt::QueuedConnection);
        connect(t, &QtDatapackClientLoaderThread::loadMonsterImage, this,
                &QtDatapackClientLoader::loadMonsterImage,
                Qt::QueuedConnection);
...
void QtDatapackClientLoader::threadFinished() {
    QtDatapackClientLoaderThread *thread =
        qobject_cast<QtDatapackClientLoaderThread *>(sender());
    if (thread == nullptr) abort();
    if (threads.find(thread) != threads.cend())
        threads.erase(thread);
    else {
        std::cerr << "GameLoader::threadFinished() thread not found"
                  << std::endl;
        abort();
        return;
    }
...
void QtDatapackClientLoader::parseDatapackMainSub(
    const std::string &mainDatapackCode, const std::string &subDatapackCode) {
    auto start_time = std::chrono::high_resolution_clock::now();
    std::string hashMain;

    std::string datapackPathMain =
        datapackPath + DATAPACK_BASE_PATH_MAPMAIN + mainDatapackCode;
    std::string datapackPathSub =
        datapackPath + std::string(DATAPACK_BASE_PATH_MAPSUB1) +
        mainDatapackCode + std::string(DATAPACK_BASE_PATH_MAPSUB2) +
        subDatapackCode;

    if (Settings::settings->contains(
            "DatapackHashMain-" + QString::fromStdString(datapackPathMain))) {
        const QByteArray &data = QByteArray::fromHex(
            Settings::settings
                ->value("DatapackHashMain-" +
                        QString::fromStdString(datapackPathMain))
                .toString()
                .toUtf8());
        hashMain = std::string(data.constData(), data.size());
    }
    if (hashMain.size() != 28) {
        Settings::settings->remove("DatapackHashMain-" +
                                   QString::fromStdString(datapackPathMain));
        hashMain.empty();
    }
    std::string hashSub;
    if (Settings::settings->contains("DatapackHashSub-" +
                                     QString::fromStdString(datapackPathSub))) {
        const QByteArray &data = QByteArray::fromHex(
            Settings::settings
    ...
    DatapackClientLoader::parseDatapackMainSub(
        mainDatapackCode, subDatapackCode, hashMain, hashSub);
    if (mDefaultInventoryImage == NULL)
        mDefaultInventoryImage = new QPixmap(
            QStringLiteral(":/CC/images/inventory/unknown-object.png"));
    CatchChallenger::CommonDatapackServerSpec::commonDatapackServerSpec
        .parseDatapack(datapackPath, mainDatapackCode, subDatapackCode);
    auto end_time = std::chrono::high_resolution_clock::now();
    auto time = end_time - start_time;
    std::cout << "parseDatapackMainSub took "
              << time / std::chrono::milliseconds(1) << "ms to parse "
              << datapackPath << "(" << mainDatapackCode << ","
              << subDatapackCode << ")" << std::endl;
    parseTileset();
...
void QtDatapackClientLoader::emitdatapackParsed() { emit datapackParsed(); }
...
void QtDatapackClientLoader::parseTopLib() { parseTileset(); }
...
void QtDatapackClientLoader::resetAll() {
    ...
    for (const auto &n : QtplantExtra) delete n.second.tileset;
    QtplantExtra.clear();
    {
        QHashIterator<QString, Tiled::Tileset *> i(
            Tiled::Tileset::preloadedTileset);
        while (i.hasNext()) {
            i.next();
            delete i.value();
        }
...
void QtDatapackClientLoader::parseTileset() {
    const std::vector<std::string> &fileList =
        CatchChallenger::FacilityLibGeneral::listFolder(
            datapackPath + DATAPACK_BASE_PATH_MAPBASE);
    unsigned int index = 0;
    while (index < fileList.size()) {
        ...
        if (stringEndsWith(filePath, ".tsx")) {
            ...
            if (file.open(QIODevice::ReadOnly)) {
                ...
                Tiled::Tileset *tileset = mapReader.readTileset(
                    &file, QString::fromStdString(source));
                if (tileset) {
                    tileset->setFileName(QString::fromStdString(source));
                    Tiled::Tileset::preloadedTileset[QString::fromStdString(
                        source)] = tileset;
                }
                file.close();
            } else
    ...
    qDebug() << QStringLiteral("%1 tileset(s) loaded")
                    .arg(Tiled::Tileset::preloadedTileset.size());
}
...
std::string QtDatapackClientLoader::getSkinPath(const std::string &skinName,
                                                const std::string &type) const {
    {
        QFileInfo pngFile(
            QString::fromStdString(getDatapackPath()) +
            DATAPACK_BASE_PATH_SKIN + QString::fromStdString(skinName) +
            QStringLiteral("/%1.png").arg(QString::fromStdString(type)));
        if (pngFile.exists()) return pngFile.absoluteFilePath().toStdString();
    }
    {
        QFileInfo gifFile(
            QString::fromStdString(getDatapackPath()) +
        ...
        if (gifFile.exists()) return gifFile.absoluteFilePath().toStdString();
    }
    QDir folderList(QString::fromStdString(getDatapackPath()) +
                    DATAPACK_BASE_PATH_SKINBASE);
    const QStringList &entryList =
        folderList.entryList(QDir::Dirs | QDir::NoDotAndDotDot);
    int entryListIndex = 0;
    while (entryListIndex < entryList.size()) {
        {
            QFileInfo pngFile(
                QStringLiteral("%1/skin/%2/%3/%4.png")
        ...
        {
            QFileInfo gifFile(
                QStringLiteral("%1/skin/%2/%3/%4.gif")
                    .arg(QString::fromStdString(getDatapackPath()))
                    .arg(entryList.at(entryListIndex))
                    .arg(QString::fromStdString(skinName))
                    .arg(QString::fromStdString(type)));
...
std::string QtDatapackClientLoader::getFrontSkinPath(
    const uint32_t &skinId) const {
    /// \todo merge it cache string + id
    const std::vector<std::string> &skinFolderList =
        QtDatapackClientLoader::datapackLoader->skins;
    // front image
    if (skinId < (uint32_t)skinFolderList.size())
        return getSkinPath(skinFolderList.at(skinId), "front");
    else
        qDebug() << "The skin id: " + QString::number(skinId) +
                        ", into a list of: " +
                        QString::number(skinFolderList.size()) +
                        " item(s) into BaseWindow::updatePlayerImage()";
...
void QtDatapackClientLoader::parseMonstersExtra() {
    ...
    while (file_index < fileList.size()) {
        ...
        const std::string &file =
            fileList.at(file_index).absoluteFilePath().toStdString();
        ...
        if (CatchChallenger::CommonDatapack::commonDatapack.get_xmlLoadedFile()
                .find(file) !=
            CatchChallenger::CommonDatapack::commonDatapack.get_xmlLoadedFile()
                .cend())
            domDocument = &CatchChallenger::CommonDatapack::commonDatapack
                               .get_xmlLoadedFile_rw()
                               .at(file);
        else {
            domDocument = &CatchChallenger::CommonDatapack::commonDatapack
                               .get_xmlLoadedFile_rw()[file];
            const auto loadOkay = domDocument->LoadFile(file.c_str());
            if (loadOkay != 0) {
                std::cerr << file << ", " << tinyxml2errordoc(domDocument)
                          << std::endl;
                file_index++;
                continue;
            }
        ...
        if (root == NULL || root->Name() == NULL ||
            strcmp(root->Name(), "monsters") != 0) {
            // qDebug() << (QStringLiteral("Unable to open the xml file: %1,
            // \"monsters\" root balise not found for the xml
            // file").arg(QString::fromStdString(file)));
            file_index++;
            continue;
        }
        ...
        while (item != NULL) {
            if (item->Attribute("id") != NULL) {
                const uint32_t tempid =
                    stringtouint32(item->Attribute("id"), &ok);
                if (!ok)
                    qDebug()
                        << (QStringLiteral("Unable to open the xml file: %1, "
                                           "id not a number: child->Name(): %2")
                    ...
                    if (CatchChallenger::CommonDatapack::commonDatapack
                        ...
                        qDebug() << (QStringLiteral(
                                         "Unable to open the xml file: %1, id "
                                         "not into monster list into monster "
                                         "extra: child->Name(): %2")
                                         .arg(QString::fromStdString(file))
                                         .arg(item->Name()));
                    else {
                        ...
                        qDebug() << (QStringLiteral("monster extra loading: %1")
                                         .arg(id));
#endif
                        {
                            bool found = false;
                            const tinyxml2::XMLElement *name =
                                item->FirstChildElement("name");
                            if (!language.empty() && language != "en")
                                while (name != NULL) {
                                    if (name->Attribute("lang") != NULL &&
                                        name->Attribute("lang") == language &&
                                        name->GetText() != NULL) {
                                        monsterExtraEntry.name =
                                            name->GetText();
                                        found = true;
                                        break;
                                    }
                        ...
                        }
                        {
                            ...
                            if (!language.empty() && language != "en")
                                while (description != NULL) {
                                    if (description->Attribute("lang") !=
                                            NULL &&
                                        description->Attribute("lang") ==
                                            language &&
                                        description->GetText() != NULL) {
                                        monsterExtraEntry.description =
                                            description->GetText();
                                        found = true;
                                        break;
                                    }
                                    description =
                                        description->NextSiblingElement(
                                            "description");
                                }
                            if (!found) {
                                description =
                                ...
                                while (description != NULL) {
                                    if (description->Attribute("lang") ==
                                            NULL ||
                                        strcmp(description->Attribute("lang"),
                                               "en") == 0)
                        ...
                        // load the kind
                        {
                            ...
                            const tinyxml2::XMLElement *kind =
                                item->FirstChildElement("kind");
                            if (!language.empty() && language != "en")
                                while (kind != NULL) {
                                    if (kind->Attribute("lang") != NULL &&
                                        kind->Attribute("lang") == language &&
                                        kind->GetText() != NULL) {
                                        monsterExtraEntry.kind =
                                            kind->GetText();
                                        kind_found = true;
                                        break;
                                    }
                        ...
                        // load the habitat
                        {
                            ...
                            const tinyxml2::XMLElement *habitat =
                                item->FirstChildElement("habitat");
                            if (!language.empty() && language != "en")
                                while (habitat != NULL) {
                                    if (habitat->Attribute("lang") &&
                                        habitat->Attribute("lang") ==
                                            language &&
                                        habitat->GetText() != NULL) {
                                        monsterExtraEntry.habitat =
                                            habitat->GetText();
                                        habitat_found = true;
                                        break;
                                    }
                                    habitat =
                                        habitat->NextSiblingElement("habitat");
                                }
                            if (!habitat_found) {
                                habitat = item->FirstChildElement("habitat");
                                while (habitat != NULL) {
                                    if (habitat->Attribute("lang") == NULL ||
                                        strcmp(habitat->Attribute("lang"),
                                               "en") == 0)
                        ...
                        QtDatapackClientLoader::datapackLoader
                            ->monsterExtra[id] = monsterExtraEntry;
                        auto end_time2 =
                            std::chrono::high_resolution_clock::now();
                ...
                qDebug()
                    << (QStringLiteral(
                            "Unable to open the xml file: %1, have not the "
                            "monster id at monster extra: child->Name(): %2")
                            .arg(QString::fromStdString(file))
                            .arg(item->Name()));
    ...
    auto i =
        CatchChallenger::CommonDatapack::commonDatapack.get_monsters().begin();
    while (
        i !=
        ...
        if (QtDatapackClientLoader::datapackLoader->monsterExtra.find(
            ...
            qDebug() << (QStringLiteral(
                             "Strange, have entry into monster list, but not "
                             "into monster extra for id: %1")
                             .arg(i->first));
            QtDatapackClientLoader::MonsterExtra monsterExtraEntry;
            monsterExtraEntry.name = tr("Unknown").toStdString();
            monsterExtraEntry.description = tr("Unknown").toStdString();
            QtDatapackClientLoader::datapackLoader->monsterExtra[i->first] =
                monsterExtraEntry;
        }
        ++i;
    }
    ...
    qDebug()
        << QStringLiteral("%1 monster(s) extra loaded")
               .arg(QtDatapackClientLoader::datapackLoader->monsterExtra.size())
        << "into" << time / std::chrono::milliseconds(1) - imagens / 1000000
...
void QtDatapackClientLoader::parseBuffExtra() {
    ...
    while (file_index < fileList.size()) {
        ...
        const tinyxml2::XMLElement *root = domDocument->RootElement();
        if (root == NULL || root->Name() == NULL ||
            strcmp(root->Name(), "buffs") != 0) {
            qDebug() << (QStringLiteral(
                             "Unable to open the xml file: %1, \"buffs\" root "
                             "balise not found for the xml file")
                             .arg(QString::fromStdString(file)));
            file_index++;
            continue;
        }
        ...
        while (item != NULL) {
            if (item->Attribute("id") != NULL) {
                const uint32_t tempid =
                    stringtouint8(item->Attribute("id"), &ok);
                if (!ok || tempid > 255)
                    qDebug()
                        << (QStringLiteral("Unable to open the xml file: %1, "
                                           "id not a number: child->Name(): %2")
                    ...
                    if (CatchChallenger::CommonDatapack::commonDatapack
                        ...
                        qDebug() << (QStringLiteral(
                                         "Unable to open the xml file: %1, id "
                                         "not into monster buff list into buff "
                                         "extra: child->Name(): %2")
                                         .arg(QString::fromStdString(file))
                                         .arg(item->Name()));
                    else {
                        QtDatapackClientLoader::MonsterExtra::Buff
                            monsterBuffExtraEntry;
#ifdef DEBUG_MESSAGE_MONSTER_LOAD
                        qDebug() << (QStringLiteral("monster extra loading: %1")
                                         .arg(id));
#endif
                            ...
                            while (description != NULL) {
                                if (description->Attribute("lang") &&
                                    description->Attribute("lang") ==
                                        language &&
                                    description->GetText() != NULL) {
                                    monsterBuffExtraEntry.description =
                                        description->GetText();
                                    found = true;
                                    break;
                                }
                        ...
                        if (monsterBuffExtraEntry.description.empty())
                            monsterBuffExtraEntry.description =
                                tr("Unknown").toStdString();
                        QtDatapackClientLoader::datapackLoader
                            ->monsterBuffsExtra[id] = monsterBuffExtraEntry;
                    }
                }
            } else
                qDebug() << (QStringLiteral(
                                 "Unable to open the xml file: %1, have not "
                                 "the buff id: child->Name(): %2")
                                 .arg(QString::fromStdString(file))
                                 .arg(item->Name()));
    ...
    auto i = CatchChallenger::CommonDatapack::commonDatapack.get_monsterBuffs()
                 .begin();
    while (i !=
           CatchChallenger::CommonDatapack::commonDatapack.get_monsterBuffs()
               .cend()) {
        if (QtDatapackClientLoader::datapackLoader->monsterBuffsExtra.find(
            ...
            qDebug() << (QStringLiteral(
                             "Strange, have entry into monster list, but not "
                             "into monster buffer extra for id: %1")
                             .arg(i->first));
            ...
            QtmonsterBuffExtraEntry.icon =
                QPixmap(":/CC/images/interface/buff.png");
            QtDatapackClientLoader::datapackLoader
                ->monsterBuffsExtra[i->first] = monsterBuffExtraEntry;
            QtDatapackClientLoader::datapackLoader
                ->QtmonsterBuffsExtra[i->first] = QtmonsterBuffExtraEntry;
        }
        ++i;
    }

    qDebug() << QStringLiteral("%1 buff(s) extra loaded")
                    .arg(QtDatapackClientLoader::datapackLoader
                             ->monsterBuffsExtra.size());
}
...
void QtDatapackClientLoader::parsePlantsExtra() {
    ...
    auto i =
        CatchChallenger::CommonDatapack::commonDatapack.get_plants().begin();
    while (
        i !=
        ...
        plant.tileset =
            new Tiled::Tileset(QString::fromStdString(text_plant), 16, 32);
        const std::string &path =
            basePath + std::to_string(i->first) + text_dotpng;
        if (!plant.tileset->loadFromImage(QImage(QString::fromStdString(path)),
                                          QString::fromStdString(path))) {
            const std::string &path =
                basePath + std::to_string(i->first) + text_dotgif;
            if (!plant.tileset->loadFromImage(
                    QImage(QString::fromStdString(path)),
                    QString::fromStdString(path)))
                if (!plant.tileset->loadFromImage(
                        QImage(QStringLiteral(
                            ":/CC/images/plant/unknow_plant.png")),
                        QStringLiteral(":/CC/images/plant/unknow_plant.png")))
                    qDebug() << "Unable the load the default plant tileset";
        }
        QtDatapackClientLoader::datapackLoader->QtplantExtra[i->first] = plant;
        itemToPlants[CatchChallenger::CommonDatapack::commonDatapack
                         .get_plants()
                         .at(i->first)
                         .itemUsed] = i->first;
        ++i;
    ...
    qDebug() << QStringLiteral("%1 plant(s) extra loaded")
                    .arg(QtDatapackClientLoader::datapackLoader->QtplantExtra
                             .size());
}

const QtDatapackClientLoader::QtMonsterExtra &
QtDatapackClientLoader::getMonsterExtra(const uint16_t &id) {
    ...
    const std::string basepath =
        ...
        "/monsters/";
    n.front = QPixmap(
        QString::fromStdString(basepath + std::to_string(id) + "/front.png"));
    if (n.front.isNull()) {
        n.front = QPixmap(QString::fromStdString(basepath + std::to_string(id) +
                                                 "/front.gif"));
        if (n.front.isNull())
            n.front = QPixmap(":/CC/images/monsters/default/front.png");
    }
    n.back = QPixmap(
        QString::fromStdString(basepath + std::to_string(id) + "/back.png"));
    if (n.back.isNull()) {
        n.back = QPixmap(QString::fromStdString(basepath + std::to_string(id) +
                                                "/back.gif"));
        if (n.back.isNull())
            n.back = QPixmap(":/CC/images/monsters/default/back.png");
    }
    n.thumb = QPixmap(
        QString::fromStdString(basepath + std::to_string(id) + "/small.png"));
    if (n.thumb.isNull()) {
        n.thumb = QPixmap(QString::fromStdString(basepath + std::to_string(id) +
                                                 "/small.gif"));
        if (n.thumb.isNull())
...
const QtDatapackClientLoader::QtItemExtra &QtDatapackClientLoader::getItemExtra(
    const uint16_t &id) {
    ...
    const DatapackClientLoader::ItemExtra &extra =
...
const QtDatapackClientLoader::QtBuffExtra &
QtDatapackClientLoader::getMonsterBuffExtra(const uint8_t &id) {
    if (QtmonsterBuffsExtra.find(id) != QtmonsterBuffsExtra.cend())
        ...
        if (monsterBuffsExtra.find(id) != monsterBuffsExtra.cend()) {
            ...
            QtBuffExtra n;
            n.icon = QPixmap(QString::fromStdString(getDatapackPath()) +
                             "/monsters/buff/" + QString::number(id) + ".png");
...
const QtDatapackClientLoader::QtPlantExtra &
QtDatapackClientLoader::getPlantExtra(const uint8_t &id) {
    ...
    n.tileset->loadFromImage(
        QImage(QStringLiteral(":/CC/images/plant/unknow_plant.png")),
        QStringLiteral(":/CC/images/plant/unknow_plant.png"));
    QtplantExtra[id] = n;
    return QtplantExtra.at(id);
}
