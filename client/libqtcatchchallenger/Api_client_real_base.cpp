...
void Api_client_real::writeNewFileBase(const std::string &fileName,const std::string &data)
{
    ...
    {
        cacheRemovedBase=true;
        const QString &path=QString::fromStdString(mDatapackBaseCache);
        QDir dir(path);
...
void Api_client_real::datapackChecksumDoneBase(const std::vector<std::string> &datapackFilesList,const std::vector<char> &hash,const std::vector<uint32_t> &partialHashList)
{
    ...
    if(datapackFilesListBase.empty())
        std::cerr << "WARNING Api_client_real::datapackChecksumDoneBase(): local Base datapack file list is empty" << std::endl;
...
void Api_client_real::test_mirror_base()
{
    ...
    if(reply->error()==QNetworkReply::NoError)
    {
        if(!connect(reply, &QNetworkReply::errorOccurred, this, &Api_client_real::httpErrorEventBase))
            abort();
        if(!connect(reply, &QNetworkReply::downloadProgress, this, &Api_client_real::downloadProgressDatapackBase))
            abort();
    }
    ...
    else
    {
        ...
        else
        {
            ...
            std::vector<char> partialHashListRaw(data.cbegin()+endOfText+3,data.cend());
            {
                ...
                {
                    std::cerr << "partialHashList not divisible by 4 (base), check: "+reply->url().toString().toStdString() << std::endl;
                    newError(tr("Wrong datapack format").toStdString(),"partialHashList not divisible by 4 (base), check: "+reply->url().toString().toStdString());
                    return;
                }
                {
...
void Api_client_real::httpErrorEventBase(QNetworkReply::NetworkError code)
{
    QNetworkReply *reply = qobject_cast<QNetworkReply *>(sender());
    if(reply==NULL)
    {
