...
bool gameData::mapSupported(Map* map)
{
    if(mRenderer == 0)
        setMapRenderer(map);
...
Player_private_and_public_informations gameData::getPlayerInfo()
{
    if(mClient == 0)
        return mClient->get_player_informations();
    Player_private_and_public_informations info;
    return info;
}
...
void gameData::setClient(Pokecraft_client *client)
{
	mClient = client;
        mDir = QDir(QCoreApplication::applicationDirPath());
...
void gameData::setMapRenderer(Map *map)
{
    if(map->orientation()==Map::Orthogonal&&mRenderer==0)
    {
...
void gameData::have_player_information(QList<Player_public_informations> information)
{
    foreach(Player_public_informations info,information)
        foreach(player* play, getPlayer())
