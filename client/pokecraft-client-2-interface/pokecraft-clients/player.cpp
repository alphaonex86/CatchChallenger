...
player::player(QString path, quint32 id,QString mapName)
{
        moveClientEmiter=NULL;
        timer = new QTimer();
...
player::~player()
{
	if(moveClientEmiter!=NULL)
	{
		delete moveClientEmiter;
		moveClientEmiter=NULL;
	}
    if(sprite!=0)
    {
...
/// \brief ...
void player::get_player_move(quint8 movement, Direction direction)
{
    gameData *data = gameData::instance();
...
{
    if(moveClientEmiter==0)
        return;
    if(!move)
    {
        moveClientEmiter->newDirection(Direction(the_new_direction));
    }
    else
    {
        moveClientEmiter->newDirection(Direction(the_new_direction+4));
        gameData* data = gameData::instance();
...
void player::setInfo(Player_public_informations info)
{
    this->data = info;
}

/// \brief load the info of the player
void player::loadInfo(Player_public_informations info)
{
    this->setId( info.id );
...
bool player::mainPlayer()
{
    ...
    if(this == data->getThisPlayer()
            ||data->getPlayerInfo().public_informations.id == getId())
    {
        gameData::destroyInstanceAtTheLastCall();
        return true;
    }
