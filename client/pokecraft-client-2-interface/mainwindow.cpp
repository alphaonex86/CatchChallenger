...
void MainWindow::have_current_player_info()
{
	...
	ui->label_connecting_status->setText(tr("Loading the datapack..."));
        ui->player_informations_id->setText(tr("N°ID/%1").arg(informations.id));
...
void MainWindow::new_chat_text(quint32 player_id,quint8 chat_type,QString text)
{
	...
	chat_list_type << (Chat_type)chat_type;
	chat_list_text << text;
...
void MainWindow::update_chat()
{
	...
	while(index<chat_list_player_id.size())
	{
		...
		if(!addPlayerInfo)
			nameHtml+=Pokecraft_client_chat::new_chat_message("",Player_type_normal,chat_list_type.at(index),chat_list_text.at(index));
		else if(index_player!=-1)
                        nameHtml+=Pokecraft_client_chat::new_chat_message(player_informations_list.at(index_player).pseudo,player_informations_list.at(index_player).type,chat_list_type.at(index),chat_list_text.at(index));
		index++;
	}
