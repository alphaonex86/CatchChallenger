...
public:
    ...
    QHash<uint8_t,TypeExtra> typeExtra;
    QHash<uint16_t,MonsterExtra> monsterExtra;
    QHash<uint8_t,MonsterExtra::Buff> monsterBuffsExtra;
    QHash<uint16_t,MonsterExtra::Skill> monsterSkillsExtra;
    QHash<uint16_t,ItemExtra> itemsExtra;
    QHash<QString,ReputationExtra> reputationExtra;
    QHash<QString,uint8_t> reputationNameToId;//Player_private_and_public_informations, std::unordered_map<uint8_t,PlayerReputation> reputation;
    QHash<uint16_t,uint8_t> itemToPlants;
    QHash<uint8_t,PlantExtra> plantExtra;
    QHash<uint16_t,QuestExtra> questsExtra;
    QHash<uint16_t,QuestText> questsText;
    QHash<QString,uint16_t> questsPathToId;
    QMultiHash<uint16_t,uint16_t> botToQuestStart;
    QHash<uint16_t,BotFightExtra> botFightsExtra;
