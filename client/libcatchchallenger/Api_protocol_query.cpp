...
bool Api_protocol::parseQuery(const uint8_t &packetCode, const uint8_t &queryNumber, const char * const data, const unsigned int &size)
{
    ...
    if(!is_logged)
    {
        parseError("Protocol wrong or corrupted","is not logged with main ident: "+std::to_string(packetCode)+", file: "+std::string(__FILE__)+":"+std::to_string(__LINE__));
        return false;
    }
    switch(packetCode)
    {
        ...
        case 0xE0:
        {
            ...
            {
                parseError("Protocol wrong or corrupted","Already on trade");
                return false;
            }
            if(!battleRequestId.empty() || isInBattle)
            {
                parseError("Protocol wrong or corrupted","Already on battle");
                return false;
            }
            ...
            if(pseudo.empty())
            {
                parseError("Protocol wrong or corrupted","utf8 enconding failed wrong size with main ident: "+std::to_string(packetCode)+
                           ", data: "+binarytoHexa(data+pos,size)+", line: "+std::string(__FILE__)+":"+std::to_string(__LINE__)
                              );
                return false;
            }
        ...
        break;
        default:
            parseError("Protocol wrong or corrupted","unknown ident main code: %1, line: "+std::string(__FILE__)+":"+std::to_string(__LINE__));
            return false;
        break;
    }
    if((size-pos)!=0)
    {
        parseError("Protocol wrong or corrupted","remaining data: parseFullQuery("+std::to_string(packetCode)+
