...
void MonsterDetails::paint(QPainter *p, const QStyleOptionGraphicsItem *, QWidget *widget)
{
    ...
    p->drawPixmap(xLeft,yTop,min,min,QPixmap(":/CC/images/interface/backm.png"));
    if(xLeft>0)
        p->drawPixmap(0,0,xLeft,widget->height(),QPixmap(":/CC/images/interface/backm.png"),0,0,1,1);
    if(xRight>0)
        p->drawPixmap(widget->width()-xRight,0,xRight,widget->height(),QPixmap(":/CC/images/interface/backm.png"),0,0,1,1);
    if(yTop>0)
        p->drawPixmap(0,0,widget->width(),yTop,QPixmap(":/CC/images/interface/backm.png"),0,0,1,1);
    if(yBottom>0)
...
void MonsterDetails::setVar(const CatchChallenger::PlayerMonster &monster)
{
    ...
    const QtDatapackClientLoader::MonsterExtra &monsterExtraInfo=QtDatapackClientLoader::datapackLoader->get_monsterExtra(monster.monster);
    ...
    }
    {
        ...
        {
            catchPixmap=QPixmap(QString::fromStdString(QtDatapackClientLoader::datapackLoader->get_itemExtra(monster.catched_with).imagePath));
    ...
    monsterDetailsSkills->clear();
    {
        ...
        while(indexSkill<monster.skills.size())
        {
            ...
            else
            {
                ...
                const CatchChallenger::Skill &skill=CatchChallenger::CommonDatapack::commonDatapack.get_monsterSkill(monsterSkill.skill);
