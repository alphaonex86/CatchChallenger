...
void Quests::setVar(ConnexionManager *connexionManager)
{
    ...
    while(i!=playerInformations.quests.cend())
    {
        ...
        if(QtDatapackClientLoader::datapackLoader->questsExtra.find(questId)!=QtDatapackClientLoader::datapackLoader->questsExtra.cend())
        {
            if(i->second.step>0)
            {
                QListWidgetItem * item=new QListWidgetItem(QString::fromStdString(QtDatapackClientLoader::datapackLoader->questsExtra.at(questId).name));
...
void Quests::on_questsList_itemSelectionChanged()
{
    ...
    {
        std::vector<CatchChallenger::Quest::Item> items=CatchChallenger::CommonDatapackServerSpec::commonDatapackServerSpec.quests.at(questId)
        ...
        while(index<items.size())
        {
            ...
            {
                image=QtDatapackClientLoader::datapackLoader->getImageExtra(items.at(index).item).image;
    ...
    if(questExtra.showRewards || Ultimate::ultimate.isUltimate())
    {
        ...
        {
            std::vector<CatchChallenger::Quest::Item> items=CatchChallenger::CommonDatapackServerSpec::commonDatapackServerSpec.quests.at(questId).rewards.items;
            std::vector<std::string> objects;
            unsigned int index=0;
            while(index<items.size())
            {
        ...
        {
            std::vector<CatchChallenger::ReputationRewards> reputationRewards=CatchChallenger::CommonDatapackServerSpec::commonDatapackServerSpec.quests.at(questId).rewards.reputation;
        ...
        {
            std::vector<CatchChallenger::ActionAllow> allowRewards=CatchChallenger::CommonDatapackServerSpec::commonDatapackServerSpec.quests.at(questId).rewards.allow;
