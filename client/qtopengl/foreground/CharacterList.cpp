...
void CharacterList::add_finished()
{
    ...
    if(profileIndex>=CatchChallenger::CommonDatapack::commonDatapack.get_profileList().size())
        return;
    CatchChallenger::Profile profile=CatchChallenger::CommonDatapack::commonDatapack.get_profileList().at(profileIndex);
    if(newGame==nullptr)
    {
        newGame=new NewGame();
    ...
    if(charaterCount<CommonSettingsCommon::commonSettingsCommon.min_character && charaterCount<CommonSettingsCommon::commonSettingsCommon.max_character)
    {
        ...
        if(CliClientOptions::autosolo || !CliClientOptions::characterName.isEmpty())
        {
            ...
            if(!profiles.empty())
            {
                ...
                //deterministic for testingmap4client.py: pick the monstergroup
                //whose lowest monster id is smallest, and the skin with the
                //smallest id, so the same datapack always yields the same
                //pseudo/skin/follower on map.
                uint8_t monsterGroupId=0;
                {
                    ...
                    {
                        const std::vector<CatchChallenger::Profile::Monster> &grp=profile.monstergroup.at(gi);
