...
uint32_t AssetsLoaderThread::audio = 0;
uint32_t AssetsLoaderThread::image = 0;
  ...
  while (index < toLoad.size()) {
    ...
    if (file.endsWith(QStringLiteral(".opus"))) {
      ...
      for (;;) {
        ...
        for (si = 0; si < 2 * ret; si++) {  /// Ensure the data is little-endian
      ...
      if (ret == EXIT_SUCCESS) {
        // fprintf(stderr,"\nDone: played ");
        musics[file] =
            new QByteArray(buffer.data().data(), buffer.data().size());
      }
      op_free(of);
      uint64_t s = QFileInfo(file).size();
      if (s > pos) emit AddSize(s - pos);
