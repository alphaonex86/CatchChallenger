...
*/
  ...
  if (Settings::settings->contains("audioVolume")) {
    audioVolume = static_cast<uint8_t>(
        Settings::settings->value("audioVolume").toUInt(&ok));
    if (!ok || audioVolume > 100) audioVolume = 100;
  } else
    audioVolume = 100;
...
void Options::setFPS(const uint16_t &fps) {
  ...
  emit newFPS(fps);
  if (limitedFPS) emit newFinalFPS(fps);
}

void Options::setLimitedFPS(const bool &limitedFPS) {
  if (this->limitedFPS == limitedFPS) return;
...
void Options::setDeviceIndex(const int &indexDevice) {
  if (indexDevice < (int)devices.size() && indexDevice >= 0) {
    if (this->indexDevice == indexDevice) return;
...
void Options::setAudioDeviceList(const QStringList &devices) {
  indexDevice =
      devices.indexOf(Settings::settings->value("audioDevice").toString());
  if (indexDevice == -1) {
    if (!devices.isEmpty()) indexDevice = 0;
    Settings::settings->remove("audioDevice");
  }
  this->devices = devices;
}

int Options::getIndexDevice() const { return indexDevice; }

uint16_t Options::getFPS() const { return fps; }

bool Options::getLimitedFPS() const { return limitedFPS; }
...
uint8_t Options::getAudioVolume() const { return audioVolume; }
