...
#ifndef CLIENT_V3_ENTITIES_MAP_CLIENT_HPP_
#define CLIENT_V3_ENTITIES_MAP_CLIENT_HPP_
...
class Map_client : public CommonMap {
 public:
  struct ItemOnMapForClient {
    ...
    bool infinite;
    uint16_t
        indexOfItemOnMap;  // to see if the player have get it,
                           // DatapackClientLoader::std::unordered_map<QString,std::unordered_map<QPair<uint8_t,uint8_t>,uint16_t>
                           // > itemOnMap;
  };
  std::vector<CommonMap *>
      near_map;  // only the border (left, right, top, bottom) AND them self
  std::vector<CommonMap *>
      linked_map;  // not only the border, with tp, door, ...
  ...
  std::unordered_map<std::pair<uint8_t, uint8_t>, BotDisplay, pairhash>
      botsDisplay;

  std::unordered_map<std::pair<uint8_t, uint8_t>,
                     std::vector<std::pair<uint8_t, uint8_t> >, pairhash>
      botsFightTriggerExtra;  // trigger line in front of bot fight, tigger x,y,
                              // bot x,y
  std::unordered_map<std::pair<uint8_t, uint8_t>, ItemOnMapForClient, pairhash>
      itemsOnMap;
...
#endif  // CLIENT_V3_ENTITIES_MAP_CLIENT_HPP_
