...
InternetUpdater *InternetUpdater::internetUpdater = nullptr;

InternetUpdater::InternetUpdater() {
    if (!connect(&newUpdateTimer, &QTimer::timeout, this,
                 &InternetUpdater::downloadFile))
        abort();
...
InternetUpdater *InternetUpdater::GetInstance() {
    if (internetUpdater == nullptr) internetUpdater = new InternetUpdater();
    return internetUpdater;
}
...
void InternetUpdater::downloadFile() {
    ...
    if (Ultimate::ultimate.isUltimate())
        catchChallengerVersion =
            ...
            QString::fromStdString(CatchChallenger::Version::str);
    else
        catchChallengerVersion =
            "CatchChallenger/" +
            QString::fromStdString(CatchChallenger::Version::str);
    ...
    catchChallengerVersion +=
        QStringLiteral(" (OS: %1)")
            .arg(QString::fromStdString(GetOSDisplayString()));
#endif
    catchChallengerVersion +=
    ...
    if (!connect(reply, &QNetworkReply::finished, this,
                 &InternetUpdater::httpFinished))
        abort();
}
...
void InternetUpdater::httpFinished() {
    ...
    if (!reply->isFinished()) {
        ...
        std::cerr << "get the new update failed: " +
                         reply->errorString().toStdString()
                  << std::endl;
        reply->deleteLater();
        return;
    } else if (!redirectionTarget.isNull()) {
        std::cerr << "redirection denied to: " +
                         redirectionTarget.toUrl().toString().toStdString()
                  << std::endl;
        reply->deleteLater();
        return;
    }
    ...
    if (!QString::fromStdString(newVersion)
             .contains(
...
bool InternetUpdater::versionIsNewer(const std::string &version) {
    ...
    std::vector<std::string> versionBNumber =
        stringsplit(std::string(CatchChallenger::Version::str), '.');
    ...
    while (index < versionANumber.size() && index < versionBNumber.size()) {
        ...
        if (reaNumberA > reaNumberB) return true;
        if (reaNumberA < reaNumberB) return false;
        index++;
    }
...
std::string InternetUpdater::getText(const std::string &version) {
    ...
    return QStringLiteral(
               "<a href=\"%1\" "
               "style=\"text-decoration:none;color:#100;\">%2</a>")
               .arg(QString::fromStdString(url))
               .arg(tr("New version: %1")
                        .arg("<b>" + QString::fromStdString(version) + "</b>"))
               .toStdString() +
           "<br />" + tr("Click here to go on download page").toStdString();
}
...
std::string InternetUpdater::GetOSDisplayString() {
    ...
    if (bOsVersionInfoEx == 0) return "Os detection blocked";
    ...
    if (VER_PLATFORM_WIN32_NT == osvi.dwPlatformId && osvi.dwMajorVersion > 4) {
        if (osvi.dwMajorVersion == 6) {
            switch (osvi.dwMinorVersion) {
                case 0:
                    if (osvi.wProductType == VER_NT_WORKSTATION)
                        ...
                        Os += QStringLiteral("Windows Server 2008 ");
                    break;
                case 1:
                    if (osvi.wProductType == VER_NT_WORKSTATION)
                        Os += QStringLiteral("Windows 7 ");
                    else
                        Os += QStringLiteral("Windows Server 2008 R2 ");
                    break;
                case 2:
                    if (osvi.wProductType == VER_NT_WORKSTATION)
                        Os += QStringLiteral("Windows 8 ");
                    else
                        Os += QStringLiteral("Windows Server 2012 ");
                    break;
                default:
                    if (osvi.wProductType == VER_NT_WORKSTATION)
                        Os += QStringLiteral(
                                  "Windows (dwMajorVersion: %1, "
                                  "dwMinorVersion: %2)")
                                  .arg(osvi.dwMinorVersion)
                                  .arg(osvi.dwMinorVersion);
                    else
                        Os += QStringLiteral(
                                  "Windows Server (dwMajorVersion: %1, "
                                  "dwMinorVersion: %2)")
                                  .arg(osvi.dwMinorVersion)
                                  .arg(osvi.dwMinorVersion);
                    break;
            ...
            switch (dwType) {
                ...
                case PRODUCT_SMALLBUSINESS_SERVER_PREMIUM:
                    Os +=
                        QStringLiteral("Small Business Server Premium Edition");
                    break;
                ...
                case PRODUCT_STANDARD_SERVER_CORE:
                    Os +=
                        QStringLiteral("Standard Edition (core installation)");
                    break;
            ...
            switch (osvi.dwMinorVersion) {
                ...
                case 2:
                    if (GetSystemMetrics(SM_SERVERR2))
                        ...
                        Os += QStringLiteral("Windows Server 2003, ");
                    ...
                    if (osvi.wProductType != VER_NT_WORKSTATION) {
                        if (si.wProcessorArchitecture ==
                            ...
                            if (osvi.wSuiteMask & VER_SUITE_DATACENTER)
                                ...
                                Os += QStringLiteral("Standard x64 Edition");
                        } else {
            ...
            if (osvi.wProductType == VER_NT_WORKSTATION)
                Os += QStringLiteral(
                          "Windows (dwMajorVersion: %1, dwMinorVersion: %2)")
                          .arg(osvi.dwMinorVersion)
                          .arg(osvi.dwMinorVersion);
            else
                Os += QStringLiteral(
                          "Windows Server (dwMajorVersion: %1, dwMinorVersion: "
                          "%2)")
                          .arg(osvi.dwMinorVersion)
                          .arg(osvi.dwMinorVersion);
        }
        ...
        if (osvi.wProductType == VER_NT_WORKSTATION)
            ...
            Os += QStringLiteral(
                      "Windows Server (dwMajorVersion: %1, dwMinorVersion: %2)")
                      .arg(osvi.dwMinorVersion)
                      .arg(osvi.dwMinorVersion);
    }
...
std::string InternetUpdater::GetOSDisplayString() {
    ...
    const auto loadOkay = domDocument->LoadFile(
    ...
    if (dictItem != NULL) {
        ...
        while (keyItem != NULL) {
            if (keyItem->Value() != NULL &&
                strcmp(keyItem->Value(), "Mac OS X") == 0) {
