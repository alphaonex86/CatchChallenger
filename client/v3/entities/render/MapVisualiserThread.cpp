...
std::string MapVisualiserThread::error() { return mLastError; }

// open the file, and load it into the variables
Map_full *MapVisualiserThread::loadOtherMap(
    const std::string &resolvedFileName) {
  if (stopIt) return NULL;
  Map_full *tempMapObject = new Map_full();
  ...
  // load the map
  {
    ...
    tempMapObject->tiledMap =
        reader.readMap(QString::fromStdString(resolvedFileName));
    qDebug() << QStringLiteral("%1 loaded into %2ms")
                    .arg(QString::fromStdString(resolvedFileName))
                    .arg(time.elapsed());
  }
  if (!tempMapObject->tiledMap) {
    mLastError = reader.errorString().toStdString();
    qDebug() << QStringLiteral("(1) Unable to load the map: %1, error: %2")
                    .arg(QString::fromStdString(resolvedFileName))
  ...
  tempMapObject->logicalMap.width =
      static_cast<uint8_t>(map_loader.map_to_send.width);
  tempMapObject->logicalMap.height =
      static_cast<uint8_t>(map_loader.map_to_send.height);
  ...
  // load the item
  {
    ...
    while (index < map_loader.map_to_send.items.size()) {
      const CatchChallenger::Map_to_send::ItemOnMap_Semi &item =
          map_loader.map_to_send.items.at(index);
      ...
      if (QtDatapackClientLoader::GetInstance()->get_itemOnMap().find(
        ...
        if (QtDatapackClientLoader::GetInstance()->get_itemOnMap()
          ...
          qDebug() << QStringLiteral("Map itemOnMap %1,%2 not found")
                          .arg(item.point.x)
                          .arg(item.point.y);
      } else {
        QStringList keys;
      ...
      tempMapObject->logicalMap
          .itemsOnMap[std::pair<uint8_t, uint8_t>(item.point.x, item.point.y)] =
          newItem;
      index++;
    }
  }
  ...
  if (tempMapObject->tiledMap->tileHeight() != CLIENT_BASE_TILE_SIZE ||
      tempMapObject->tiledMap->tileWidth() != CLIENT_BASE_TILE_SIZE) {
    mLastError = "Map tile size not multiple of " +
                 std::to_string(CLIENT_BASE_TILE_SIZE);
  ...
  if (!map_loader.map_to_send.border.bottom.fileName.empty())
    tempMapObject->logicalMap.border_semi.bottom.fileName =
        QFileInfo(
            QFileInfo(QString::fromStdString(resolvedFileName)).absolutePath() +
            "/" +
  ...
  if (!map_loader.map_to_send.border.top.fileName.empty())
    tempMapObject->logicalMap.border_semi.top.fileName =
        QFileInfo(
            QFileInfo(QString::fromStdString(resolvedFileName)).absolutePath() +
            "/" +
            QString::fromStdString(
  ...
  if (!map_loader.map_to_send.border.left.fileName.empty())
    tempMapObject->logicalMap.border_semi.left.fileName =
        QFileInfo(
            QFileInfo(QString::fromStdString(resolvedFileName)).absolutePath() +
            "/" +
            QString::fromStdString(
  ...
  tempMapObject->logicalMap.teleport_semi.clear();
  {
    ...
    while (index < map_loader.map_to_send.teleport.size()) {
      const CatchChallenger::Map_semi_teleport &teleport =
          map_loader.map_to_send.teleport.at(index);
      tempMapObject->logicalMap.teleport_semi.push_back(teleport);
      tempMapObject->logicalMap.teleport_semi[index].map =
          QFileInfo(QFileInfo(QString::fromStdString(resolvedFileName))
                        .absolutePath() +
                    "/" +
                    QString::fromStdString(
                        tempMapObject->logicalMap.teleport_semi.at(index).map))
              .absoluteFilePath()
              .toStdString();
      ...
      std::string conditionText;
      {
        if (item != NULL) {
          ...
          const tinyxml2::XMLElement *blockedtext =
              item->FirstChildElement("blockedtext");
          if (!language.empty() && language != "en")
            while (blockedtext != NULL) {
              if (blockedtext->Attribute("lang") != NULL &&
                  blockedtext->Attribute("lang") == language &&
                  blockedtext->GetText() != NULL) {
          ...
          if (!text_found) {
            ...
            while (blockedtext != NULL) {
              if (blockedtext->Attribute("lang") == NULL ||
                  strcmp(blockedtext->Attribute("lang"), "en") == 0)
      ...
      tempMapObject->logicalMap.teleport_condition_texts.push_back(
          conditionText);
      index++;
    }
  }

  tempMapObject->logicalMap.rescue_points =
      map_loader.map_to_send.rescue_points;

  // load the render
  switch (tempMapObject->tiledMap->orientation()) {
    case Tiled::Map::Isometric:
      tempMapObject->tiledRender =
          new Tiled::IsometricRenderer(tempMapObject->tiledMap);
      break;
    case Tiled::Map::Orthogonal:
    default:
      tempMapObject->tiledRender =
          new Tiled::OrthogonalRenderer(tempMapObject->tiledMap);
      break;
  }
  // tempMapObject->tiledRender->setObjectBorder(false);

  // do the object group to move the player on it
  tempMapObject->objectGroup = new Tiled::ObjectGroup(
      "Dyna management", 0, 0, tempMapObject->tiledMap->width(),
      tempMapObject->tiledMap->height());
  ...
  if (debugTags && tagTileset) {
    ...
    tagMapObject->setPosition(QPoint(tempMapObject->logicalMap.width / 2,
                                     tempMapObject->logicalMap.height / 2 + 1));
    tempMapObject->objectGroup->addObject(tagMapObject);
    tagTilesetIndex++;
    if (tagTilesetIndex >= tagTileset->tileCount()) tagTilesetIndex = 0;
  } else
    MapVisualiserThread::layerChangeLevelAndTagsChange(tempMapObject,
                                                       hideTheDoors);
  ...
  // load the animation into tile layer
  {
    ...
    while (index < tempMapObject->tiledMap->layerCount()) {
      if (Tiled::TileLayer *tileLayer =
              tempMapObject->tiledMap->layerAt(index)->asTileLayer()) {
          ...
          while (y < height) {
            ...
            if (tile != NULL) {
              const std::string &random =
                  tile->property("random").toStdString();
              const std::string &animation =
                  tile->property("animation").toStdString();
              if (!random.empty()) {
                bool ok = false;
                const uint8_t random_int = stringtouint8(random, &ok);
                if (ok) {
                  cell.tile = cell.tile->tileset()->tileAt(cell.tile->id() +
                                                           rand() % random_int);
                  tileLayer->setCell(x, y, cell);
                } else
                  qDebug() << "random property not uint8"
                           << tile->property("random");
              } else if (!animation.empty()) {
                const std::vector<std::string> &animationList =
                    stringsplit(animation, ';');
                if (animationList.size() >= 2) {
                  if (QString::fromStdString(animationList.at(0))
                          .contains(regexMs) &&
                      QString::fromStdString(animationList.at(1))
                          .contains(regexFrames)) {
                    ...
                    if (temp_ms >= 10 && temp_ms <= 65535) {
                      const unsigned int temp_frames =
                          stringtouint8(framesString);
                      if (temp_frames > 1 && temp_frames <= 255) {
                        const uint16_t ms = static_cast<uint16_t>(temp_ms);
                        const uint8_t frames =
                            static_cast<uint8_t>(temp_frames);
                        {
                          Tiled::Cell cell;
                          cell.tile = NULL;
                          tileLayer->setCell(x, y, cell);
                        ...
                        if (index < (tempMapObject->tiledMap->layerCount()))
                          if (Tiled::ObjectGroup *objectGroupTemp =
                                  tempMapObject->tiledMap->layerAt(index + 1)
                                      ->asObjectGroup())
                            objectGroup = objectGroupTemp;
                        if (objectGroup == NULL) {
                          objectGroup = new Tiled::ObjectGroup;
                          objectGroup->setName("Layer for animation " +
                                               tileLayer->name());
                          tempMapObject->tiledMap->insertLayer(index + 1,
                                                               objectGroup);
                        }
                        ...
                        const int tileId =
                            tile->id();  // to have const, ignore random
                        if (tempMapObject->animatedObject.find(ms) ==
                                tempMapObject->animatedObject.cend() ||
                            tempMapObject->animatedObject.at(ms).find(tileId) ==
                                tempMapObject->animatedObject.at(ms).cend()) {
                          Map_animation tempAnimationDescriptor;
                          tempAnimationDescriptor.minId = tileId;
                          tempAnimationDescriptor.maxId = tileId + frames;
                          tempMapObject->animatedObject[ms][tileId] =
                              tempAnimationDescriptor;
                        }
                        Map_animation_object map_animation_object;
                        if (animationList.size() >= 3 &&
                            animationList.at(2) == "random-offset")
                          cell.tile =
                              tile->tileset()->tileAt(tileId + rand() % frames);
                        else
                          cell.tile = tile;
                        object->setCell(cell);
                        ...
                        tempMapObject->animatedObject[ms][tileId]
                            .animatedObjectList.push_back(map_animation_object);
                      } else
                        qDebug() << "frames is not in good range"
                                 << tile->property("animation");
                    } else
                  ...
                  qDebug() << "Wrong animation tile args count (thread)"
                           << tile->property("animation");
              } else {
                if (tileToTriggerAnimationContent.find(tile) !=
                    tileToTriggerAnimationContent.cend()) {
                  const TriggerAnimationContent &content =
                      tileToTriggerAnimationContent.at(tile);
                  Tiled::ObjectGroup *objectGroup = NULL;
                  if (index < (tempMapObject->tiledMap->layerCount()))
                    if (Tiled::ObjectGroup *objectGroupTemp =
                            tempMapObject->tiledMap->layerAt(index + 1)
                  ...
                  if (objectGroup == NULL) {
                    objectGroup = new Tiled::ObjectGroup;
                    objectGroup->setName("From cache under " +
                                         tileLayer->name());
                    tempMapObject->tiledMap->insertLayer(index + 1,
                                                         objectGroup);
                  }
                  ...
                  // register on map
                  tempMapObject->triggerAnimations[std::pair<uint8_t, uint8_t>(
                      static_cast<uint8_t>(x), static_cast<uint8_t>(y))] =
                      new TriggerAnimation(
                          object, objectOver, content.framesCountEnter,
                          content.msEnter, content.framesCountLeave,
                          content.msLeave, content.framesCountAgain,
                          content.msAgain, content.over);
                  {
                    Tiled::Cell cell;
                    cell.tile = NULL;
                    tileLayer->setCell(x, y, cell);
                  ...
                  const std::string &trigger =
                      tile->property("trigger").toStdString();
                  if (!trigger.empty()) {
                    if (QString::fromStdString(trigger).contains(
                            regexTrigger)) {
                      ...
                      tempString = QString::fromStdString(tempString)
                                       .replace(regexTrigger, "\\1")
                                       .toStdString();
                      content.msEnter = stringtouint16(tempString);
                      tempString = trigger;
                      tempString = QString::fromStdString(tempString)
                                       .replace(regexTrigger, "\\2")
                                       .toStdString();
                      content.framesCountEnter = stringtouint8(tempString);
                      tempString = trigger;
                      tempString = QString::fromStdString(tempString)
                                       .replace(regexTrigger, "\\3")
                                       .toStdString();
                      content.msLeave = stringtouint16(tempString);
                      tempString = trigger;
                      tempString = QString::fromStdString(tempString)
                                       .replace(regexTrigger, "\\4")
                                       .toStdString();
                      content.framesCountLeave = stringtouint8(tempString);
                      tempString = trigger;
                      tempString = QString::fromStdString(tempString)
                                       .replace(regexTrigger, "\\5")
                                       .toStdString();
                      // again here
                      if (QString::fromStdString(tempString)
                              .contains(regexTriggerAgain)) {
                        std::string againString = tempString;
                        tempString = QString::fromStdString(againString)
                                         .replace(regexTriggerAgain, "\\1")
                                         .toStdString();
                        content.msAgain = stringtouint16(againString);
                        againString = tempString;
                        tempString = QString::fromStdString(againString)
                                         .replace(regexTriggerAgain, "\\2")
                                         .toStdString();
                      ...
                      if (tempString.find("over") != std::string::npos) {
                        content.over = true;
                        content.objectTileOver = tile->tileset()->tileAt(
                      ...
                      // create tile and add
                      {
                        ...
                        // register on map
                        tempMapObject
                            ->triggerAnimations[std::pair<uint8_t, uint8_t>(
...
bool MapVisualiserThread::loadOtherMapClientPart(Map_full *parsedMap) {
  ...
  if (root->Name() == NULL) {
    qDebug() << QString::fromStdString(fileName)
             << QStringLiteral(
                    ", MapVisualiserThread::loadOtherMapClientPart(): \"map\" "
                    "root balise not found 2 for the xml file: ")
             << root->Name();
    return false;
  }
  if (strcmp(root->Name(), "map") != 0) {
    qDebug() << QString::fromStdString(fileName)
             << QStringLiteral(
                    ", MapVisualiserThread::loadOtherMapClientPart(): \"map\" "
                    "root balise not found for the xml file: ")
             << root->Name();
    return false;
  }
  bool ok, ok2;
  ...
  while (child != NULL) {
    if (child->Attribute("name") == NULL)
      qDebug() << (QStringLiteral(
                       "Has not attribute \"name\": child->Name(): %1")
                       .arg(child->Name()));
    else {
      if (strcmp(child->Attribute("name"), "Object") == 0) {
        ...
        while (bot != NULL) {
          if (bot->Attribute("type") == NULL)
            qDebug() << (QStringLiteral(
                             "Has not attribute \"type\": bot->Name(): %1")
                             .arg(bot->Name()));
          else if (bot->Attribute("x") == NULL)
            qDebug() << (QStringLiteral(
                             "Has not attribute \"x\": bot->Name(): %1")
                             .arg(bot->Name()));
          else if (bot->Attribute("y") == NULL)
            qDebug() << (QStringLiteral(
                             "Has not attribute \"y\": bot->Name(): %1")
                             .arg(bot->Name()));
          else {
            const uint32_t &x = stringtouint32(bot->Attribute("x"), &ok) /
                                CLIENT_BASE_TILE_SIZE;
            const uint32_t &y = stringtouint32(bot->Attribute("y"), &ok2) /
                                    CLIENT_BASE_TILE_SIZE -
                                1;
            if (ok && ok2 &&
              ...
              const tinyxml2::XMLElement *properties =
                  bot->FirstChildElement("properties");
              while (properties != NULL) {
                ...
                while (property != NULL) {
                  if (property->Attribute("name") == NULL)
                    qDebug() << (QStringLiteral("Has not attribute \"name\": "
                                                "property->Name(): %1")
                                     .arg(property->Name()));
                  else if (property->Attribute("value") == NULL)
                    qDebug() << (QStringLiteral("Has not attribute \"value\": "
                                                "property->Name(): %1")
                                     .arg(property->Name()));
                  else
                    property_parsed[property->Attribute("name")] =
                        property->Attribute("value");
                  property = property->NextSiblingElement("property");
                }
                if (property_parsed.find("file") != property_parsed.cend() &&
                    property_parsed.find("id") != property_parsed.cend()) {
                  const uint16_t &botId =
                      stringtouint16(property_parsed.at("id"), &ok);
                  if (ok) {
                    std::string botFile =
                        QFileInfo(
                            ...
                            .absoluteFilePath()
                            .toStdString();
                    if (!stringEndsWith(botFile, ".xml")) botFile += ".xml";
                    if (strcmp(bot->Attribute("type"), "bot") == 0) {
                      if (stopIt) return false;
                      loadBotFile(botFile);
                      if (stopIt) return false;
                      if (botFiles.find(botFile) != botFiles.cend()) {
                        if (botFiles.at(botFile).find(botId) !=
                            botFiles.at(botFile).cend()) {
#ifdef DEBUG_CLIENT_BOT
                          qDebug() << (QStringLiteral(
                                           "Put bot %1 (%2) at %3 (%4,%5)")
                                           .arg(botFile)
                                           .arg(botId)
                          ...
                          while (i != bot.step.cend()) {
                            ...
                            } else if (strcmp(step->Attribute("type"),
                                              "shop") == 0) {
                              if (step->Attribute("shop") == NULL)
                                qDebug() << (QStringLiteral(
                                                 "Has not attribute \"shop\": "
                                                 "for bot id: %1 (%2)")
                                                 .arg(botId)
                                                 .arg(QString::fromStdString(
                                                     botFile)));
                              else {
                                const uint16_t shop = stringtouint16(
                                    step->Attribute("shop"), &ok);
                                if (!ok)
                                  qDebug() << (QStringLiteral(
                                                   "shop is not a number: for "
                                                   "bot id: %1 (%2)")
                                                   .arg(botId)
                                                   .arg(QString::fromStdString(
                                                       botFile)));
                                  ...
                                  parsedMap->logicalMap
                                      .shops[std::pair<uint8_t, uint8_t>(x, y)]
                                      .push_back(shop);
                              }
                            } else if (strcmp(step->Attribute("type"),
                                              "learn") == 0) {
                              if (parsedMap->logicalMap.learn.find(
                                      std::pair<uint8_t, uint8_t>(x, y)) !=
                                  parsedMap->logicalMap.learn.cend())
                                qDebug() << (QStringLiteral(
                                                 "learn point already on the "
                                                 "map: for bot id: %1 (%2)")
                                                 .arg(botId)
                                                 .arg(QString::fromStdString(
                                                     botFile)));
                              else
                                parsedMap->logicalMap.learn.insert(
                                    std::pair<uint8_t, uint8_t>(x, y));
                            } else if (strcmp(step->Attribute("type"),
                                              "heal") == 0) {
                              if (parsedMap->logicalMap.heal.find(
                                      std::pair<uint8_t, uint8_t>(x, y)) !=
                                  parsedMap->logicalMap.heal.cend())
                                qDebug() << (QStringLiteral(
                                                 "heal point already on the "
                                                 "map: for bot id: %1 (%2)")
                                                 .arg(botId)
                                                 .arg(QString::fromStdString(
                                                     botFile)));
                              else
                                parsedMap->logicalMap.heal.insert(
                                    std::pair<uint8_t, uint8_t>(x, y));
                            } else if (strcmp(step->Attribute("type"),
                                              "market") == 0) {
                              if (parsedMap->logicalMap.market.find(
                                      std::pair<uint8_t, uint8_t>(x, y)) !=
                                  parsedMap->logicalMap.market.cend())
                                qDebug() << (QStringLiteral(
                                                 "market point already on the "
                                                 "map: for bot id: %1 (%2)")
                                                 .arg(botId)
                                                 .arg(QString::fromStdString(
                                                     botFile)));
                              else
                                parsedMap->logicalMap.market.insert(
                                    std::pair<uint8_t, uint8_t>(x, y));
                            } else if (strcmp(step->Attribute("type"),
                                              "zonecapture") == 0) {
                              if (step->Attribute("zone") == NULL)
                                qDebug() << (QStringLiteral(
                                                 "zonecapture point have not "
                                                 "the zone attribute: for bot "
                                                 "id: %1 (%2)")
                                                 .arg(botId)
                                                 .arg(QString::fromStdString(
                                                     botFile)));
                              else if (parsedMap->logicalMap.zonecapture.find(
                                           std::pair<uint8_t, uint8_t>(x, y)) !=
                                       parsedMap->logicalMap.zonecapture.cend())
                                qDebug() << (QStringLiteral(
                                                 "zonecapture point already on "
                                                 "the map: for bot id: %1 (%2)")
                                                 .arg(botId)
                                                 .arg(QString::fromStdString(
                                                     botFile)));
                              else {
                                if (CatchChallenger::CommonDatapackServerSpec::
                                  ...
                                  qDebug()
                                      << (QStringLiteral(
                                              "zonecapture %1 (%2) can't be "
                                              "converted because zone not "
                                              "exists: %3")
                                              .arg(botId)
                                              .arg(QString::fromStdString(
                                                  botFile))
                                              .arg(step->Attribute("zone")));
                              }
                            } else if (strcmp(step->Attribute("type"),
                                              "fight") == 0) {
                              if (parsedMap->logicalMap.botsFight.find(
                                      std::pair<uint8_t, uint8_t>(x, y)) !=
                                  parsedMap->logicalMap.botsFight.cend())
                                qDebug() << (QStringLiteral(
                                                 "botsFight point already on "
                                                 "the map: for bot id: %1 (%2)")
                                                 .arg(botId)
                                                 .arg(QString::fromStdString(
                                                     botFile)));
                              else {
                                const uint16_t &fightid = stringtouint16(
                                    step->Attribute("fightid"), &ok);
                                if (ok)
                                  if (CatchChallenger::
                                          CommonDatapackServerSpec::
                          ...
                          if (botFiles.find(botFile) != botFiles.cend()) {
                            ...
                            if (botList.find(botId) != botList.cend()) {
                              qDebug()
                                  << (QStringLiteral(
                                          "botId %1 into %2: "
                                          "properties->Name(): %3, file: %4")
                                          .arg(botId)
                                          .arg(QString::fromStdString(botFile))
                                          .arg(bot->Name())
                                          .arg(QString::fromStdString(
                                              parsedMap->logicalMap.map_file)));
                              CatchChallenger::Bot &bot =
                                  parsedMap->logicalMap
                                      .bots[std::pair<uint8_t, uint8_t>(
                                          static_cast<uint8_t>(x),
                            ...
                            qDebug()
                                << (QStringLiteral(
                                        "missing file botFile, botId %1 into "
                                        "%2: properties->Name(): %3, file: %4")
                                        .arg(botId)
                                        .arg(QString::fromStdString(botFile))
                                        .arg(bot->Name())
                                        .arg(QString::fromStdString(
...
bool MapVisualiserThread::loadOtherMapMetaData(Map_full *parsedMap) {
  ...
  if (root->Name() != MapVisualiserThread::text_map) {
    qDebug() << QString::fromStdString(fileName)
             << QStringLiteral(
                    ", MapVisualiserThread::loadOtherMapMetaData(): \"map\" "
                    "root balise not found for the xml file");
    return false;
  }
...
void MapVisualiserThread::loadBotFile(const std::string &file) {
  if (botFiles.find(file) != botFiles.cend()) return;
  ...
  if (root->Name() == NULL) {
    std::cerr << "Unable to open the file: " << file
              << ", \"reputations\" root balise not found 2 for reputation of "
                 "the xml file"
              << std::endl;
    return;
  }
  if (strcmp(root->Name(), "bots") != 0) {
    std::cerr << "Unable to open the file: " << file
              << ", \"reputations\" root balise not found for reputation of "
                 "the xml file"
              << std::endl;
    return;
  }
  ...
  while (child != NULL) {
    if (child->Attribute("id") == NULL)
      qDebug() << (QStringLiteral(
                       "Has not attribute \"id\": child->Value(): %1")
                       .arg(child->Value()));
    else {
      ...
      if (ok) {
        if (botFiles.at(file).find(botId) != botFiles.at(file).cend())
          qDebug() << (QStringLiteral(
                           "bot already found with this id: bot->Value(): %1")
                           .arg(child->Value()));
        else {
          ...
          while (step != NULL) {
            if (step->Attribute("id") == NULL)
              ...
              qDebug() << (QStringLiteral(
                               "Has not attribute \"type\": bot->Value(): %1")
                               .arg(step->Value()));
            else {
              const uint8_t &stepId = stringtouint8(step->Attribute("id"), &ok);
              if (ok) botFiles[file][botId].step[stepId] = step;
            }
          ...
          if (bot.step.find(1) == bot.step.cend()) botFiles[file].erase(botId);
        }
      } else
        qDebug() << (QStringLiteral(
                         "Attribute \"id\" is not a number: bot->Value(): %1")
                         .arg(child->Value()));
    }
