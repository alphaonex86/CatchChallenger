...
class MapControllerMP : public MapVisualiserPlayerWithFight {
 ...
 public slots:
  // map move Qt
  void insert_player(const CatchChallenger::Player_public_informations &player,
                     const uint32_t &mapId, const uint16_t &x,
                     const uint16_t &y,
  ...
  void reinsert_player(const uint16_t &id, const uint8_t &x, const uint8_t &y,
                       const CatchChallenger::Direction &direction);
  ...
  bool move_player_final(
      const uint16_t &id,
      const std::vector<std::pair<uint8_t, CatchChallenger::Direction> >
          &movement,
      bool inReplayMode);
  bool move_otherMonster(
      OtherPlayer &otherPlayer, const bool &haveMoved,
  ...
  bool reinsert_player_final(const uint16_t &id, const uint8_t &x,
                             const uint8_t &y,
                             const CatchChallenger::Direction &direction,
                             bool inReplayMode);
  bool full_reinsert_player_final(const uint16_t &id, const uint32_t &mapId,
                                  const uint8_t &x, const uint8_t &y,
                                  const CatchChallenger::Direction &direction,
                                  bool inReplayMode);
  ...
  void haveCharacter();
  bool teleportTo(const uint32_t &mapId, const uint16_t &x, const uint16_t &y,
                  const CatchChallenger::Direction &direction) override;
  virtual bool asyncMapLoaded(const std::string &fileName,
                              Map_full *tempMapObject) override;
 ...
 private:
  ...
  std::unordered_map<QTimer *, uint16_t> otherPlayerListByTimer,
      otherPlayerListByAnimationTimer;
 ...
 private slots:
  ...
  // virtual std::unordered_set<std::string> loadMap(Map_full *map,const bool
  // &display);
  void updateOtherPlayerMonsterTile(OtherPlayer &tempPlayer,
 ...
 protected slots:
  bool loadPlayerMap(const std::string &fileName, const uint8_t &x,
                     const uint8_t &y) override;
  virtual void finalPlayerStep(bool parseKey = true) override;
  // call after enter on new map
  virtual void loadOtherPlayerFromMap(const OtherPlayer &otherPlayer,
                                      const bool &display = true);
  // call before leave the old map (and before loadPlayerFromCurrentMap())
  virtual void unloadOtherPlayerFromMap(const OtherPlayer &otherPlayer);
  void pathFindingResult(
