...
class MapController : public MapControllerMP {
 ...
 public:
  explicit MapController(const bool &debugTags = false);
  ~MapController();
  ...
  void remove_plant_full(const std::string &map, const uint8_t &x,
                         const uint8_t &y);
  void insert_plant_full(const std::string &map, const uint8_t &x,
 ...
 private:
  ...
  std::unordered_map<std::string, std::vector<DelayedPlantInsert> >
      delayedPlantInsertOnMap;
  struct PlantTimer {
 ...
 protected slots:
  ...
  bool updatePlantGrowing(CatchChallenger::ClientPlantWithTimer
                              *plant);  // return true if is growing
 ...
 private slots:
  void loadBotOnTheMap(Map_full *parsedMap, const uint32_t &botId,
