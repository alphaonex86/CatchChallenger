...
MapControllerMP::MapControllerMP(const bool &debugTags)
  ...
  qRegisterMetaType<CatchChallenger::Player_public_informations>(
      "CatchChallenger::Player_public_informations");
  ...
  qRegisterMetaType<
      std::vector<std::pair<uint8_t, CatchChallenger::Direction> > >(
      "std::vector<std::pair<uint8_t,CatchChallenger::Direction> >");
  qRegisterMetaType<std::vector<Map_full> >("std::vector<Map_full>");
  qRegisterMetaType<
      std::vector<std::pair<CatchChallenger::Direction, uint8_t> > >(
      "std::vector<std::pair<CatchChallenger::Direction,uint8_t> >");
  qRegisterMetaType<
      std::vector<std::pair<CatchChallenger::Orientation, uint8_t> > >(
      "std::vector<std::pair<CatchChallenger::Orientation,uint8_t> >");
  if (!connect(&pathFinding, &PathFinding::result, this,
               &MapControllerMP::pathFindingResult))
    abort();
...
MapControllerMP::~MapControllerMP() { pathFinding.cancel(); }
...
void MapControllerMP::connectAllSignals(
  ...
  if (!QObject::connect(
          client, &CatchChallenger::Api_client_real::Qthave_current_player_info,
          this, &MapControllerMP::have_current_player_info,
          (Qt::ConnectionType)(Qt::QueuedConnection | Qt::UniqueConnection)))
    abort();
  if (!QObject::connect(
          client, &CatchChallenger::Api_client_real::Qtinsert_player, this,
          &MapControllerMP::insert_player,
          (Qt::ConnectionType)(Qt::QueuedConnection | Qt::UniqueConnection)))
    abort();
  if (!QObject::connect(
          client, &CatchChallenger::Api_client_real::Qtremove_player, this,
          &MapControllerMP::remove_player,
          (Qt::ConnectionType)(Qt::QueuedConnection | Qt::UniqueConnection)))
    abort();
  if (!QObject::connect(
          client, &CatchChallenger::Api_client_real::Qtmove_player, this,
          &MapControllerMP::move_player,
          (Qt::ConnectionType)(Qt::QueuedConnection | Qt::UniqueConnection)))
    abort();
  if (!QObject::connect(
          client, &CatchChallenger::Api_client_real::Qtreinsert_player, this,
          &MapControllerMP::reinsert_player,
          (Qt::ConnectionType)(Qt::QueuedConnection | Qt::UniqueConnection)))
    abort();
  if (!QObject::connect(
          client, &CatchChallenger::Api_client_real::Qtfull_reinsert_player,
          this, &MapControllerMP::full_reinsert_player,
          (Qt::ConnectionType)(Qt::QueuedConnection | Qt::UniqueConnection)))
    abort();
  if (!QObject::connect(
          client, &CatchChallenger::Api_client_real::QtdropAllPlayerOnTheMap,
          this, &MapControllerMP::dropAllPlayerOnTheMap,
          (Qt::ConnectionType)(Qt::QueuedConnection | Qt::UniqueConnection)))
    abort();
  if (!QObject::connect(
          client, &CatchChallenger::Api_client_real::QtteleportTo, this,
          &MapControllerMP::teleportTo,
          (Qt::ConnectionType)(Qt::QueuedConnection | Qt::UniqueConnection)))
    abort();
  if (!QObject::connect(
          client, &CatchChallenger::Api_client_real::QthaveCharacter, this,
          &MapControllerMP::haveCharacter,
          (Qt::ConnectionType)(Qt::QueuedConnection | Qt::UniqueConnection)))
    abort();

  if (!QObject::connect(
          this, &MapControllerMP::send_player_direction, client,
...
void MapControllerMP::setScale(const float &scaleSize) {
  ...
  if (scaleSize < 1 || scaleSize > 4) {
    qDebug()
        << QStringLiteral("scaleSize out of range 1-4: %1").arg(scaleSizeQReal);
    return;
  }
...
void MapControllerMP::updateScale() { setScale(this->scaleSize); }
...
const std::unordered_map<uint16_t, MapControllerMP::OtherPlayer>
    &MapControllerMP::getOtherPlayerList() const {
  return otherPlayerList;
}

bool MapControllerMP::loadPlayerMap(const std::string &fileName,
                                    const uint8_t &x, const uint8_t &y) {
#ifdef CATCHCHALLENGER_EXTRA_CHECK
  if (!QFileInfo(QString::fromStdString(fileName)).exists())
    qDebug() << (QStringLiteral("fileName: %1")
...
void MapControllerMP::updateOtherPlayerMonsterTile(OtherPlayer &tempPlayer,
  ...
  if (monsterTilesetCache.find(imagePath) != monsterTilesetCache.cend())
    ...
    if (!image.isNull()) {
      tempPlayer.monsterTileset =
          new Tiled::Tileset(QString::fromStdString(lastTileset), 32, 32);
      if (!tempPlayer.monsterTileset->loadFromImage(
  ...
  if (resetMonster) loadOtherMonsterFromCurrentMap(tempPlayer);
  resetOtherMonsterTile(tempPlayer);
}

void MapControllerMP::resetOtherMonsterTile(OtherPlayer &tempPlayer) {
  if (tempPlayer.monsterMapObject == nullptr) return;
...
void MapControllerMP::loadOtherPlayerFromMap(const OtherPlayer &otherPlayer,
                                             const bool &display) {
  Q_UNUSED(display);
  ...
  if (otherPlayer.labelMapObject != NULL)
    otherPlayer.labelMapObject->setPosition(QPointF(
        static_cast<qreal>(otherPlayer.x) -
  ...
  if (ObjectGroupItem::objectGroupLink.find(
          otherPlayer.presumed_map->objectGroup) !=
      ObjectGroupItem::objectGroupLink.cend()) {
    ObjectGroupItem::objectGroupLink.at(otherPlayer.presumed_map->objectGroup)
        ->addObject(otherPlayer.playerMapObject);
    if (otherPlayer.labelMapObject != NULL)
      ObjectGroupItem::objectGroupLink
          .at(otherPlayer.presumed_map->objectGroup)
          ->addObject(otherPlayer.labelMapObject);
    if (MapObjectItem::objectLink.find(otherPlayer.playerMapObject) ==
        MapObjectItem::objectLink.cend())
      qDebug() << QStringLiteral(
          "loadOtherPlayerFromMap(), MapObjectItem::objectLink don't have "
          "otherPlayer.playerMapObject");
    else {
      if (MapObjectItem::objectLink.at(otherPlayer.playerMapObject) == NULL)
        qDebug() << QStringLiteral(
            "loadOtherPlayerFromMap(), "
            "MapObjectItem::objectLink[otherPlayer.playerMapObject]==NULL");
      else {
    ...
    qDebug() << QStringLiteral(
        "loadOtherPlayerFromMap(), ObjectGroupItem::objectGroupLink not "
...
void MapControllerMP::loadOtherMonsterFromCurrentMap(
    const OtherPlayer &tempPlayer) {
  if (tempPlayer.monsterMapObject == nullptr) return;
  // monster
  if (all_map.find(tempPlayer.current_monster_map) == all_map.cend()) {
    qDebug() << QStringLiteral("all_map have not the current map: %1")
                    .arg(
                        QString::fromStdString(tempPlayer.current_monster_map));
    return;
  }
  {
    Tiled::ObjectGroup *currentGroup =
        tempPlayer.monsterMapObject->objectGroup();
    if (currentGroup != NULL) {
      if (ObjectGroupItem::objectGroupLink.find(currentGroup) !=
          ObjectGroupItem::objectGroupLink.cend())
        ObjectGroupItem::objectGroupLink.at(currentGroup)
            ->removeObject(tempPlayer.monsterMapObject);
    ...
    if (ObjectGroupItem::objectGroupLink.find(
            all_map.at(tempPlayer.current_map)->objectGroup) !=
        ObjectGroupItem::objectGroupLink.cend())
      ObjectGroupItem::objectGroupLink
          .at(all_map.at(tempPlayer.current_map)->objectGroup)
          ->addObject(tempPlayer.monsterMapObject);
    else
      qDebug() << QStringLiteral(
    ...
    // move to the final position (integer), y+1 because the tile lib start y to
    // 1, not 0
    tempPlayer.monsterMapObject->setPosition(
        QPointF(tempPlayer.monster_x - 0.5, tempPlayer.monster_y + 1));
    MapObjectItem::objectLink.at(tempPlayer.monsterMapObject)
        ->SetZValue(tempPlayer.monster_y);
  }
}
...
void MapControllerMP::unloadOtherPlayerFromMap(const OtherPlayer &otherPlayer) {
  ...
  if (ObjectGroupItem::objectGroupLink.find(
          otherPlayer.playerMapObject->objectGroup()) !=
      ObjectGroupItem::objectGroupLink.cend())
    ObjectGroupItem::objectGroupLink
        .at(otherPlayer.playerMapObject->objectGroup())
        ->removeObject(otherPlayer.playerMapObject);
  else
    qDebug() << QStringLiteral(
        "unloadOtherPlayerFromMap(), ObjectGroupItem::objectGroupLink not "
        "contains otherPlayer.playerMapObject->objectGroup()");
  if (otherPlayer.labelMapObject != NULL) {
    // unload the player sprite
    if (ObjectGroupItem::objectGroupLink.find(
            otherPlayer.labelMapObject->objectGroup()) !=
        ObjectGroupItem::objectGroupLink.cend())
      ObjectGroupItem::objectGroupLink
          .at(otherPlayer.labelMapObject->objectGroup())
          ->removeObject(otherPlayer.labelMapObject);
    else
      qDebug() << QStringLiteral(
          "unloadOtherPlayerFromMap(), ObjectGroupItem::objectGroupLink not "
          "contains otherPlayer.labelMapObject->objectGroup()");
  }
  /* done into remove_player_final()
  {
      ...
      if(otherPlayerList.contains(simplifiedId))
          ...
          qDebug() << QStringLiteral("simplifiedId: %1 not found into
  otherPlayerList").arg(simplifiedId);
  }*/
  ...
  for (const auto &n : otherPlayer.mapUsed) {
    ...
    if (mapUsedByOtherPlayer.find(map) != mapUsedByOtherPlayer.cend()) {
      mapUsedByOtherPlayer[map]--;
      if (mapUsedByOtherPlayer.at(map) == 0) mapUsedByOtherPlayer.erase(map);
    } else
      qDebug() << QStringLiteral(
                      "map not found into mapUsedByOtherPlayer for player: %1, "
                      "map: %2")
...
void MapControllerMP::unloadOtherMonsterFromCurrentMap(
    const MapControllerMP::OtherPlayer &tempPlayer) {
  // monster
  {
    Tiled::ObjectGroup *currentGroup =
        tempPlayer.monsterMapObject->objectGroup();
    ...
    if (ObjectGroupItem::objectGroupLink.find(
        ...
        ObjectGroupItem::objectGroupLink.cend())
      ObjectGroupItem::objectGroupLink
          .at(tempPlayer.monsterMapObject->objectGroup())
          ->removeObject(tempPlayer.monsterMapObject);
    else
      qDebug() << QStringLiteral(
...
void MapControllerMP::haveCharacter() {
  ...
  if (playerInformations.bot_already_beaten == NULL) {
    std::cerr
        << "void BaseWindow::updateConnectingStatus(): waitedData.isEmpty(), "
           "playerInformations.bot_already_beaten==NULL"
        << std::endl;
    abort();
  }
}

bool MapControllerMP::teleportTo(const uint32_t &mapId, const uint16_t &x,
                                 const uint16_t &y,
  ...
  if (!mHaveTheDatapack || !player_informations_is_set) {
    ...
    qDebug() << QStringLiteral("delayed teleportTo(%1,%2,%3,%4)")
                    .arg(DatapackClientLoader::datapackLoader.maps.value(mapId))
                    .arg(x)
                    .arg(y)
  ...
  if (!MapVisualiserPlayer::teleportTo(mapId, x, y, direction)) return false;
...
// player info
void MapControllerMP::have_current_player_info(
  ...
  if (mHaveTheDatapack) reinject_signals();
}
...
void MapControllerMP::datapackParsedMainSub() {
  ...
  skinFolderList = CatchChallenger::FacilityLibGeneral::skinIdList(
      datapackPath + DATAPACK_BASE_PATH_SKIN);

  if (player_informations_is_set) reinject_signals();
}
  ...
  if (mHaveTheDatapack && player_informations_is_set) {
#ifdef DEBUG_CLIENT_LOAD_ORDER
    qDebug() << QStringLiteral(
        "MapControllerMP::reinject_signals(): mHaveTheDatapack && "
    ...
    while (index < delayedActions.size()) {
      ...
      switch (delayedActions.at(index).type) {
        case DelayedType_Insert:
          if (insert_player_final(delayedActions.at(index).insert.player,
                                  ...
                                  delayedActions.at(index).insert.x,
                                  delayedActions.at(index).insert.y,
                                  delayedActions.at(index).insert.direction,
                                  true))
            delayedActions.erase(delayedActions.cbegin() + index);
          break;
        case DelayedType_Move:
          if (move_player_final(delayedActions.at(index).move.id,
                                delayedActions.at(index).move.movement, true))
            delayedActions.erase(delayedActions.cbegin() + index);
          break;
        ...
        case DelayedType_Reinsert_single:
          if (reinsert_player_final(
                  delayedActions.at(index).reinsert_single.id,
                  delayedActions.at(index).reinsert_single.x,
                  delayedActions.at(index).reinsert_single.y,
                  delayedActions.at(index).reinsert_single.direction, true))
            delayedActions.erase(delayedActions.cbegin() + index);
          break;
        case DelayedType_Reinsert_full:
          if (full_reinsert_player_final(
                  delayedActions.at(index).reinsert_full.id,
                  delayedActions.at(index).reinsert_full.mapId,
                  delayedActions.at(index).reinsert_full.x,
                  delayedActions.at(index).reinsert_full.y,
                  delayedActions.at(index).reinsert_full.direction, true))
            delayedActions.erase(delayedActions.cbegin() + index);
          break;
      ...
      if (delayedActions.size() > delayedActions_size) {
        qDebug() << QStringLiteral(
            "MapControllerMP::reinject_signals(): can't grow delayedActions");
        abort();
      }
      index++;
    }
    ...
    while (index < delayedTeleportTo.size()) {
      teleportTo(delayedTeleportTo.at(index).mapId,
                 delayedTeleportTo.at(index).x, delayedTeleportTo.at(index).y,
                 delayedTeleportTo.at(index).direction);
      index++;
    }
    delayedTeleportTo.clear();
  } else
    qDebug() << QStringLiteral(
        "MapControllerMP::reinject_signals(): should not pass here because all "
        "is not previously loaded");
}

void MapControllerMP::reinject_signals_on_valid_map() {
#ifdef DEBUG_CLIENT_LOAD_ORDER
  qDebug() << QStringLiteral(
      "MapControllerMP::reinject_signals_on_valid_map()");
#endif
  unsigned int index;
  ...
  if (mHaveTheDatapack && player_informations_is_set) {
#ifdef DEBUG_CLIENT_LOAD_ORDER
    qDebug() << QStringLiteral(
        "MapControllerMP::reinject_signals_on_valid_map(): mHaveTheDatapack && "
        "player_informations_is_set");
#endif
    index = 0;
    const unsigned int delayedActions_size = delayedActions.size();
    while (index < delayedActions.size()) {
      ...
      switch (delayedActions.at(index).type) {
        case DelayedType_Insert:
          if (delayedActions.at(index).insert.player.simplifiedId !=
              client->get_player_informations_ro()
                  .public_informations.simplifiedId) {
          ...
          break;
        case DelayedType_Move:
          if (otherPlayerList.find(delayedActions.at(index).move.id) !=
              otherPlayerList.cend()) {
            if (move_player_final(delayedActions.at(index).move.id,
                                  delayedActions.at(index).move.movement, true))
              delayedActions.erase(delayedActions.cbegin() + index);
            index--;
    ...
    qDebug() << QStringLiteral(
        "MapControllerMP::reinject_signals_on_valid_map(): should not pass "
        "here because all is not previously loaded");
}

bool MapControllerMP::asyncMapLoaded(const std::string &fileName,
                                     Map_full *tempMapObject) {
  if (!mHaveTheDatapack) return false;
  if (!player_informations_is_set) return false;
  const bool &result =
      MapVisualiserPlayer::asyncMapLoaded(fileName, tempMapObject);
...
void MapControllerMP::doMoveOtherAnimation() {
  ...
  if (otherPlayerListByAnimationTimer.find(timer) ==
      otherPlayerListByAnimationTimer.cend()) {
    timer->stop();
    return;
  }
  ...
  if (!CatchChallenger::CommonDatapack::commonDatapack.monstersCollision
    ...
    if (otherPlayer.monsterMapObject != NULL) {
      const Map_full *current_monster_map_pointer =
          all_map.at(otherPlayer.current_monster_map);
      ...
      }
      {
        const CatchChallenger::MonstersCollisionValue &monstersCollisionValue =
            CatchChallenger::MoveOnTheMap::getZoneCollision(
                current_monster_map_pointer->logicalMap, otherPlayer.monster_x,
                otherPlayer.monster_y);
    ...
    const CatchChallenger::MonstersCollisionValue &monstersCollisionValue =
        CatchChallenger::MoveOnTheMap::getZoneCollision(
            current_map_pointer->logicalMap, otherPlayer.presumed_x,
            otherPlayer.presumed_y);
    unsigned int index = 0;
    while (index < monstersCollisionValue.walkOn.size()) {
      ...
      if (newIndex < CatchChallenger::CommonDatapack::commonDatapack
        ...
        if (monstersCollisionTemp.tile != otherPlayer.lastTileset) {
          ...
          if (playerTilesetCache.find(otherPlayer.lastTileset) !=
              playerTilesetCache.cend())
            otherPlayer.playerTileset =
                playerTilesetCache.at(otherPlayer.lastTileset);
          else {
            if (otherPlayer.lastTileset.empty())
              otherPlayer.playerTileset = playerTilesetCache[defaultTileset];
            else {
              const std::string &playerSkinPath =
                  datapackPath + DATAPACK_BASE_PATH_SKIN +
                  skinFolderList.at(otherPlayer.informations.skinId);
              const std::string &imagePath =
                  playerSkinPath + "/" + otherPlayer.lastTileset + ".png";
              QImage image(QString::fromStdString(imagePath));
              if (!image.isNull()) {
                otherPlayer.playerTileset = new Tiled::Tileset(
                    QString::fromStdString(lastTileset), 16, 24);
                otherPlayer.playerTileset->loadFromImage(
                    image, QString::fromStdString(imagePath));
              } else {
...
void MapControllerMP::destroyMap(Map_full *map) {
  ...
  for (const auto &n : otherPlayerList2) {
    if (n.second.presumed_map == map) remove_player_final(n.first, true);
  }
...
CatchChallenger::Direction MapControllerMP::moveFromPath() {
  ...
  if (pathList.size() > 1) {
    ...
    if (startPoint.map == currentMap() && startPoint.x == x &&
        startPoint.y == y) {
      while (pathList.size() > 1) pathList.erase(pathList.cbegin());
    }

    PathResolved &pathFirst = pathList.front();
    std::pair<CatchChallenger::Orientation, uint8_t> &pathFirstUnit =
        pathFirst.path.front();
...
void MapControllerMP::eventOnMap(CatchChallenger::MapEvent event,
                                 Map_full *tempMapObject, uint8_t x,
                                 uint8_t y) {
  const std::pair<uint8_t, uint8_t> pos(getPos());
  ...
  if (event == CatchChallenger::MapEvent_SimpleClick) {
    if (keyAccepted.empty() ||
      ...
      pathFinding.searchPath(all_map, tempMapObject->logicalMap.map_file, x, y,
                             current_map, thisx, thisy,
                             client->get_player_informations_ro().items);
      if (pathList.empty()) {
        while (pathList.size() > 1) pathList.pop_back();
        // pathList.clear();
      }
    }
  }
...
bool MapControllerMP::nextPathStep()  // true if have step
{
  if (!pathList.empty()) {
    const CatchChallenger::Direction &direction =
        MapControllerMP::moveFromPath();
    return MapVisualiserPlayer::nextPathStepInternal(pathList, direction);
  }
  return false;
}

void MapControllerMP::pathFindingResult(
    const std::string &current_map, const uint8_t &x, const uint8_t &y,
    const std::vector<std::pair<CatchChallenger::Orientation, uint8_t> >
        &path) {
  if (!path.empty()) {
    if (path.front().second == 0) {
      std::cerr
          << "MapControllerMP::pathFindingResult(): path.first().second==0"
          << std::endl;
      pathFindingNotFound();
      return;
    }
    MapVisualiserPlayer::pathFindingResultInternal(pathList, current_map, x, y,
                                                   path);
  } else
    pathFindingNotFound();
}
