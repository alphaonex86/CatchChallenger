...
void MapControllerMP::moveOtherPlayerStepSlot() {
  ...
  //    std::cout << "MapControllerMP::moveOtherPlayerStepSlot(): " <<
  //    std::to_string(otherPlayer.presumed_direction) << std::endl;
  moveOtherPlayerStepSlotWithPlayer(otherPlayerList[simplifiedId]);
}

void MapControllerMP::moveOtherPlayerStepSlotWithPlayer(
    OtherPlayer &otherPlayer) {
#ifdef DEBUG_CLIENT_OTHER_PLAYER_MOVE_STEP
  qDebug() << QStringLiteral(
                  "moveOtherPlayerStepSlot() player: %1 (%2), moveStep: %3")
  ...
  if (!otherPlayer.animationDisplayed) {
    ...
    if (all_map.find(map->map_file) != all_map.cend())
      if (all_map.at(map->map_file)
              ->doors.find(std::pair<uint8_t, uint8_t>(
                  static_cast<uint8_t>(x), static_cast<uint8_t>(y))) !=
          all_map.at(map->map_file)->doors.cend()) {
        if (otherPlayerListByAnimationTimer.find(
                otherPlayer.moveAnimationTimer) !=
            otherPlayerListByAnimationTimer.cend()) {
          MapDoor *door =
              all_map.at(map->map_file)
                  ->doors.at(std::pair<uint8_t, uint8_t>(
                      static_cast<uint8_t>(x), static_cast<uint8_t>(y)));
  ...
  if (otherPlayer.monsterMapObject != NULL) {
    ...
    if (otherPlayer.pendingMonsterMoves.size() > 2) abort();
#endif
      ...
      switch (otherPlayer.pendingMonsterMoves.front()) {
        case CatchChallenger::Direction_move_at_left:
          ...
          switch (otherPlayer.moveStep) {
            ...
            case 3:
            case 4:
              otherPlayer.monsterMapObject->setX(
                  otherPlayer.monsterMapObject->x() - 0.20);
              break;
          }
          break;
        case CatchChallenger::Direction_move_at_right:
        ...
        case CatchChallenger::Direction_move_at_top:
          ...
          switch (otherPlayer.moveStep) {
            ...
            case 3:
            case 4:
              otherPlayer.monsterMapObject->setY(
                  otherPlayer.monsterMapObject->y() - 0.20);
              break;
          }
          break;
        case CatchChallenger::Direction_move_at_bottom:
      ...
      switch (otherPlayer.moveStep % 5) {
        ...
        case 1:
          MapObjectItem::objectLink.at(otherPlayer.monsterMapObject)
              ->SetZValue(qCeil(otherPlayer.monsterMapObject->y()));
          break;
        // transition step
        case 2: {
          Tiled::Cell cell = otherPlayer.monsterMapObject->cell();
  ...
  switch (otherPlayer.presumed_direction) {
    case CatchChallenger::Direction_move_at_left:
      ...
      switch (otherPlayer.moveStep) {
        ...
        case 3:
        case 4:
          otherPlayer.playerMapObject->setX(otherPlayer.playerMapObject->x() -
                                            0.20);
          if (otherPlayer.labelMapObject != NULL)
            otherPlayer.labelMapObject->setX(otherPlayer.labelMapObject->x() -
                                             0.20);
          break;
      }
      break;
    ...
    case CatchChallenger::Direction_move_at_top:
      ...
      switch (otherPlayer.moveStep) {
        ...
        case 3:
        case 4:
          otherPlayer.playerMapObject->setY(otherPlayer.playerMapObject->y() -
                                            0.20);
          if (otherPlayer.labelMapObject != NULL)
            otherPlayer.labelMapObject->setY(otherPlayer.labelMapObject->y() -
                                             0.20);
          break;
      }
      break;
    ...
    default:
      qDebug()
          << QStringLiteral(
                 "moveOtherPlayerStepSlot(): moveStep: %1, wrong direction")
                 .arg(otherPlayer.moveStep);
    ...
    case 1:
      if (MapObjectItem::objectLink.find(otherPlayer.playerMapObject) !=
        ...
        MapObjectItem::objectLink.at(otherPlayer.playerMapObject)
            ->SetZValue(qCeil(otherPlayer.playerMapObject->y()));
      break;
    // transition step
    case 2: {
      Tiled::Cell cell = otherPlayer.playerMapObject->cell();
  ...
  if (otherPlayer.moveStep > 5) {
    if (otherPlayer.pendingMonsterMoves.size() > 1) {
      ...
      if (old_map != map) {
        ...
        if (old_all_map.find(otherPlayer.current_monster_map) ==
            old_all_map.cend())
    ...
    switch (otherPlayer.presumed_direction) {
      ...
      default:
        qDebug() << QStringLiteral(
                        "moveStepSlot(): moveStep: %1, wrong direction when "
                        "moveStep>2")
                        .arg(otherPlayer.moveStep);
        return;
    }
    ...
    if (otherPlayer.presumed_direction ==
      ...
      if (!CatchChallenger::MoveOnTheMap::canGoTo(
              CatchChallenger::Direction_move_at_left,
              otherPlayer.presumed_map->logicalMap, x, y, true)) {
        otherPlayer.presumed_direction =
            CatchChallenger::Direction_look_at_left;
        Tiled::Cell cell = otherPlayer.playerMapObject->cell();
...
#ifdef CATCHCHALLENGER_EXTRA_CHECK
        if (otherPlayer.pendingMonsterMoves.size() > 1) abort();
#endif
        finalOtherPlayerStep(otherPlayer);
        moveOtherPlayerStepSlotWithPlayer(otherPlayer);
      }
    } else if (otherPlayer.presumed_direction ==
