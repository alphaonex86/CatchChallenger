...
std::string MapController::text_DATAPACK_BASE_PATH_SKIN =
...
MapController::MapController(const bool &debugTags)
  ...
  if (!connect(this, &MapController::mapDisplayed, this,
               &MapController::tryLoadPlantOnMapDisplayed,
               Qt::QueuedConnection))
    abort();
  botFlags = NULL;
  ...
  if (!connect(&updateColorTimer, &QTimer::timeout, this,
               &MapController::updateColor))
    abort();
  updateBotTimer.setInterval(1000);
  updateBotTimer.start();
  if (!connect(&updateBotTimer, &QTimer::timeout, this,
               &MapController::updateBot))
    abort();
}
...
bool MapController::asyncMapLoaded(const std::string &fileName,
  ...
  if (MapControllerMP::asyncMapLoaded(fileName, tempMapObject)) {
    {
      if (QtDatapackClientLoader::GetInstance()->get_plantOnMap().find(fileName) !=
        ...
        const std::unordered_map<std::pair<uint8_t, uint8_t>, uint16_t,
                                 pairhash> &plantCoor =
            QtDatapackClientLoader::GetInstance()->get_plantOnMap().at(fileName);
        for (const auto &n : plantCoor) {
          const uint16_t indexOfMap = n.second;
          if (player_informations.plantOnMap.size() > 1000000) abort();
          if (player_informations.plantOnMap.find(indexOfMap) !=
              player_informations.plantOnMap.cend()) {
            const uint8_t &x = n.first.first;
            const uint8_t &y = n.first.second;
            const CatchChallenger::PlayerPlant &playerPlant =
                player_informations.plantOnMap.at(indexOfMap);
            uint32_t seconds_to_mature = 0;
            if (playerPlant.mature_at >
            ...
            if (QtDatapackClientLoader::GetInstance()->get_fullMapPathToId().find(
              ...
              qDebug() << "!QtDatapackClientLoader::GetInstance()->"
                          "fullMapPathToId.contains(plantIndexContent.map) for "
                          "CommonSettingsServer::commonSettingsServer."
                          "plantOnlyVisibleByPlayer";
          }
        }
      }
    }
...
void MapController::updateBot() {
  if (!player_informations_is_set) return;
  if (current_map.empty()) return;
  if (all_map.find(current_map) == all_map.cend()) return;
  ...
  for (const auto &n : currentMap->logicalMap.botsDisplay) {
    CatchChallenger::BotDisplay &botDisplay =
        currentMap->logicalMap.botsDisplay[n.first];
    if (botDisplay.mapObject == getPlayerMapObject()) continue;
    if (botDisplay.tileset != NULL && botDisplay.mapObject != NULL)
      switch (botDisplay.botMove) {
        ...
        case CatchChallenger::BotMove::BotMove_Loop: {
          uint8_t baseTile =
              static_cast<uint8_t>(botDisplay.mapObject->cell().tile->id());
          switch (baseTile) {
            default:
            case 7:
              baseTile = 10;  // left
...
void MapController::connectAllSignals(
  ...
  if (!connect(client, &CatchChallenger::Api_client_real::Qtinsert_plant, this,
               &MapController::insert_plant, Qt::UniqueConnection))
    abort();
  if (!connect(client, &CatchChallenger::Api_client_real::Qtremove_plant, this,
               &MapController::remove_plant, Qt::UniqueConnection))
    abort();
  if (!connect(client, &CatchChallenger::Api_client_real::Qtseed_planted, this,
               &MapController::seed_planted, Qt::UniqueConnection))
    abort();
  if (!connect(client, &CatchChallenger::Api_client_real::Qtplant_collected,
               this, &MapController::plant_collected, Qt::UniqueConnection))
    abort();
}
...
void MapController::datapackParsed() { MapControllerMP::datapackParsed(); }
...
void MapController::datapackParsedMainSub() {
  ...
  while (index < delayedPlantInsert.size()) {
    insert_plant(delayedPlantInsert.at(index).mapId,
                 delayedPlantInsert.at(index).x, delayedPlantInsert.at(index).y,
                 delayedPlantInsert.at(index).plant_id,
...
bool MapController::canGoTo(const CatchChallenger::Direction &direction,
                            CatchChallenger::CommonMap map, COORD_TYPE x,
                            COORD_TYPE y, const bool &checkCollision) {
  if (!MapVisualiserPlayerWithFight::canGoTo(direction, map, x, y,
                                             checkCollision))
    return false;
  ...
  if (all_map.at(new_map->map_file)
          ->logicalMap.bots.find(std::pair<uint8_t, uint8_t>(
              static_cast<uint8_t>(x), static_cast<uint8_t>(y))) !=
      all_map.at(new_map->map_file)->logicalMap.bots.cend())
    return false;
  return true;
}

void MapController::loadBotOnTheMap(Map_full *parsedMap, const uint32_t &botId,
  ...
  if (parsedMap->logicalMap.botsDisplay.find(std::pair<uint8_t, uint8_t>(
          static_cast<uint8_t>(x), static_cast<uint8_t>(y))) !=
      parsedMap->logicalMap.botsDisplay.cend()) {
    /*CatchChallenger::BotDisplay
    *botDisplay=&parsedMap->logicalMap.botsDisplay[std::pair<uint8_t,uint8_t>(static_cast<uint8_t>(x),static_cast<uint8_t>(y))];
    ...
    1, not 0 botDisplay->mapObject->setPosition(QPoint(x,y+1));
    MapObjectItem::objectLink.value(botDisplay->mapObject)->setZValue(y);*/

    std::cerr << "MapController::loadBotOnTheMap() "
                 "parsedMap->logicalMap.botsDisplay.contains(std::pair<uint8_t,"
                 "uint8_t>(static_cast<uint8_t>(x),static_cast<uint8_t>(y)))"
              << std::endl;
    return;
  }

  if (ObjectGroupItem::objectGroupLink.find(parsedMap->objectGroup) ==
      ObjectGroupItem::objectGroupLink.cend()) {
    qDebug() << QStringLiteral(
        "loadBotOnTheMap(), ObjectGroupItem::objectGroupLink not contains "
        "parsedMap->objectGroup");
    return;
  }
  CatchChallenger::BotDisplay *botDisplay =
      &parsedMap->logicalMap.botsDisplay[std::pair<uint8_t, uint8_t>(
          static_cast<uint8_t>(x), static_cast<uint8_t>(y))];
  botDisplay->botMove = CatchChallenger::BotMove::BotMove_Fixed;
  CatchChallenger::Direction direction;
  int baseTile = -1;
  if (lookAt == MapController::text_random ||
      lookAt == MapController::text_loop ||
      lookAt == MapController::text_move) {
    switch (rand() % 4) {
      default:
      case 0:
        baseTile = 10;
    ...
    if (lookAt != MapController::text_bottom)
      qDebug() << (QStringLiteral("Wrong direction for the bot at %1 (%2,%3)")
  ...
  std::string skinPath =
      datapackPath + MapController::text_DATAPACK_BASE_PATH_SKIN +
      MapController::text_slash + skin + MapController::text_slashtrainerpng;
  if (!QFile(QString::fromStdString(skinPath)).exists()) {
    QDir folderList(
        QStringLiteral("%1/skin/").arg(QString::fromStdString(datapackPath)));
    ...
    while (entryListIndex < entryList.size()) {
      skinPath = datapackPath + DATAPACK_BASE_PATH_SKINBASE +
                 entryList.at(entryListIndex).toStdString() +
                 MapController::text_slash + skin +
                 MapController::text_slashtrainerpng;
      if (QFile(QString::fromStdString(skinPath)).exists()) break;
      entryListIndex++;
    }
  }
  // do a cache here?
  if (!QFile(QString::fromStdString(skinPath)).exists()) {
    qDebug() << "Unable the load the bot tileset (not found):"
             << QString::fromStdString(skinPath);
    if (!botDisplay->tileset->loadFromImage(
  ...
  } else if (!botDisplay->tileset->loadFromImage(
                 QImage(QString::fromStdString(skinPath)),
                 QString::fromStdString(skinPath))) {
...

  {
    ...
    ObjectGroupItem::objectGroupLink.at(parsedMap->objectGroup)
        ->addObject(botDisplay->mapObject);
    // move to the final position (integer), y+1 because the tile lib start y to
    // 1, not 0
  ...
  std::pair<uint8_t, uint8_t> Qtpos(x, y);
  {
    ...
    if (botFlags == NULL) {
      ...
      botFlags->loadFromImage(QImage(QStringLiteral(":/CC/images/flags.png")),
                              QStringLiteral(":/CC/images/flags.png"));
    ...
    // fight collision bot
    {
      ...
      ObjectGroupItem::objectGroupLink.at(parsedMap->objectGroup)
          ->addObject(flag);
      // move to the final position (integer), y+1 because the tile lib start y
      // to 1, not 0
      flag->setPosition(QPointF(x, y - 1.5));
      MapObjectItem::objectLink.at(flag)->SetZValue(9999);
    }
    if (parsedMap->logicalMap.shops.find(pos) !=
        parsedMap->logicalMap.shops.cend()) {
      Tiled::MapObject *flag = new Tiled::MapObject();
      flag->setName("Shops");
    ...
    if (parsedMap->logicalMap.learn.find(pos) !=
    ...
    if (parsedMap->logicalMap.heal.find(pos) !=
    ...
    if (parsedMap->logicalMap.market.find(pos) !=
    ...
    if (parsedMap->logicalMap.zonecapture.find(pos) !=
    ...
    /* asked by tgjklmda
    if(parsedMap->logicalMap.botsFight.find(pos)!=parsedMap->logicalMap.botsFight.cend())
    {
        Tiled::MapObject * flag=new Tiled::MapObject();
  ...
  if (parsedMap->logicalMap.bots.at(Qtpos).step.find(1) !=
      parsedMap->logicalMap.bots.at(Qtpos).step.cend()) {
    auto stepBot = parsedMap->logicalMap.bots.at(Qtpos).step.at(1);
    if (stepBot->Attribute("type") != NULL &&
        strcmp(stepBot->Attribute("type"), "fight") == 0) {
      if (stepBot->Attribute("fightid") != NULL) {
        bool ok;
        // 16Bit: \see CommonDatapackServerSpec, Map_to_send,struct
        // Bot_Semi,uint16_t id
        ...
        if (ok) {
          if (CatchChallenger::CommonDatapackServerSpec::
            ...
            qDebug()
                << (QStringLiteral(
                        "Put bot fight point %1 at %2 (%3,%4) in direction: %5")
                        .arg(fightid)
            ...
            if (parsedMap->logicalMap.bots.at(Qtpos).properties.find(
                    "fightRange") !=
                parsedMap->logicalMap.bots.at(Qtpos).properties.cend()) {
              fightRange = stringtouint32(
                  parsedMap->logicalMap.bots.at(Qtpos).properties.at(
                      "fightRange"),
                  &ok);
              if (!ok) {
                qDebug() << (QStringLiteral(
                                 "fightRange is not a number at %1 (%2,%3): %4")
                                 .arg(QString::fromStdString(
                                     parsedMap->logicalMap.map_file))
                                 .arg(x)
                                 .arg(y)
                                 .arg(QString::fromStdString(
                                     parsedMap->logicalMap.bots.at(Qtpos)
                                         .properties.at("fightRange"))));
                fightRange = 5;
              } else {
                if (fightRange > 10) {
                  qDebug()
                      << (QStringLiteral(
                              "fightRange is greater than 10 at %1 (%2,%3): %4")
                              .arg(QString::fromStdString(
                                  parsedMap->logicalMap.map_file))
                              .arg(x)
                              .arg(y)
            ...
            while (index_botfight_range < fightRange) {
              if (!CatchChallenger::MoveOnTheMap::canGoTo(
                      direction, *map, temp_x, temp_y, true, false))
                break;
              if (!CatchChallenger::MoveOnTheMap::move(direction, &map, &temp_x,
                                                       &temp_y, true, false))
                break;
              if (map != old_map) break;
              std::pair<uint8_t, uint8_t> temp_pos(temp_x, temp_y);
              std::pair<uint8_t, uint8_t> Qttemp_pos(temp_x, temp_y);
              parsedMap->logicalMap.botsFightTrigger[temp_pos].push_back(
                  fightid);
              parsedMap->logicalMap.botsFightTriggerExtra[Qttemp_pos].push_back(
                  Qtpos);
              index_botfight_range++;
            }
          } else
            qDebug() << QStringLiteral(
                            "No fightid %1 at MapController::loadBotOnTheMap")
                            .arg(fightid);
        } else
          qDebug() << QStringLiteral("fightid not a number: ")
                   << stepBot->Attribute("fightid");
      } else
        qDebug() << QStringLiteral(
                        "stepBot->Attribute(std::string(\"type\"))!=NULL && "
                        "*stepBot->Attribute(std::string(\"type\"))==\"fight\" "
                        "&& stepBot->Attribute(std::string(\"fightid\"))!=NULL")
                 << " at "
                 << QString::fromStdString(parsedMap->logicalMap.map_file) << ""
                 << Qtpos.first << "," << Qtpos.second
  ...
  /*else
      qDebug() <<
     QStringLiteral("parsedMap->logicalMap.bots.value(Qtpos).step.find(1)!=parsedMap->logicalMap.bots.value(Qtpos).step.cend()");*/
}
...
void MapController::setColor(const QColor &color, const uint32_t &timeInMS) {
  ...
  if (timeInMS < 50) {
    ...
    if (newColor.alpha() == 0)
      newColor =
          QColor(tempColor.red(), tempColor.green(), tempColor.blue(), 0);
...
void MapController::updateColor() {
  ...
  tempColor = QColor(tempColor.red() + rdiff, tempColor.green() + gdiff,
                     tempColor.blue() + bdiff, tempColor.alpha() + adiff);
  /*QPixmap pixmap(width()*4,height()*4);
