...
To not send: store "is blocked but direction not send", cautch the close event,
at close: if "is blocked but direction not send" then send it
*/
...
MapVisualiserPlayer::MapVisualiserPlayer(const bool &debugTags)
  ...
  if (!connect(&lookToMove, &QTimer::timeout, this,
               &MapVisualiserPlayer::transformLookToMove))
    abort();
  if (!connect(this, &MapVisualiserPlayer::mapDisplayed, this,
  ...
  if (!connect(&moveTimer, &QTimer::timeout, this,
               &MapVisualiserPlayer::moveStepSlot))
    abort();
  moveAnimationTimer.setInterval(currentPlayerSpeed / 5);
  moveAnimationTimer.setSingleShot(true);
  if (!connect(&moveAnimationTimer, &QTimer::timeout, this,
               &MapVisualiserPlayer::doMoveAnimation))
    abort();
...
MapVisualiserPlayer::~MapVisualiserPlayer() {
  if (animationTileset != NULL) delete animationTileset;
  if (nextCurrentObject != NULL) delete nextCurrentObject;
  if (grassCurrentObject != NULL) delete grassCurrentObject;
  if (playerMapObject != NULL) delete playerMapObject;
...
bool MapVisualiserPlayer::haveMapInMemory(const std::string &mapPath) {
  return all_map.find(mapPath) != all_map.cend() ||
         old_all_map.find(mapPath) != old_all_map.cend();
}
...
void MapVisualiserPlayer::keyPressReset() { keyPressed.clear(); }
...
void MapVisualiserPlayer::keyPressParse() {
  ...
  if (inMove) {
    if (!moveTimer.isActive()) {
      std::cerr
          << "!moveTimer.isActive() && inMove==true, reset, internal error"
          << std::endl;
      return;
    } else
      return;
  ...
  if (keyPressed.size() == 1 &&
      keyPressed.find(Qt::Key_Return) != keyPressed.cend()) {
  ...
  if (keyPressed.find(Qt::Key_Left) != keyPressed.cend()) {
    ...
    if (direction == CatchChallenger::Direction_look_at_left) {
      if (!canGoTo(CatchChallenger::Direction_move_at_left,
                   all_map.at(current_map)->logicalMap, x, y, true))
    ...
    if (direction == CatchChallenger::Direction_look_at_right) {
      if (!canGoTo(CatchChallenger::Direction_move_at_right,
                   all_map.at(current_map)->logicalMap, x, y, true))
    ...
    if (direction == CatchChallenger::Direction_look_at_top) {
      if (!canGoTo(CatchChallenger::Direction_move_at_top,
                   all_map.at(current_map)->logicalMap, x, y, true))
    ...
    if (direction == CatchChallenger::Direction_look_at_bottom) {
      if (!canGoTo(CatchChallenger::Direction_move_at_bottom,
                   all_map.at(current_map)->logicalMap, x, y, true))
...
void MapVisualiserPlayer::doMoveAnimation() { moveStepSlot(); }

void MapVisualiserPlayer::moveStepSlot() {
  Map_full *map_full = all_map.at(current_map);
  if (!animationDisplayed) {
    // leave
    if (map_full->triggerAnimations.find(std::pair<uint8_t, uint8_t>(
            static_cast<uint8_t>(x), static_cast<uint8_t>(y))) !=
        map_full->triggerAnimations.cend()) {
      TriggerAnimation *triggerAnimation =
          map_full->triggerAnimations.at(std::pair<uint8_t, uint8_t>(
              static_cast<uint8_t>(x), static_cast<uint8_t>(y)));
    ...
    // door
    if (map_full->doors.find(std::pair<uint8_t, uint8_t>(
            static_cast<uint8_t>(x), static_cast<uint8_t>(y))) !=
        map_full->doors.cend()) {
      MapDoor *door = map_full->doors.at(std::pair<uint8_t, uint8_t>(
          static_cast<uint8_t>(x), static_cast<uint8_t>(y)));
      ...
      switch (direction) {
        ...
        default:
          qDebug() << QStringLiteral(
                          "moveStepSlot(): moveStep: %1, wrong direction")
                          .arg(moveStep);
          return;
      }
      Tiled::Cell cell = playerMapObject->cell();
  ...
  if (monsterMapObject != NULL) {
    if (inMove && moveStep == 1) switch (direction) {
        case CatchChallenger::Direction_move_at_left:
        case CatchChallenger::Direction_move_at_right:
        case CatchChallenger::Direction_move_at_top:
        case CatchChallenger::Direction_move_at_bottom:
...
#ifdef CATCHCHALLENGER_EXTRA_CHECK
    if (pendingMonsterMoves.size() > 2) abort();
#endif
    if (pendingMonsterMoves.size() > 1) {
      // start move
      // moveTimer.stop();
      int baseTile = 1;
      // move the player for intermediate step and define the base tile (define
      // the stopped step with direction)
      ...
      // apply the right step of the base step defined previously by the
      // direction
      switch (moveStep % 5) {
        // stopped step
        case 0: {
        ...
        case 1:
          MapObjectItem::objectLink.at(monsterMapObject)
              ->SetZValue(qCeil(monsterMapObject->y()));
          break;
        // transition step
        case 2: {
          Tiled::Cell cell = monsterMapObject->cell();
  ...
  // moveTimer.stop();
  int baseTile = 1;
  // move the player for intermediate step and define the base tile (define the
  // stopped step with direction)
  switch (direction) {
    case CatchChallenger::Direction_move_at_left:
      baseTile = 10;
      switch (moveStep) {
  ...
  switch (moveStep % 5) {
    ...
    case 1:
      MapObjectItem::objectLink.at(playerMapObject)
          ->SetZValue(qCeil(playerMapObject->y()));
      break;
    // transition step
    case 2: {
      Tiled::Cell cell = playerMapObject->cell();
  ...
  if (moveStep > 5) {
    if (monsterMapObject != NULL)
      if (pendingMonsterMoves.size() > 1) {
        ...
        switch (direction) {
          ...
          case CatchChallenger::Direction_move_at_top:
          case CatchChallenger::Direction_move_at_bottom:
            if (!CatchChallenger::MoveOnTheMap::move(direction, &map,
                                                     &monster_x, &monster_y)) {
              std::cerr << "Bug at move for pendingMonsterMoves, unknown move: "
                        << std::to_string(direction) << " from "
                        << map->map_file << " (" << std::to_string(monster_x)
                        << "," << std::to_string(monster_y) << ")" << std::endl;
              resetMonsterTile();
            }
            break;
          default:
        ...
        if (old_map != map) {
          ...
          if (old_all_map.find(current_monster_map) == old_all_map.cend())
            std::cerr << "old_all_map.find(current_map)==old_all_map.cend() in "
                         "monster follow"
                      << std::endl;
          if (!vectorcontainsAtLeastOne(
                  static_cast<const CatchChallenger::Map_client *>(old_map)
                      ->near_map,
                  map))
            resetMonsterTile();
    ...
    switch (direction) {
      ...
      case CatchChallenger::Direction_move_at_bottom:
        if (!CatchChallenger::MoveOnTheMap::move(direction, &map, &x, &y)) {
          std::cerr << "Bug at move, unknown move: "
                    << std::to_string(direction) << " from " << map->map_file
                    << " (" << std::to_string(x) << "," << std::to_string(y)
                    << ")" << std::endl;
          return;
        }
        direction =
            CatchChallenger::MoveOnTheMap::directionToDirectionLook(direction);
        break;
      default:
        qDebug() << QStringLiteral(
                        "moveStepSlot(): moveStep: %1, wrong direction (%2) "
                        "when moveStep>2")
                        .arg(moveStep)
                        .arg(direction);
        return;
    ...
    if (old_map != map) {
      ...
      if (!blocked) finalPlayerStep();
    }
  } else
    moveTimer.start();
}

bool MapVisualiserPlayer::asyncMapLoaded(const std::string &fileName,
                                         Map_full *tempMapObject) {
  const CatchChallenger::Player_private_and_public_informations
      &player_informations = client->get_player_informations_ro();
  ...
  if (MapVisualiser::asyncMapLoaded(fileName, tempMapObject)) {
    if (tempMapObject != NULL) {
      ...
      while (index < tempMapObject->tiledMap->layerCount()) {
        if (Tiled::ObjectGroup *objectGroup =
                tempMapObject->tiledMap->layerAt(index)->asObjectGroup()) {
          if (objectGroup->name() == "Object") {
            ...
            while (index2 < objects.size()) {
              ...
              if (object->type() == "object") {
                ...
                if (tempMapObject->logicalMap.itemsOnMap.find(
                        std::pair<uint8_t, uint8_t>(static_cast<uint8_t>(x),
                                                    static_cast<uint8_t>(y))) !=
                    tempMapObject->logicalMap.itemsOnMap.cend()) {
                  if (object->property("visible") == "false") {
                    // The tiled object not exist on this layer
                    if (ObjectGroupItem::objectGroupLink.find(objectGroup) !=
                        ObjectGroupItem::objectGroupLink.cend()) {
                      ObjectGroupItem::objectGroupLink.at(objectGroup)
                          ->removeObject(object);
                      tempMapObject->logicalMap
                          .itemsOnMap[std::pair<uint8_t, uint8_t>(
                              static_cast<uint8_t>(x), static_cast<uint8_t>(y))]
                          .tileObject = NULL;
                    } else
                      std::cerr
                          << "Try removeObject(object) on not existant "
                             "ObjectGroupItem::objectGroupLink.at(objectGroup)"
                          << std::endl;
                    ...
                    if (QtDatapackClientLoader::datapackLoader->itemOnMap.find(
                      ...
                      const std::unordered_map<std::pair<uint8_t, uint8_t>,
                          ...
                          QtDatapackClientLoader::datapackLoader->itemOnMap.at(
                              tempMap);
                      if (tempIndexItem.find(std::pair<uint8_t, uint8_t>(
                        ...
                        const uint16_t &itemIndex =
                            tempIndexItem.at(std::pair<uint8_t, uint8_t>(
                                static_cast<uint8_t>(x),
                                static_cast<uint8_t>(y)));
                        if (player_informations.itemOnMap.find(itemIndex) !=
                            player_informations.itemOnMap.cend()) {
                          if (ObjectGroupItem::objectGroupLink.find(
                                  objectGroup) !=
                              ObjectGroupItem::objectGroupLink.cend()) {
                            ObjectGroupItem *objectGroupItem =
                                ObjectGroupItem::objectGroupLink.at(
                                    objectGroup);
                            objectGroupItem->removeObject(object);
                            objects.removeAt(index2);
                          } else {
                            std::cerr << "ObjectGroupItem::objectGroupLink."
                                         "find(objectGroup)!=ObjectGroupItem::"
                                         "objectGroupLink.cend(), map mixed?"
                                      << std::endl;
                            index2++;
                          }
                        } else {
...
void MapVisualiserPlayer::unblock() { blocked = false; }
...
bool MapVisualiserPlayer::finalPlayerStepTeleported(bool &isTeleported) {
  ...
  if (current_map_pointer == NULL) {
    qDebug() << "current map not loaded null pointer, unable to do "
                "finalPlayerStep()";
    return false;
  }
  if (!isTeleported) {
    ...
    while (index < size) {
      const CatchChallenger::Map_semi_teleport &current_teleport =
          current_map_pointer->logicalMap.teleport_semi.at(index);
      ...
      if (current_teleport.source_x == x && current_teleport.source_y == y) {
        ...
        if (!haveMapInMemory(current_map)) emit inWaitingOfMap();
        loadOtherMap(current_map);
        hideNotloadedMap();
        resetMonsterTile();
        return true;
...
void MapVisualiserPlayer::finalPlayerStep(bool parseKey) {
  ...
  /// botFightRemotePointList=all_map.value(current_map)->logicalMap.botsFightTriggerExtra.values(std::pair<uint8_t,uint8_t>(static_cast<uint8_t>(x),static_cast<uint8_t>(y)));
  if (!CatchChallenger::CommonDatapack::commonDatapack.monstersCollision
           .empty()) {
    // locate the right layer for monster
    if (monsterMapObject != NULL) {
      const Map_full *current_monster_map_pointer =
          all_map.at(current_monster_map);
      if (current_monster_map_pointer == NULL) {
        qDebug() << "current_monster_map_pointer not loaded null pointer, "
                    "unable to do finalPlayerStep()";
        return;
      }
      {
        const CatchChallenger::MonstersCollisionValue &monstersCollisionValue =
            CatchChallenger::MoveOnTheMap::getZoneCollision(
                current_monster_map_pointer->logicalMap, monster_x, monster_y);
        ...
        while (index < monstersCollisionValue.walkOn.size()) {
          ...
          if (newIndex < CatchChallenger::CommonDatapack::commonDatapack
            ...
            if (monstersCollision.item == 0 ||
              ...
              monsterMapObject->setVisible(
                  (monstersCollisionTemp.tile.empty() &&
                   pendingMonsterMoves.size() >= 1) ||
      ...
      if (newIndex < CatchChallenger::CommonDatapack::commonDatapack
        ...
        if (monstersCollision.item == 0 ||
          ...
          if (monstersCollisionTemp.tile != lastTileset) {
            ...
            if (playerTilesetCache.find(lastTileset) !=
                playerTilesetCache.cend())
              ...
              if (lastTileset.empty())
                ...
                const std::string &imagePath =
                    playerSkinPath + "/" + lastTileset + ".png";
                QImage image(QString::fromStdString(imagePath));
                if (!image.isNull()) {
                  playerTileset = new Tiled::Tileset(
                      QString::fromStdString(lastTileset), 16, 24);
                  playerTileset->loadFromImage(
                      image, QString::fromStdString(imagePath));
                } else {
  ...
  if (haveStopTileAction()) return;

  if (CatchChallenger::MoveOnTheMap::getLedge(current_map_pointer->logicalMap,
                                              x, y) !=
      CatchChallenger::ParsedLayerLedges_NoLedges) {
    switch (direction) {
      ...
      default:
        qDebug() << QStringLiteral(
                        "moveStepSlot(): direction: %1, wrong direction")
                        .arg(direction);
        return;
    }
    moveStep = 0;
  ...
  if (keyPressed.find(Qt::Key_Left) != keyPressed.cend()) {
    ...
    if (!canGoTo(CatchChallenger::Direction_move_at_left,
                 current_map_pointer->logicalMap, x, y, true)) {
    ...
    // if can go, then do the move
      ...
      moveStepSlot();
      emit send_player_direction(direction);
      if (CommonSettingsServer::commonSettingsServer
              .forceClientToSendAtMapChange &&
          x == 0)
    ...
    // if can go, then do the move
      ...
      if (CommonSettingsServer::commonSettingsServer
              .forceClientToSendAtMapChange &&
          x == (current_map_pointer->logicalMap.width - 1))
    ...
    // if can go, then do the move
      ...
      if (CommonSettingsServer::commonSettingsServer
              .forceClientToSendAtMapChange &&
          y == (current_map_pointer->logicalMap.height - 1))
  ...
  // now stop walking, no more arrow key is pressed
    ...
    if (!nextPathStep()) {
      ...
      if (wasPathFindingUsed) {
        if (keyPressed.empty()) parseAction();
        wasPathFindingUsed = false;
      }
    }
  }
...
bool MapVisualiserPlayer::haveStopTileAction() { return false; }
    ...
    case CatchChallenger::Direction_look_at_left:
      if (CatchChallenger::MoveOnTheMap::canGoTo(
        ...
        if (!CatchChallenger::MoveOnTheMap::move(
                CatchChallenger::Direction_move_at_left, &map, &x, &y, false))
          qDebug() << QStringLiteral(
                          "can't go at left at map %1 (%2,%3) when move have "
                          "been checked")
                          .arg(QString::fromStdString(map->map_file))
                          .arg(x)
                          .arg(y);
        else
          emit stopped_in_front_of(
              static_cast<CatchChallenger::Map_client *>(map), x, y);
      }
      break;
    case CatchChallenger::Direction_look_at_right:
      if (CatchChallenger::MoveOnTheMap::canGoTo(
        ...
        if (!CatchChallenger::MoveOnTheMap::move(
                CatchChallenger::Direction_move_at_right, &map, &x, &y, false))
          qDebug() << QStringLiteral(
                          "can't go at right at map %1 (%2,%3) when move have "
                          "been checked")
                          .arg(QString::fromStdString(map->map_file))
                          .arg(x)
                          .arg(y);
    ...
    case CatchChallenger::Direction_look_at_top:
      if (CatchChallenger::MoveOnTheMap::canGoTo(
        ...
        if (!CatchChallenger::MoveOnTheMap::move(
                CatchChallenger::Direction_move_at_top, &map, &x, &y, false))
          qDebug() << QStringLiteral(
                          "can't go at top at map %1 (%2,%3) when move have "
                          "been checked")
                          .arg(QString::fromStdString(map->map_file))
                          .arg(x)
                          .arg(y);
    ...
    case CatchChallenger::Direction_look_at_bottom:
      if (CatchChallenger::MoveOnTheMap::canGoTo(
        ...
        if (!CatchChallenger::MoveOnTheMap::move(
                CatchChallenger::Direction_move_at_bottom, &map, &x, &y, false))
          qDebug() << QStringLiteral(
                          "can't go at bottom at map %1 (%2,%3) when move have "
                          "been checked")
                          .arg(QString::fromStdString(map->map_file))
                          .arg(x)
                          .arg(y);
    ...
    case CatchChallenger::Direction_look_at_left:
      if (CatchChallenger::MoveOnTheMap::canGoTo(
        ...
        if (!CatchChallenger::MoveOnTheMap::move(
          ...
          CatchChallenger::Map_client *map_client =
              static_cast<CatchChallenger::Map_client *>(map);
          if (map_client->botsDisplay.find(std::pair<uint8_t, uint8_t>(
                  static_cast<uint8_t>(x), static_cast<uint8_t>(y))) !=
              map_client->botsDisplay.cend()) {
            CatchChallenger::BotDisplay *botDisplay =
                &map_client->botsDisplay[std::pair<uint8_t, uint8_t>(
                    static_cast<uint8_t>(x), static_cast<uint8_t>(y))];
            Tiled::Cell cell = botDisplay->mapObject->cell();
            cell.tile = botDisplay->tileset->tileAt(4);
            botDisplay->mapObject->setCell(cell);
          } else if (map_client->itemsOnMap.find(std::pair<uint8_t, uint8_t>(
                         static_cast<uint8_t>(x), static_cast<uint8_t>(y))) !=
                     map_client->itemsOnMap.cend()) {
            const CatchChallenger::Map_client::ItemOnMapForClient &item =
                map_client->itemsOnMap.at(std::pair<uint8_t, uint8_t>(
                    static_cast<uint8_t>(x), static_cast<uint8_t>(y)));
            if (item.tileObject != NULL && !item.infinite) {
              ObjectGroupItem::objectGroupLink[item.tileObject->objectGroup()]
                  ->removeObject(item.tileObject);
              map_client
                  ->itemsOnMap[std::pair<uint8_t, uint8_t>(
                      static_cast<uint8_t>(x), static_cast<uint8_t>(y))]
                  .tileObject = NULL;
...
void MapVisualiserPlayer::transformLookToMove() {
  ...
  switch (direction) {
    case CatchChallenger::Direction_look_at_left:
      if (keyPressed.find(Qt::Key_Left) != keyPressed.cend() &&
          canGoTo(CatchChallenger::Direction_move_at_left,
                  all_map.at(current_map)->logicalMap, x, y, true)) {
        direction = CatchChallenger::Direction_move_at_left;
        inMove = true;
        moveStep = 1;
    ...
    case CatchChallenger::Direction_look_at_right:
      if (keyPressed.find(Qt::Key_Right) != keyPressed.cend() &&
          canGoTo(CatchChallenger::Direction_move_at_right,
                  all_map.at(current_map)->logicalMap, x, y, true)) {
        direction = CatchChallenger::Direction_move_at_right;
        inMove = true;
        moveStep = 1;
    ...
    case CatchChallenger::Direction_look_at_top:
      if (keyPressed.find(Qt::Key_Up) != keyPressed.cend() &&
          canGoTo(CatchChallenger::Direction_move_at_top,
                  all_map.at(current_map)->logicalMap, x, y, true)) {
        direction = CatchChallenger::Direction_move_at_top;
        inMove = true;
        moveStep = 1;
    ...
    case CatchChallenger::Direction_look_at_bottom:
      if (keyPressed.find(Qt::Key_Down) != keyPressed.cend() &&
          canGoTo(CatchChallenger::Direction_move_at_bottom,
                  all_map.at(current_map)->logicalMap, x, y, true)) {
        direction = CatchChallenger::Direction_move_at_bottom;
        inMove = true;
        moveStep = 1;
...
void MapVisualiserPlayer::keyReleaseEvent(QKeyEvent *event) {
  ...
  // remove from the key list pressed if exists (can be dropped by onther event
  // as click on map)
...
std::string MapVisualiserPlayer::lastLocation() const { return mLastLocation; }

std::string MapVisualiserPlayer::currentMap() const { return current_map; }
...
bool MapVisualiserPlayer::currentMapIsLoaded() const {
  if (all_map.find(current_map) == all_map.cend()) return false;
  return true;
}

std::string MapVisualiserPlayer::currentMapType() const {
  if (all_map.find(current_map) == all_map.cend()) return std::string();
  ...
  if (mapFull->logicalMap.xmlRoot == NULL) return std::string();
  if (mapFull->logicalMap.xmlRoot->Attribute("type") != NULL)
    // if(!std::string(all_map.value(current_map)->logicalMap.xmlRoot->Attribute("type")->empty())
    // if empty return empty or empty?
...
std::string MapVisualiserPlayer::currentZone() const {
  ...
  if (mapFull->logicalMap.xmlRoot->Attribute("zone") != NULL)
    // if(!std::string(all_map.value(current_map)->logicalMap.xmlRoot->Attribute("zone")->empty())
    // if empty return empty or empty?
...
std::string MapVisualiserPlayer::currentBackgroundsound() const {
  ...
  if (mapFull->logicalMap.xmlRoot->Attribute("backgroundsound") != NULL)
    // if(!std::string(all_map.value(current_map)->logicalMap.xmlRoot->Attribute("backgroundsound")->empty())
    // if empty return empty or empty?
...
bool MapVisualiserPlayer::loadPlayerMap(const std::string &fileName,
...
bool MapVisualiserPlayer::insert_player_internal(
  ...
  if (!mHaveTheDatapack || !player_informations_is_set) {
    emit error(
        "MapVisualiserPlayer::insert_player_final(): !mHaveTheDatapack || "
  ...
  if (player.simplifiedId ==
    ...
    // ignore to improve the performance server because can reinsert all player
    // of map using the overall client list
    ...
    if (player.skinId < skinFolderList.size()) {
      playerSkinPath = datapackPath + DATAPACK_BASE_PATH_SKIN +
                       skinFolderList.at(player.skinId);
      const std::string &imagePath = playerSkinPath + "/trainer.png";
      QImage image(QString::fromStdString(imagePath));
      if (!image.isNull()) {
        if (!playerTileset->loadFromImage(image,
                                          QString::fromStdString(imagePath)))
          abort();
      } else {
        qDebug() << "Unable to load the player tilset: " +
                        QString::fromStdString(imagePath);
        playerTileset->loadFromImage(
            QImage(QStringLiteral(":/CC/images/player_default/trainer.png")),
            QStringLiteral(":/CC/images/player_default/trainer.png"));
      }
    } else {
      qDebug() << "The skin id: " + QString::number(player.skinId) +
                      ", into a list of: " +
    ...
    switch (direction) {
      ...
      case CatchChallenger::Direction_move_at_left:
        QMessageBox::critical(NULL,
                              ...
                              tr("The direction send by the server is wrong"));
        return true;
      default:
        break;
    }
...
void MapVisualiserPlayer::resetAll() {
  if (!playerTileset->loadFromImage(
          QImage(QStringLiteral(":/CC/images/player_default/trainer.png")),
          QStringLiteral(":/CC/images/player_default/trainer.png")))
  ...
  {
    std::unordered_set<Tiled::Tileset *> deletedTileset;
    for (auto iter = playerTilesetCache.begin();
         iter != playerTilesetCache.end(); ++iter) {
...
bool MapVisualiserPlayer::canGoTo(const CatchChallenger::Direction &direction,
                                  CatchChallenger::CommonMap map, uint8_t x,
                                  uint8_t y, const bool &checkCollision) {
  CatchChallenger::CommonMap *mapPointer = &map;
  CatchChallenger::ParsedLayerLedges ledge;
  do {
    if (!CatchChallenger::MoveOnTheMap::canGoTo(direction, *mapPointer, x, y,
                                                checkCollision && !clip))
      return false;
    if (!CatchChallenger::MoveOnTheMap::move(direction, &mapPointer, &x, &y,
                                             checkCollision && !clip))
      return false;
    CatchChallenger::Map_client *map_client =
        static_cast<CatchChallenger::Map_client *>(
            &all_map.at(map.map_file)->logicalMap);
    if (map_client->itemsOnMap.find(std::pair<uint8_t, uint8_t>(
            static_cast<uint8_t>(x), static_cast<uint8_t>(y))) !=
        map_client->itemsOnMap.cend()) {
      const CatchChallenger::Map_client::ItemOnMapForClient &item =
          map_client->itemsOnMap.at(std::pair<uint8_t, uint8_t>(
              static_cast<uint8_t>(x), static_cast<uint8_t>(y)));
      if (item.tileObject != NULL) return false;
    }
    ledge = CatchChallenger::MoveOnTheMap::getLedge(map, x, y);
    if (ledge == CatchChallenger::ParsedLayerLedges_NoLedges) return true;
    switch (direction) {
      case CatchChallenger::Direction_move_at_bottom:
        ...
        break;
      case CatchChallenger::Direction_move_at_top:
        if (ledge != CatchChallenger::ParsedLayerLedges_LedgesTop) return false;
        break;
      case CatchChallenger::Direction_move_at_left:
...
void MapVisualiserPlayer::loadPlayerFromCurrentMap() {
  if (all_map.find(current_map) == all_map.cend()) {
    qDebug() << QStringLiteral("all_map have not the current map: %1")
  ...
  }
  {
    ...
    if (ObjectGroupItem::objectGroupLink.find(objectGroup) !=
        ObjectGroupItem::objectGroupLink.cend())
      ObjectGroupItem::objectGroupLink.at(objectGroup)
          ->addObject(playerMapObject);
    else
      qDebug() << QStringLiteral(
          "loadPlayerFromCurrentMap(), ObjectGroupItem::objectGroupLink not "
          "contains current_map->objectGroup");
...
void MapVisualiserPlayer::loadMonsterFromCurrentMap() {
  if (monsterMapObject == nullptr) return;
  // monster
  if (all_map.find(current_monster_map) == all_map.cend()) {
    qDebug() << QStringLiteral("all_map have not the current map: %1")
                    .arg(QString::fromStdString(current_monster_map));
    return;
  }
  {
    ...
    if (currentGroup != NULL) {
      if (ObjectGroupItem::objectGroupLink.find(currentGroup) !=
          ObjectGroupItem::objectGroupLink.cend())
        ObjectGroupItem::objectGroupLink.at(currentGroup)
            ->removeObject(monsterMapObject);
      // currentGroup->removeObject(monsterMapObject);
      if (currentGroup != all_map.at(current_map)->objectGroup)
        qDebug() << QStringLiteral(
                        "loadPlayerFromCurrentMap(), the monsterMapObject "
                        "group is wrong: %1")
                        .arg(currentGroup->name());
    }
    if (ObjectGroupItem::objectGroupLink.find(
            all_map.at(current_map)->objectGroup) !=
        ObjectGroupItem::objectGroupLink.cend())
      ObjectGroupItem::objectGroupLink.at(all_map.at(current_map)->objectGroup)
          ->addObject(monsterMapObject);
    else
      qDebug() << QStringLiteral(
          "loadPlayerFromCurrentMap(), ObjectGroupItem::objectGroupLink not "
          "contains current_map->objectGroup");
...
void MapVisualiserPlayer::unloadPlayerFromCurrentMap() {
  if (monsterMapObject == nullptr) return;
  {
    ...
    if (ObjectGroupItem::objectGroupLink.find(playerMapObject->objectGroup()) !=
        ObjectGroupItem::objectGroupLink.cend())
      ObjectGroupItem::objectGroupLink.at(playerMapObject->objectGroup())
          ->removeObject(playerMapObject);
    else
      qDebug() << QStringLiteral(
          "unloadPlayerFromCurrentMap(), ObjectGroupItem::objectGroupLink not "
          "contains playerMapObject->objectGroup()");
  }
}
...
void MapVisualiserPlayer::unloadMonsterFromCurrentMap() {
  // monster
  {
    ...
    // unload the player sprite
    if (ObjectGroupItem::objectGroupLink.find(
            monsterMapObject->objectGroup()) !=
        ObjectGroupItem::objectGroupLink.cend())
      ObjectGroupItem::objectGroupLink.at(monsterMapObject->objectGroup())
          ->removeObject(monsterMapObject);
    else
      qDebug() << QStringLiteral(
          "unloadPlayerFromCurrentMap(), ObjectGroupItem::objectGroupLink not "
          "contains monsterMapObject->objectGroup()");
  }
}

/*void MapVisualiserPlayer::startGrassAnimation(const CatchChallenger::Direction
&direction)
{
    switch(direction)
    {
    ...
    if(!haveNextCurrentObject)
    {
        haveNextCurrentObject=false;
        CatchChallenger::Map *
map_destination=&all_map.value(current_map)->logicalMap; COORD_TYPE
x_destination=x; COORD_TYPE y_destination=y;
...
uint8_t MapVisualiserPlayer::getX() { return x; }

uint8_t MapVisualiserPlayer::getY() { return y; }
...
// the datapack
void MapVisualiserPlayer::setDatapackPath(const std::string &path,
  ...
  if (stringEndsWith(path, '/') || stringEndsWith(path, '\\'))
    ...
    datapackPath = path + "/";
  datapackMapPathBase = QFileInfo(QString::fromStdString(datapackPath) +
                                  DATAPACK_BASE_PATH_MAPBASE)
                            .absoluteFilePath()
                            .toStdString();
  if (!stringEndsWith(datapackMapPathBase, '/') &&
      !stringEndsWith(datapackMapPathBase, '\\'))
    datapackMapPathBase += "/";
  datapackMapPathSpec =
      QFileInfo(QString::fromStdString(datapackPath) +
                DATAPACK_BASE_PATH_MAPMAIN +
                QString::fromStdString(mainDatapackCode) + "/")
          .absoluteFilePath()
          .toStdString();
  if (!stringEndsWith(datapackMapPathSpec, '/') &&
      !stringEndsWith(datapackMapPathSpec, '\\'))
    datapackMapPathSpec += "/";
  mLastLocation.clear();
}
...
void MapVisualiserPlayer::updatePlayerMonsterTile(const uint16_t &monster) {
  ...
  const std::string &imagePath = datapackPath + DATAPACK_BASE_PATH_MONSTERS +
                                 std::to_string(monster) + "/overworld.png";
  if (monsterTilesetCache.find(imagePath) != monsterTilesetCache.cend())
    ...
    if (!image.isNull()) {
      monsterTileset =
          new Tiled::Tileset(QString::fromStdString(lastTileset), 32, 32);
      if (!monsterTileset->loadFromImage(image,
                                         QString::fromStdString(imagePath)))
        abort();
  ...
  if (resetMonster) loadMonsterFromCurrentMap();
  resetMonsterTile();
}

void MapVisualiserPlayer::setClip(const bool &clip) { this->clip = clip; }
...
bool MapVisualiserPlayer::isInMove() const { return inMove; }

CatchChallenger::Direction MapVisualiserPlayer::getDirection() const {
  return direction;
}

void MapVisualiserPlayer::stopMove() { inMove = false; }
...
bool MapVisualiserPlayer::teleportTo(
  ...
  if (monsterMapObject != nullptr) monsterMapObject->setVisible(false);
  pendingMonsterMoves.clear();
...
bool MapVisualiserPlayer::nextPathStepInternal(
    ...
    const CatchChallenger::Direction &direction)  // true if have step
{
  ...
  {
    if (!inMove) {
      std::cerr << "inMove=false into MapControllerMP::nextPathStep(), fixed"
                << std::endl;
      inMove = true;
    }
    if (canGoTo(direction, all_map.at(current_map)->logicalMap, x, y, true)) {
      ...
      if (CommonSettingsServer::commonSettingsServer
        ...
        if (direction == CatchChallenger::Direction_move_at_bottom) {
          if (y == (all_map.at(current_map)->logicalMap.height - 1))
            emit send_player_direction(
                CatchChallenger::Direction_look_at_bottom);
          ...
          if (x == (all_map.at(current_map)->logicalMap.width - 1))
            emit send_player_direction(
                CatchChallenger::Direction_look_at_right);
      ...
      switch (this->direction) {
        ...
        default:
          qDebug() << QStringLiteral(
                          "moveStepSlot(): nextPathStep: %1, wrong direction")
                          .arg(moveStep);
          pathList.clear();
          return false;
      }
...
void MapVisualiserPlayer::pathFindingResultInternal(
    ...
    const std::vector<std::pair<CatchChallenger::Orientation, uint8_t> >
        &path) {
  if (keyAccepted.empty() ||
      keyAccepted.find(Qt::Key_Return) != keyAccepted.cend()) {
    ...
    if (!inMove) {
      ...
      if (this->current_map == current_map && this->x == x && this->y == y) {
        std::cout
            << "this->current_map==current_map && this->x==x && this->y==y"
            << std::endl;
        if (pathList.size() > 1) pathList.pop_back();
        pathList.push_back(pathResolved);
        if (!nextPathStep()) {
          std::cout << "this->current_map==current_map && this->x==x && "
                       "this->y==y stopAndSend"
                    << std::endl;
          stopAndSend();
          parseStop();
        }
        ...
        std::cout << this->current_map << " " << std::to_string(this->x) << ","
                  << std::to_string(this->y) << " != " << current_map << " "
...
void MapVisualiserPlayer::forcePlayerTileset(QString path) {
  ...
  if (QFile::exists(externalFile)) {
    ...
    if (!externalImage.isNull() && externalImage.width() == 48 &&
        externalImage.height() == 96)
...
void MapVisualiserPlayer::centerOnPlayer() {
  /*    const float &z=CCMap::zoom();
      const std::unordered_set<QGraphicsItem *> &values =
     displayed_layer.at(map); for( const auto& value : values )
          value->setPos(static_cast<qreal>(static_cast<double>(x))*z,static_cast<qreal>(static_cast<double>(y))*z);*/
}

void MapVisualiserPlayer::Draw(QPainter *painter) {
  // TODO(lanstat): verificar si es nodo o scena
  ...
  if (MapObjectItem::objectLink.find(const_cast<Tiled::MapObject *>(
          playerMapObject)) != MapObjectItem::objectLink.cend()) {
    SetPos(1440 / 2 - px - pw / 2, 900 / 2 - py - ph / 2);
  }
}
