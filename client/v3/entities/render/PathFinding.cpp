...
PathFinding::PathFinding() : tryCancel(false) {
  setObjectName("PathFinding");
#ifndef NOTHREADS
  start();
  moveToThread(this);
#endif
  if (!connect(this, &PathFinding::internalCancel, this, &PathFinding::cancel,
               Qt::BlockingQueuedConnection))
    abort();
  if (!connect(this, &PathFinding::emitSearchPath, this,
               &PathFinding::internalSearchPath, Qt::QueuedConnection))
    abort();
}
...
void PathFinding::searchPath(
    const std::unordered_map<std::string, Map_full *> &all_map,
    const std::string &destination_map, const uint8_t &destination_x,
    const uint8_t &destination_y, const std::string &current_map,
  ...
  tryCancel = false;
  std::unordered_map<std::string, SimplifiedMapForPathFinding *>
      map_list;
  // load the data
  for (const auto &n : all_map) {
    ...
    if (map->displayed) {
      SimplifiedMapForPathFinding *simplifiedMap =
          new SimplifiedMapForPathFinding;
      ...
      if (map->logicalMap.parsed_layer.simplifiedMap == NULL)
        ...
        simplifiedMap->simplifiedMap =
            new uint8_t[simplifiedMap->width * simplifiedMap->height];
        memcpy(simplifiedMap->simplifiedMap,
               map->logicalMap.parsed_layer.simplifiedMap,
               simplifiedMap->width * simplifiedMap->height);
      }
      map_list[n.first] = simplifiedMap;
    }
  }
  // resolv the border
  for (const auto &n : map_list) {
    const std::string &mapString = n.first;
    SimplifiedMapForPathFinding *simplifiedMap = map_list[mapString];
    if (simplifiedMap == nullptr) {
      continue;
    }
    if (all_map.find(
            all_map.at(mapString)->logicalMap.border_semi.bottom.fileName) !=
        all_map.cend()) {
      simplifiedMap->border.bottom.map =
          map_list[all_map.at(mapString)
                                ->logicalMap.border_semi.bottom.fileName];
      simplifiedMap->border.bottom.x_offset =
          all_map
              .at(all_map.at(mapString)->logicalMap.border_semi.bottom.fileName)
              ->logicalMap.border_semi.top.x_offset -
    ...
    if (all_map.find(
            all_map.at(mapString)->logicalMap.border_semi.left.fileName) !=
        all_map.cend()) {
      simplifiedMap->border.left.map =
          map_list[all_map.at(mapString)
                                ->logicalMap.border_semi.left.fileName];
      simplifiedMap->border.left.y_offset =
          all_map
              .at(all_map.at(mapString)->logicalMap.border_semi.left.fileName)
              ->logicalMap.border_semi.right.y_offset -
    ...
    if (all_map.find(
            all_map.at(mapString)->logicalMap.border_semi.right.fileName) !=
        all_map.cend()) {
      simplifiedMap->border.right.map =
          map_list[all_map.at(mapString)
                                ->logicalMap.border_semi.right.fileName];
      simplifiedMap->border.right.y_offset =
          all_map
              .at(all_map.at(mapString)->logicalMap.border_semi.right.fileName)
              ->logicalMap.border_semi.left.y_offset -
    ...
    if (all_map.find(
            all_map.at(mapString)->logicalMap.border_semi.top.fileName) !=
        all_map.cend()) {
      simplifiedMap->border.top.map =
          map_list[all_map.at(mapString)
                                ->logicalMap.border_semi.top.fileName];
      simplifiedMap->border.top.x_offset =
          all_map
              .at(all_map.at(mapString)->logicalMap.border_semi.top.fileName)
              ->logicalMap.border_semi.bottom.x_offset -
  ...
  // load for thread and unload if needed
  {
    ...
    this->simplifiedMapList = map_list;
  }
  emit emitSearchPath(destination_map, destination_x, destination_y,
                      current_map, x, y, items);
}

bool PathFinding::tryMove(SimplifiedMapForPathFinding *&current_map, uint8_t &x,
                          uint8_t &y,
                          const CatchChallenger::Orientation &orientation) {
...
bool PathFinding::canGo(
    const SimplifiedMapForPathFinding &simplifiedMapForPathFinding,
    const uint8_t &x, const uint8_t &y) {
  ...
  const uint8_t &var =
      simplifiedMapForPathFinding
          .simplifiedMap[x + y * (simplifiedMapForPathFinding.width)];
  return var == 255 || var < 200;
}

#ifdef CATCHCHALLENGER_EXTRA_CHECK
void PathFinding::extraControlOnData(
    const std::vector<std::pair<CatchChallenger::Orientation,
                                uint8_t /*step number*/> > &controlVar,
    const CatchChallenger::Orientation &orientation) {
  if (!controlVar.empty()) {
    unsigned int index = 1;
    CatchChallenger::Orientation lastOrientation = controlVar.front().first;
    while (index < controlVar.size()) {
      const std::pair<CatchChallenger::Orientation, uint8_t /*step number*/>
          &controlVarCurrent = controlVar.at(index);
      ...
      if (lastOrientation == CatchChallenger::Orientation_left) {
        if (controlVarCurrent.first != CatchChallenger::Orientation_bottom &&
            controlVarCurrent.first != CatchChallenger::Orientation_top) {
      ...
      if (lastOrientation == CatchChallenger::Orientation_top) {
        if (controlVarCurrent.first != CatchChallenger::Orientation_right &&
            controlVarCurrent.first != CatchChallenger::Orientation_left) {
...
std::string PathFinding::stepToString(
    const std::vector<std::pair<CatchChallenger::Orientation,
                                uint8_t /*step number*/> > &data) {
  std::string r;
  unsigned int index = 0;
  while (index < data.size()) {
    const std::pair<CatchChallenger::Orientation, uint8_t /*step number*/>
        &controlVarCurrent = data.at(index);
...
void PathFinding::internalSearchPath(
  ...
  std::unordered_map<const SimplifiedMapForPathFinding *, std::string>
      mapToString;
  std::unordered_map<const SimplifiedMapForPathFinding *, std::string>
      mapToStringFull;
  ...
  if (simplifiedMapList.find(current_map) == simplifiedMapList.cend()) {
    ...
    std::cerr
        << "bug: simplifiedMapList.find(current_map)==simplifiedMapList.cend()"
        << std::endl;
    return;
  }
  if (simplifiedMapList.at(current_map)->simplifiedMap == nullptr)
    std::cout << "PathFinding::canGoOn() walkable is NULL for " << current_map
              << std::endl;

  MapPointToParse fromPoint;
  fromPoint.map = current_map;
  fromPoint.x = x;
  fromPoint.y = y;
  // resolv the path
  if (!tryCancel) {
    ...
    // init the first case
    {
      ...
      tempMap->pathToGo[coord].left.push_back(
          std::pair<CatchChallenger::Orientation, uint8_t /*step number*/>(
              CatchChallenger::Orientation_left, 1));
      tempMap->pathToGo[coord].right.push_back(
          std::pair<CatchChallenger::Orientation, uint8_t /*step number*/>(
              CatchChallenger::Orientation_right, 1));
      tempMap->pathToGo[coord].bottom.push_back(
          std::pair<CatchChallenger::Orientation, uint8_t /*step number*/>(
              CatchChallenger::Orientation_bottom, 1));
      tempMap->pathToGo[coord].top.push_back(
          std::pair<CatchChallenger::Orientation, uint8_t /*step number*/>(
              CatchChallenger::Orientation_top, 1));
    }
    ...
    while (!mapPointToParseList.empty()) {
      ...
      const SimplifiedMapForPathFinding *scannedPoint =
          simplifiedMapList.at(current_map);
      mapPointToParseList.erase(mapPointToParseList.cbegin());
      SimplifiedMapForPathFinding::PathToGo pathToGo;
      if (destination_map == current_map && tempPoint.x == destination_x &&
          tempPoint.y == destination_y)
      ...
      while (index < 1) /*2*/
      {
        ...
        }
        {
          // if the right case have been parsed
          {
            ...
            SimplifiedMapForPathFinding *simplifiedMapForPathFinding =
                simplifiedMapList.at(newPoint.map);
            if (PathFinding::tryMove(
                    simplifiedMapForPathFinding, newPoint.x, newPoint.y,
                    CatchChallenger::Orientation::Orientation_right)) {
              const std::pair<uint8_t, uint8_t> coord(newPoint.x, newPoint.y);
              if (simplifiedMapForPathFinding->pathToGo.find(coord) !=
                  simplifiedMapForPathFinding->pathToGo.cend()) {
                const SimplifiedMapForPathFinding::PathToGo &nearPathToGo =
                    simplifiedMapForPathFinding->pathToGo.at(coord);
                if (pathToGo.left.empty() ||
                    pathToGo.left.size() > nearPathToGo.left.size()) {
                ...
                if (pathToGo.top.empty() ||
                    pathToGo.top.size() > (nearPathToGo.left.size() + 1)) {
                  pathToGo.top = nearPathToGo.left;
                  pathToGo.top.push_back(std::pair<CatchChallenger::Orientation,
                                                   uint8_t /*step number*/>(
                      CatchChallenger::Orientation_top, 1));
                }
                if (pathToGo.bottom.empty() ||
                    pathToGo.bottom.size() > (nearPathToGo.left.size() + 1)) {
                  pathToGo.bottom = nearPathToGo.left;
                  pathToGo.bottom.push_back(
                      std::pair<CatchChallenger::Orientation,
                                uint8_t /*step number*/>(
                          CatchChallenger::Orientation_bottom, 1));
                }
                // debug
                /*std::cout << "1a)" << std::to_string(newPoint.x) << "," <<
            ...
            if (PathFinding::tryMove(
              ...
              if (simplifiedMapForPathFinding->pathToGo.find(coord) !=
                ...
                if (pathToGo.right.empty() ||
                    pathToGo.right.size() > nearPathToGo.right.size()) {
                ...
                if (pathToGo.top.empty() ||
                    pathToGo.top.size() > (nearPathToGo.right.size() + 1)) {
                ...
                if (pathToGo.bottom.empty() ||
                    pathToGo.bottom.size() > (nearPathToGo.right.size() + 1)) {
                ...
                // debug
                /*std::cout << "1b)" << std::to_string(newPoint.x) << "," <<
                   std::to_string(newPoint.y)
                          << " \"" << stepToString(pathToGo.left) << "\""
                          << " \"" << stepToString(pathToGo.right) << "\""
                          << " \"" << stepToString(pathToGo.top) << "\""
            ...
            if (PathFinding::tryMove(
              ...
              if (simplifiedMapForPathFinding->pathToGo.find(coord) !=
                ...
                const SimplifiedMapForPathFinding::PathToGo &nearPathToGo =
                    simplifiedMapForPathFinding->pathToGo.at(coord);
                if (pathToGo.top.empty() ||
                    pathToGo.top.size() > nearPathToGo.top.size()) {
                ...
                if (pathToGo.left.empty() ||
                    pathToGo.left.size() > (nearPathToGo.top.size() + 1)) {
                  pathToGo.left = nearPathToGo.top;
                  pathToGo.left.push_back(
                      std::pair<CatchChallenger::Orientation,
                                uint8_t /*step number*/>(
                          CatchChallenger::Orientation_left, 1));
                }
                if (pathToGo.right.empty() ||
                    pathToGo.right.size() > (nearPathToGo.top.size() + 1)) {
                  pathToGo.right = nearPathToGo.top;
                  pathToGo.right.push_back(
                      std::pair<CatchChallenger::Orientation,
                                uint8_t /*step number*/>(
                          CatchChallenger::Orientation_right, 1));
                }
                // debug
                /*std::cout << "1c)" << std::to_string(newPoint.x) << "," <<
                   std::to_string(newPoint.y)
                          << " \"" << stepToString(pathToGo.left) << "\""
                          << " \"" << stepToString(pathToGo.right) << "\""
                          << " \"" << stepToString(pathToGo.top) << "\""
            ...
            if (PathFinding::tryMove(
              ...
              if (simplifiedMapForPathFinding->pathToGo.find(coord) !=
                ...
                const SimplifiedMapForPathFinding::PathToGo &nearPathToGo =
                    simplifiedMapForPathFinding->pathToGo.at(coord);
                if (pathToGo.bottom.empty() ||
                    pathToGo.bottom.size() > nearPathToGo.bottom.size()) {
                ...
                if (pathToGo.left.empty() ||
                    pathToGo.left.size() > (nearPathToGo.bottom.size() + 1)) {
                ...
                if (pathToGo.right.empty() ||
                    pathToGo.right.size() > (nearPathToGo.bottom.size() + 1)) {
                ...
                // debug
                /*std::cout << "1d)"  << std::to_string(newPoint.x) << "," <<
                   std::to_string(newPoint.y)
                          << " \"" << stepToString(pathToGo.left) << "\""
                          << " \"" << stepToString(pathToGo.right) << "\""
                          << " \"" << stepToString(pathToGo.top) << "\""
      ...
      if (scannedPoint->pathToGo.find(coord) == scannedPoint->pathToGo.cend()) {
        ...
        extraControlOnData(pathToGo.bottom,
                           CatchChallenger::Orientation_bottom);
#endif
        ...
        if(pos != std::string::npos)
            s=s.substr(pos+1);
        std::cout << s << ":" << std::to_string(tempPoint.x) << "," <<
        std::to_string(tempPoint.y)
                  << " \"" << stepToString(pathToGo.left) << "\""
                  << " \"" << stepToString(pathToGo.right) << "\""
                  << " \"" << stepToString(pathToGo.top) << "\""
                  << " \"" << stepToString(pathToGo.bottom) << "\""
      ...
      if (destination_map == current_map && tempPoint.x == destination_x &&
        ...
        if (returnedVar.empty() || pathToGo.bottom.size() < returnedVar.size())
          if (!pathToGo.bottom.empty()) returnedVar = pathToGo.bottom;
        if (returnedVar.empty() || pathToGo.top.size() < returnedVar.size())
          if (!pathToGo.top.empty()) returnedVar = pathToGo.top;
        if (returnedVar.empty() || pathToGo.right.size() < returnedVar.size())
          if (!pathToGo.right.empty()) returnedVar = pathToGo.right;
        if (returnedVar.empty() || pathToGo.left.size() < returnedVar.size())
          if (!pathToGo.left.empty()) returnedVar = pathToGo.left;
        if (!returnedVar.empty()) {
          if (returnedVar.back().second <= 1) {
            std::cerr << "Bug due for last step" << std::endl;
            return;
          } else {
            std::cout << "Path result into " << time.elapsed() << "ms from "
                      << fromPoint.map << " " << std::to_string(fromPoint.x)
                      << "," << std::to_string(fromPoint.y) << std::endl;
            returnedVar.back().second--;
            emit result(fromPoint.map, fromPoint.x, fromPoint.y, returnedVar);
            return;
          }
        } else {
          returnedVar.clear();
      ...
      // add to point to parse
      {
        // if the right case have been parsed
        {
          ...
          if (PathFinding::tryMove(
                  simplifiedMapForPathFinding, newPoint.x, newPoint.y,
                  CatchChallenger::Orientation::Orientation_right)) {
            if (mapToString.find(simplifiedMapForPathFinding) ==
                mapToString.cend())
              abort();
            if (PathFinding::canGo(*simplifiedMapForPathFinding, newPoint.x,
                                   newPoint.y) ||
                (destination_map ==
                     mapToStringFull.at(simplifiedMapForPathFinding) &&
                 newPoint.x == destination_x && newPoint.y == destination_y)) {
              std::pair<uint8_t, uint8_t> p(newPoint.x, newPoint.y);
              if (simplifiedMapForPathFinding->pointQueued.find(p) ==
                  simplifiedMapForPathFinding->pointQueued.cend()) {
        ...
        // if the left case have been parsed
        {
          ...
          if (PathFinding::tryMove(
            ...
            /*
                                    ...
                                    if(pos != std::string::npos)
                                        m1=m1.substr(pos+1);
                                    std::cout << "PathFinding::canGo() " <<
               mapToString.at(simplifiedMapForPathFinding) << "," <<
               std::to_string(newPoint.x) << "," << std::to_string(newPoint.y)
               << ")"
                                              << " OR == destination: " << m1 <<
               " " << std::to_string(destination_x) << "," <<
               std::to_string(destination_y) << std::endl;*/
            if (PathFinding::canGo(*simplifiedMapForPathFinding, newPoint.x,
                                   newPoint.y) ||
                (destination_map ==
                     mapToStringFull.at(simplifiedMapForPathFinding) &&
  ...
  // drop the local variable
  {
    for (auto &n : simplifiedMapList) {
      ...
      if (simplifiedMapForPathFinding != nullptr) {
  ...
  tryCancel = false;
  emit result(std::string(), 0, 0,
              std::vector<std::pair<CatchChallenger::Orientation, uint8_t> >());
  std::cerr << "Path not found into " << time.elapsed() << "ms" << std::endl;
}

void PathFinding::cancel() { tryCancel = true; }
