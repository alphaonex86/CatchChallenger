...
#ifndef CLIENT_V3_SCENES_SHARED_WAREHOUSE_WAREHOUSE_HPP_
#define CLIENT_V3_SCENES_SHARED_WAREHOUSE_WAREHOUSE_HPP_
...
namespace Scenes {
class Warehouse : public UI::Dialog {
 public:
 ...
 private:
  ...
  WarehouseItem *ItemToItem(const uint16_t &item_id, const uint32_t &quantity);
  WarehouseItem *ItemToMonster(const uint32_t &monster_id,
                               const uint16_t &monster,
                               const uint32_t &quantity);
  std::vector<CatchChallenger::PlayerMonster> MonstersWithdrawed() const;
};
}  // namespace Scenes
#endif  // CLIENT_V3_SCENES_SHARED_WAREHOUSE_WAREHOUSE_HPP_
