...
&playerInformations=client->get_player_informations_ro();
    ...
    {
        QMessageBox::warning(this,tr("No requirements"),tr("You don't meet the
requirements")); return;
    }
    int quantityToBuy=1;
...
1, &ok); if(!ok || quantityToBuy<=0) return;
    ...
    {
        const Industry
&industry=CommonDatapack::commonDatapack.industries.at(CommonDatapack::commonDatapack.industriesLink.at(factoryId).industry);
...
selectedItems=ui->factoryResources->selectedItems(); if(selectedItems.size()!=1)
        return;
...
{
    const CatchChallenger::Player_private_and_public_informations
&playerInformations=client->get_player_informations_ro(); uint16_t
itemid=static_cast<uint16_t>(item->data(99).toUInt()); uint32_t
price=item->data(98).toUInt(); uint32_t quantity=item->data(97).toUInt();
    if(playerInformations.items.find(itemid)==playerInformations.items.cend())
    {
        QMessageBox::warning(this,tr("No item"),tr("You have not the item to
sell")); return;
    }
    if(!haveReputationRequirements(CommonDatapack::commonDatapack.industriesLink.at(factoryId).requirements.reputation))
    {
...
void OverMapLogic::haveBuyFactoryObject(const CatchChallenger::BuyStat &stat,
...
void OverMapLogic::haveSellFactoryObject(const CatchChallenger::SoldStat &stat,
                                         const uint32_t &newPrice) {
  waitToSell = false;
  switch (stat) {
    case CatchChallenger::SoldStat_Done:
      if (industryStatus.resources.find(itemsToSell.front().object) ==
          industryStatus.resources.cend())
        industryStatus.resources[itemsToSell.front().object] = 0;
      industryStatus.resources[itemsToSell.front().object] +=
          itemsToSell.front().quantity;
    ...
    case CatchChallenger::SoldStat_WrongQuantity:
      add_to_inventory(itemsToSell.front().object, itemsToSell.front().quantity,
                       false);
      showTip(tr("Sorry but have not the quantity of this item").toStdString());
      /*load_inventory();
...
void OverMapLogic::haveFactoryList(
  ...
  const CatchChallenger::Industry
  &industry=CatchChallenger::CommonDatapack::commonDatapack.industries.at(CatchChallenger::CommonDatapack::commonDatapack.industriesLink.at(factoryId).industry);
  ...
  while(index<resources.size())
  {
      ...
      {
          const CatchChallenger::Industry::Resource
  &resource=industry.resources.at(sub_index);
  ...
  &resource=industry.resources.at(sub_index);
          ...
          {
              std::cerr << "Ressource " << resource.item << " not returned,
  consider as full: " << industry.cycletobefull*resource.quantity << std::endl;
...
void OverMapLogic::updateFactoryStatProduction(
  ...
  if((uint64_t)(industryStatus.last_update+industry.time)<(uint64_t)(QDateTime::currentMSecsSinceEpoch()/1000))
          ...
          if(remainingProductionTime>0)
          {
              ...
              if(remainingProductionTime<60)
                  ...
                  productionTime+=tr("%n
  minute(s)","",static_cast<uint32_t>(remainingProductionTime/60)).toStdString();
          }
          ui->factoryStatText->setText(tr("In production")+"<br
  />"+QString::fromStdString(productionTime));
      }
      else
...
/*void OverMapLogic::factoryToResourceItem(QListWidgetItem *item)
{
    abort();
    const CatchChallenger::Player_private_and_public_informations
&playerInformations=client->get_player_informations_ro(); QFont MissingQuantity;
    MissingQuantity.setItalic(true);
    if(item->data(97).toUInt()>1)
        item->setText(QStringLiteral("%1 at
%2$").arg(item->data(97).toUInt()).arg(item->data(98).toUInt())); else
...
%2$").arg(itemId).arg(item->data(98).toUInt())); else item->setToolTip(tr("Item
%1 at %2$\nQuantity:
%3").arg(itemId).arg(item->data(98).toUInt()).arg(item->data(97).toUInt()));
    }
    if(playerInformations.items.find(static_cast<uint16_t>(item->data(99).toUInt()))==playerInformations.items.cend()
||
!haveReputationRequirements(CommonDatapack::commonDatapack.industriesLink.at(factoryId).requirements.reputation))
    {
