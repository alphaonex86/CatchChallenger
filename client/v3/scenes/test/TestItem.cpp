// Copyright 2021 CatchChallenger
#include "TestItem.hpp"

#include <QPainter>
#include <iostream>

#include "../../core/AssetsLoader.hpp"
#include "../../core/Sprite.hpp"
#include "../../ui/Label.hpp"

using Scenes::TestItem;

TestItem::TestItem() {
  SetSize(48, 48);
  is_selected_ = false;
}

TestItem *TestItem::Create() {
  return new (std::nothrow) TestItem();
}

void TestItem::SetPixmap(const QPixmap pixmap) { pixmap_ = pixmap; }

void TestItem::SetText(const QString text) { text_ = text; }

QString TestItem::Text() const { return text_; }

void TestItem::SetSelected(bool selected) {
  is_selected_ = selected;
  ReDraw();
}

bool TestItem::IsSelected() { return is_selected_; }

void TestItem::Draw(QPainter *painter) {
  const auto size = BoundingRect();
  QPixmap background = *AssetsLoader::GetInstance()->GetImage(
      ":/CC/images/interface/green-square.png");
  if (is_selected_) {
    painter->drawPixmap(0, 0, size.width(), size.height(), background, 0, 0,
                        114, 117);
  } else {
    painter->drawPixmap(0, 0, size.width(), size.height(), background, 0, 117,
                        114, 117);
  }
  painter->drawPixmap(0, 0, pixmap_);

  auto text = UI::Label::Create();
  text->SetPixelSize(10);
  text->SetPos(40, 0);
  text->SetText(text_);
  text->Render(painter);

  delete text;
}

void TestItem::MousePressEvent(const QPointF &point,
                                    bool &press_validated) {
  if (press_validated) {
    is_selected_ = false;
    ReDraw();
    return;
  }
  const QRectF &b = BoundingRect();
  const QRectF &t = MapRectToScene(b);
  if (t.contains(point)) {
    press_validated = true;
  }
}

void TestItem::MouseReleaseEvent(const QPointF &point,
                                      bool &prev_validated) {
  if (prev_validated) {
    is_selected_ = false;
    ReDraw();
    return;
  }

  const QRectF &b = BoundingRect();
  const QRectF &t = MapRectToScene(b);
  if (!prev_validated && IsVisible()) {
    if (t.contains(point)) {
      is_selected_ = true;
      ReDraw();
      if (on_click_) {
        on_click_(this);
      }
    }
  }
}

void TestItem::OnResize() {
  ReDraw();
}

